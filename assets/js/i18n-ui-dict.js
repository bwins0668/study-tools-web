// Study Tools static UI multilingual dictionary
(function () {
  "use strict";

  window.I18nUiDict = {
    "ja-JP": {
      home: {
        eyebrow: "学習ワークスペース",
        title: "今日も続けましょう",
        currentContext: "現在の学習",
        startLearning: "学習を始める",
        continueLearning: "続きから学習",
        reviewCourse: "最初から復習",
        courseDone: "完了",
        goContinue: "続ける",
        emptyHint: "学習記録はまだありません。SQL の第1課から始めるのがおすすめです。",
        openDashboard: "学習統計",
        progressOverview: "コース進捗",
        quickActions: "クイックアクション",
        openGlossary: "用語辞典"
      },
      lessonNav: {
        title: "レッスンナビゲーション",
        prev: "前の課",
        next: "次の課",
        completed: "この課は完了済み",
        notCompleted: "未完了 · 課題クリアで完了"
      },
      common: {
        save: "保存",
        cancel: "キャンセル",
        confirm: "確認",
        delete: "削除",
        reset: "リセット",
        resetProgress: "進捗をリセット",
        loading: "読み込み中...",
        themeToggle: "ライト/ダークモードを切り替え",
        close: "閉じる",
        open: "開く",
        search: "検索",
        random: "ランダム",
        restart: "最初から",
        copy: "コピー",
        maximize: "最大化",
        minimize: "最小化",
        challenging: "挑戦中",
        lessonIndex: "第{id}課",
        changeQuestion: "別の問題",
        stop: "終了",
        subtitle: "学習支援プラットフォーム"
      },
      nav: {
        sql: "SQL学習",
        java: "Java学習",
        python: "Python学習",
        itpass: "ITパスポート対策",
        sg: "情報セキュリティマネジメント対策",
        typing: "日本語タイピング",
        codingTyping: "プログラミングタイピング",
        sqlChallenge: "総合ランダム挑戦"
      },
      moduleSwitch: {
        title: "学習セクションを選択",
        close: "閉じる",
        triggerLabel: "セクション選択を開く"
      },
      moduleDesc: {
        sql: "データベース基礎、クエリ、練習、サンドボックス",
        sg: "情報セキュリティマネジメント試験対策",
        itpass: "ITパスポート基礎と過去問演習",
        typing: "日本語入力とタイピング練習",
        codingTyping: "コード入力と構文練習",
        java: "Java の基礎、構文、練習",
        python: "Python の基礎、構文、練習"
      },
      codingTyping: {
        title: "プログラミングタイピング練習",
        subtitle: "タイピング練習でプログラミング文法をマスター。言語とカテゴリを選んで始めましょう。",
        language: "言語選択",
        category: "カテゴリ",
        allCategories: "すべて",
        difficulty: "難易度",
        code: "目標コード",
        input: "入力エリア",
        inputPlaceholder: "目標コードを入力...",
        focus: "練習ポイント",
        hint: "ヒント",
        speed: "CPM",
        accuracy: "正確率",
        progress: "進捗",
        time: "時間",
        errors: "エラー",
        complete: "完了！",
        result: "結果",
        totalTime: "所用時間",
        errorChars: "よく間違える文字",
        recentHistory: "最近の履歴",
        noHistory: "履歴なし",
        next: "次の問題 (Alt+N)",
        random: "ランダム (Alt+S)",
        restart: "やり直し (Alt+R)",
        continueRandom: "ランダム練習を続ける",
        python: "Python",
        java: "Java",
        sql: "SQL",
        symbols: "記号",
        normalizeFullWidth: "全角→半角",
        lenientIndent: "インデント寛容",
        sendToSandbox: "サンドボックスに送信",        runInSandbox: "サンドボックスで実行",        sandboxUnavailable: "サンドボックス利用不可",        sentToSandbox: "サンドボックスに送信しました",        openSandbox: "サンドボックスを開く",        shortcutHelp: "ショートカット: Alt+N 次 / Alt+R 再 / Alt+S 随机",
        summaryLine: "CPM: {cpm} | 正確率: {acc}% | エラー: {err} | 時間: {time}",
        examInsight: "試験理解カード",
        examRelevance: "出題頻度",
        examTargets: "試験対策対象",
        relatedTerms: "関連用語",
        meaning: "意味",
        memoryHook: "覚え方",
        examPoint: "試験ポイント",
        commonMistake: "間違えやすい点",
        high: "高",
        medium: "中",
        low: "低",
        backToList: "練習リストへ戻る",
        exerciseList: "練習リスト"
      },
      subnav: {
        lessons: "テキスト・演習",
        practicalExam: "実技模擬試験",
        study: "テキスト学習",
        pastExams: "過去問道場・模擬試験",
        javaIntro: "入門編 — 第1〜8章",
        javaPractice: "実践編 — 第1〜11章"
      },
      sidebar: {
        progress: "進捗状況",
        unitList: "単元リスト"
      },
      sandbox: {
        run: "実行",
        playground: "サンドボックス",
        runSql: "SQLを実行",
        resetDb: "DBリセット",
        hint: "ヒント",
        maximize: "最大化",
        output: "実行結果",
        sqlPlaceholder: "ここにSQLを入力してください...",
        currentMission: "ミッション (当前任务)",
        randomPractice: "ランダム指令練習",
        databaseStructure: "データベース構造 (DB 表结构参考)",
        schoolDb: "学校DB",
        bookstoreDb: "書店DB",
        javaSandboxTitle: "Java プログラミング沙盒 / Java编程沙盒",
        pythonSandboxTitle: "Python プログラミング沙盒 / Python编程沙盒",
        pythonSandboxDesc: "Python 3.12 · 10s · オフラインサンドボックス",
        sqlPlayground: "SQL プレイグラウンド",
        chooseLessonLoadCode: "— レッスンを選択してコードをロード —",
        toolbarTemplate: "テンプレ",
        toolbarTemplateTitle: "テンプレートを読み込む",
        toolbarCopy: "コピー",
        toolbarCopyTitle: "コードをコピー",
        toolbarClear: "リセット",
        toolbarClearTitle: "コードをクリア",
        javaEditorPlaceholder: "Java コードをここに入力してください...",
        pythonEditorPlaceholder: "Python コードをここに入力してください...",
        stdinHeader: "標準入力 (stdin)",
        stdinPlaceholder: "標準入力の値をここに入力...",
        runCode: "実行",
        readyStatus: "準備完了 / Ready",
        outputPlaceholder: "（実行ボタンを押すと、ここに結果が表示されます）"
      },
      exam: {
        allExamTitle: "総合ランダム模擬試験",
        yearExamTitle: "{year} 公開問題 模擬試験",
        verify: "判定して保存",
        submit: "試験終了",
        previous: "前へ",
        next: "次へ",
        exit: "試験離脱",
        ruleTitle: "過去問道場 — CBT模擬試験システム",
        ruleDesc: "過去のIT試験に基づき、実際のCBT環境をシミュレートします。",
        ruleDetailTitle: "試験規則:",
        ruleDetail1: "問題はすべて実技プログラミング/SQLクエリで、日本語タスクまたは期待出力が与えられます。",
        ruleDetail2: "右側のシミュレータエディタで即座にコードを記述・実行・デバッグできます。",
        ruleDetail3: "コード完成後、「判定して保存」をクリックしてテストケース検証を行います。",
        ruleDetail4: "テスト合格（Passed ✓）で得点となり、合格するまで何度でも修正可能です。",
        ruleDetail5: "制限時間内に「試験終了」をクリックしてください。タイムアウト時は自動回収されます。",
        selectYear: "試験年度を選択:",
        questionCount: "模擬試験の問題数:",
        launchBtn: "試験開始",
        status: "テスト状態: ",
        notStarted: "未着手",
        cbtSimulator: "CBT模擬システム",
        remainingTime: "残り時間: ",
        exitCbt: "中断退出",
        submitCbt: "試験提出",
        questionNumber: "問 {current}",
        flagReview: "この問題をマーク",
        metaExpand: "問題情報 <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "閉じる <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "出典：",
        metaYear: "年度",
        metaQNum: "問番号",
        metaCategory: "分類",
        metaSubcategory: "サブカテゴリ",
        metaTopic: "トピック",
        ruleTitleCoding: "実技プログラミング模擬試験",
        ruleDescCoding: "IT専門学校のプログラミング試験環境を再現。制限時間内にコードを記述します。",
        launchBtnCoding: "試験開始",
        cbtSimulatorCoding: "実技模擬試験",
        expectedOutput: "期待される出力結果",
        resultsTitle: "評価結果",
        resultsTitleCbt: "模擬試験成績書",
        backToHome: "ホームに戻る",
        backToDojo: "道場に戻る",
        passLabel: "合否判定",
        passStatus: "合格",
        failStatus: "不合格",
        correctCountText: "{count} / {total} 問合格",
        totalScoreText: "{score} / 1000 点",
        correctRate: "得点率",
        timeSpent: "所要時間:",
        detailsTitle: "各問題の判定詳細",
        reviewTitle: "問題の振り返りと誤答分析",
        reviewTip: "行をクリックすると、問題の詳細と中日二言語の解説を展開できます。",
        reviewCollapse: "解説を閉じる",
        reviewExplanation: "解説",
        resultCorrect: "正解",
        resultIncorrect: "不正解",
        tableQNo: "問",
        tableTitle: "タイトル",
        tableField: "分野",
        tableDifficulty: "難易度",
        tableResult: "判定結果",
        tableYourAnswer: "解答",
        tableCorrectAnswer: "正解",
        tableAction: "操作",
        tableActionDetail: "詳細",
        personalizeTitle: "模擬試験の個人化設定:",
        optShuffle: "選択肢の順序をシャッフル (ア〜エ)",
        optNocalc: "計算問題をフィルター (計算式を含む問題を表示しない)",
        all: "過去問ランダムミックス (推奨 - 総合模擬試験)",
        r8: "令和8年度 (R8) 公開問題",
        r7: "令和7年度 (R7) 公開問題",
        r6: "令和6年度 (R6) 公開問題",
        r5: "令和5年度 (R5) 公開問題",
        r4: "令和4年度 (R4) 公開問題",
        r3: "令和3年度 (R3) 公開問題",
        cbtQ100: "100問 (正式CBT基準 - 制限時間120分)",
        cbtQ50: "50問 (短縮模擬試験 - 制限時間60分)",
        cbtQ30: "30問 (クイック演習 - 制限時間36分)",
        cbtQ10: "10問 (ミニテスト - 制限時間12分)",
        q5: "5問 (クイックテスト - 15分)",
        q10: "10問 (標準実技 - 30分)",
        q15: "15問 (フルチャレンジ - 45分)",
        gridTitle: "マークシート (解答状況)",
        unanswered: "未答",
        answered: "既答",
        flagged: "マーク",
        totalScoreLabel: "総合得点",
        domainStrat: "ストラテジ系",
        domainMan: "マネジメント系",
        domainTech: "テクノロジ系",
        domainSubb: "科目B"
      },
      toast: {
        alreadyFirst: "最初の問題です。",
        alreadyLast: "最後の問題です。",
        saved: "保存しました。",
        verifySuccess: "判定に合格しました。答案を保存しました。",
        dbReset: "模擬データベースを初期状態に戻しました。",
        examStarted: "CBT模擬試験を開始しました。",
        lessonQuizPassed: "🏆 {title} — クイズ合格！単元完了としてマークされました！",
        dbSwitched: "🗄️ 《{dbName}》データベースに切り替えました。現在、{tables} などのテーブルをクエリできます。",
        sqlHint: "💡 ヒント：以下の SQL 構文を参考にしてください：<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ 完了したレッスンがありません！まず少なくとも1つのレッスンを学習して完了してください。",
        challengeStarted: "🔥 総合ランダム挑戦開始！完了した {lessonCount} レッスンから計 {count} 問が出題されます。",
        challengeEnded: "✅ 総合ランダム挑戦が終了しました。",
        challengeQuestionCorrect: "🎉 正解です！レッスン {sourceId} からの問題をクリアしました！",
        challengeSuccess: "総合ランダム挑戦達成！",
        examEmpty: "⚠️ 条件に合う過去問が見つかりません。オプションを変更してください！",
        examTimeout: "⏰ 制限時間になりました。自動的に答案を提出します！",
        examPassed: "🎉 おめでとうございます！今回の模擬テストに合格しました！",
        examFailed: "💪 合格基準に達していません。引き続き頑張りましょう！",
        examDbNotFound: "⚠️ この科目の問題データベースが見つかりません！",
        practicalExamStarted: "📝 実技模擬試験が開始されました！タスクに従ってコードを記述してください。",
        sqlSyntaxError: "❌ SQL構文エラー。テスト不合格！",
        sqlOutputMismatch: "❌ 実行結果が期待値と異なります。ソート、フィルタ、または列数を確認してください！",
        compileError: "❌ コンパイルエラー。判定失敗！",
        runtimeError: "❌ 実行エラー。判定失敗！",
        outputMismatch: "❌ 出力結果が一致しません。判定失敗！",
        practicalExamSubmitted: "🎉 実技試験の提出に成功しました！成績書と評価結果が生成されました。"
      },
      dialog: {
        resetDbConfirm: "模擬データベースを初期状態に戻します。続行しますか？",
        exitExamConfirm: "現在の試験を中断して終了しますか？進捗は保存されません。",
        switchExamConfirm: "現在実行中の試験があります。科目を切り替えると現在の試験は中断されます。切り替えますか？",
        clearCodeConfirm: "コードをクリアしますか？",
        resetProgressConfirm: "すべての {subject} の学習進捗をリセットしますか？",
        submitExamConfirm: "試験を終了して提出しますか？",
        submitExamUnansweredConfirm: "未解答の問題が {count} 問あります。本当に今提出しますか？",
        submitCodingExamUnansweredConfirm: "未解答または判定未合格の問題が {count} 問あります。本当に今提出しますか？"
      },
      tools: {
        quickStart: "クイックスタート",
        quickStartTitle: "学習の流れ",
        quickStartDesc: "科目を選ぶ → 課程を学ぶ → 練習する → 統計を確認する",
        tools: "ツール",
        openMenu: "ツールメニューを開く",
        closeMenu: "ツールメニューを閉じる",
        learningTools: "学習ツール",
        glossary: "用語集",
        glossaryDesc: "IT / SQL / プログラミング用語を検索",
        settings: "設定",
        settingsDesc: "学習ツールの設定を調整",
        dangerous: "危険な操作",
        resetProgress: "進捗をリセット",
        resetProgressDesc: "現在の学習進捗を消去します。確認が必要です",
        confirmResetTitle: "進捗をリセットしますか？",
        confirmResetBody: "現在の学習進捗が消去されます。続行しますか？",
        cancel: "キャンセル",
        confirmReset: "リセットする",
        examTools: "試験ツール",
        examHistory: "試験履歴",
        examHistoryDesc: "最近の模擬試験記録を確認",
        dashboard: "学習統計",
        dashboardDesc: "学習の進捗とデータ概要を確認",
        exportData: "学習データをエクスポート",
        exportDataDesc: "ローカル学習データのバックアップをダウンロード (JSON)",
        continueLearning: "学習を継続",
        continueLearningDesc: "前回の進捗から継続",
        exportSuccess: "エクスポート成功",
        exportPartialSuccess: "一部の破損データをスキップしてエクスポートしました",
        exportFailed: "エクスポート失敗",
        exportScope: "パスワードやキーを含みません — 学習データのみ",
        importPreview: "バックアップインポートのプレビュー",
        importPreviewDesc: "読み取り専用プレビュー、データは変更されません",
        importSelectFile: "バックアップファイルを選択",
        importParseFailed: "ファイル解析に失敗しました",
        importSensitiveBlocked: "機密フィールドが検出され、ブロックされました",
        importImportable: "インポート可能なデータ",
        importNew: "新規",
        importConflict: "競合",
        importSkipped: "スキップ",
        importIncompatibleVersion: "互換性のないバックアップバージョン",
        importFileTooLarge: "ファイルが大きすぎます",
        importClosePreview: "プレビューを閉じる",
        importSummary: "インポート概要",
        importNoData: "インポート可能なデータがありません",
        importSection: "セクション",
        importSectionLessons: "完了レッスン",
        importSectionQuiz: "クイズ結果",
        importSectionWrongBook: "間違いリスト",
        importSectionBookmarks: "ブックマーク",
        importSectionTyping: "タイピング履歴",
        importSectionExam: "試験履歴",
        importSectionGoals: "学習目標",
        importSectionSettings: "言語とテーマ",
        importSectionSync: "同期状態",
        importSyncBlocked: "インポート不可 — 同期状態はデバイス管理"
      },
      dashboard: {
        dashboard: "学習統計",
        overview: "概要",
        completedLessons: "完了したレッスン",
        wrongBook: "間違えた問題",
        bookmarks: "お気に入り",
        typingPractice: "タイピング練習",
        examHistory: "模擬試験",
        quizCompleted: "クイズ完了",
        learningProgress: "学習進捗",
        wrongBookAndBookmarks: "間違えた問題とお気に入り",
        wrongBookCount: "間違えた問題数",
        bookmarkCount: "お気に入り数",
        latestAdded: "最新追加",
        typingArticles: "タイピング記事",
        japaneseFavorites: "日本語お気に入り",
        quizAndExam: "クイズと試験",
        examCount: "模擬試験回数",
        avgAccuracy: "平均正答率",
        latestExam: "最新の試験",
        japaneseTyping: "日本語タイピング",
        codingTyping: "プログラミングタイピング",
        bestCpm: "最高 CPM",
        syncStatus: "同期状態",
        loginStatus: "ログイン状態",
        loggedIn: "ログイン済み",
        notLoggedIn: "未ログイン",
        lastSync: "最終同期",
        lastSyncResult: "同期結果",
        syncSuccess: "同期成功",
        syncFailed: "同期失敗",
        collectedAt: "データ収集時刻",
        refreshStats: "統計を更新",
        noData: "データなし",
        loadError: "統計データの読み込みに失敗しました",
        infoSec: "情報セキュリティ",
        weeklyTrend: "週間学習トレンド",
        last7Days: "過去7日間",
        noTrendData: "トレンドデータなし",
        learningGoals: "学習目標",
        goalActivities: "週間学習活動",
        goalTyping: "週間タイピング",
        goalExams: "週間模擬試験",
        saveGoals: "目標を保存",
        resetGoals: "リセット",
        goalsSaved: "目標を保存しました",
        goalsReset: "目標をリセットしました",
        noActivity: "活動なし",
        examsShort: "試験",
        jpTypingShort: "日本語",
        ctTypingShort: "コーディング",
        wbShort: "間違い",
        thisWeek: "今週",
        today: "今日",
        dataIncompleteHint: "データが完全でない可能性があります。先に同期することをおすすめします。",
        syncAndRefresh: "同期して更新",
        syncing: "同期中…",
        syncComplete: "同期完了",
        syncFailedHint: "同期に失敗しました",
        loginToSyncHint: "ログインすると複数デバイス間でデータを同期できます"
      },
      examHistory: {
        title: "最近の試験履歴",
        subtitle: "保存された模擬試験結果を確認",
        close: "閉じる",
        filterAll: "すべて",
        empty: "試験履歴はまだありません",
        emptyHint: "模擬試験を完了すると、ここに記録が表示されます",
        questions: "問題数",
        correct: "正解",
        accuracy: "正答率",
        score: "点数",
        passed: "合格",
        failed: "不合格",
        time: "所要時間",
        viewDetails: "詳細を見る",
        hideDetails: "詳細を閉じる",
        examConfig: "試験設定",
        fieldScores: "分野別スコア",
        questionResults: "問題別結果",
        yearFilter: "年度フィルター",
        submittedAt: "提出日時",
        yourAnswer: "あなたの答え",
        correctAnswer: "正解",
        unanswered: "未回答",
        questionNotFound: "問題データが見つかりません"
      },
      wrongBook: {
        title: "間違いノート",
        subtitle: "間違えた問題を復習",
        emptyTitle: "間違えた問題はまだありません",
        emptyDesc: "間違えた問題は自動的にここへ保存されます",
        all: "すべて",
        unmastered: "未習得",
        mastered: "習得済み",
        markMastered: "習得済みにする",
        unmarkMastered: "未習得に戻す",
        remove: "削除",
        wrongCount: "間違えた回数",
        lastWrongAt: "最終間違い日時",
        yourAnswer: "あなたの誤答",
        correctAnswer: "正解",
        explanation: "解説",
        source: "出典",
        confirmRemove: "この問題を間違いノートから削除しますか？",
        viewDetails: "詳細を見る",
        hideDetails: "詳細を閉じる",
        choices: "選択肢",
        question: "問題",
        unknownAnswer: "記録なし"
      },
      message: {
        inputSqlRequired: "SQLクエリを入力してください。"
      },
      settings: {
        language: "言語"
      },
      lesson: {
        locateBookPage: "原書ページへ",
        explanationJa: "解説 (日本語)",
        explanationZh: "解説 (中国語)",
        compareMode: "左右対称 (日/中)",
        japaneseOnly: "日本語のみ",
        chineseOnly: "中国語のみ",
        analogy: "身近な例え",
        sqlExample: "SQL構文例",
        javaExample: "Java コード例",
        pythonExample: "Python コード例",
        glossary: "重要用語・公式",
        copyToEditor: "エディタへコピー",
        showPdf: "原書PDFを表示",
        translationZh: "中国語対訳",
        checklist: "要点確認",
        calculator: "計算ツール",
        calculatorSelect: "ツールを選択:"
      },
      flashcard: {
        title: "単語カード",
        flip: "フリップ",
        clickToFlip: "カードをクリックして裏表を切り替えます：",
        clickToFlipHint: "クリックして裏を見る",
        localizedDesc: "{title} の重要用語を確認するカードです。"
      },
      glossary: {
        title: "IT用語集",
        open: "用語集",
        close: "閉じる",
        searchPlaceholder: "用語を検索...",
        allCategories: "すべて",
        filterDatabase: "データベース",
        filterSecurity: "セキュリティ",
        filterNetwork: "ネットワーク",
        filterProgramming: "プログラミング",
        filterSystem: "システム",
        category: "カテゴリ",
        japanese: "日本語",
        english: "英語",
        currentLanguage: "現在の言語",
        aliases: "別名",
        relatedTerms: "関連用語",
        example: "コード例",
        source: "出典",
        needsReview: "⚠ 翻訳確認中",
        noResults: "該当する用語はありません",
        keepEnglishNote: "英語の技術用語を保持",
        resultCount: "{count} 件の用語"
      },
      quiz: {
        title: "理解度テスト",
        questionCount: "問題 {current} / 全 {total} 問",
        previous: "前へ",
        next: "次へ",
        submitAnswer: "回答する",
        empty: "この章にはまだ小テストがありません。",
        unavailable: "{title} の小テストはまだこの表示言語に対応していません。",
        thisLesson: "この章"
      },
      typing: {
        library: "タイピング文庫",
        libraryDesc: "N3からN1まで幅広く使える実用日本語入力。生活・職場・長文を自分のペースで練習できます。",
        search: "ライブラリ検索",
        searchPlaceholder: "タイトル、内容、またはタグ",
        articleList: "記事一覧",
        randomArticle: "ランダム",
        restart: "最初から",
        timeLimit: "制限時間",
        imeMode: "IMEモード",
        speed: "速度",
        accuracy: "正確率",
        progress: "有効進捗",
        edits: "修正回数",
        time: "時間",
        recentScores: "最近の成績",
        personalReview: "個人復習",
        reviewDesc: "正確率98%未満の記事は自動的に復習リストに追加され、100%で完了すると削除されます。",
        reviewCount: "{count} 件の復習待ち",
        emptyReview: "復習待ちの記事はありません",
        imePlaceholder: "日本語IMEをオンにして、ここから入力してください。",
        typingHelp: "システム日本語IME入力。全角/半角は統一して比較されます。バックスペースなどの修正もカウントされます。",
        resultTitle: "練習完了",
        continueRandom: "ランダム練習を続ける",
        favoriteBtn: "お気に入り",
        favoriteTitle: "この記事をお気に入りに追加",
        noLimit: "制限なし",
        t1min: "1分",
        t3min: "3分",
        t5min: "5分",
        t10min: "10分"
      },
      console: {
        output: "実行結果",
        rows: "{count}行",
        emptyResult: "クエリを実行すると、ここに結果テーブルが表示されます。",
        emptyResultSub: "（SQL実行後、結果がここに表示されます）",
        maximize: "最大化",
        minimize: "最小化"
      },
      keyboardShortcuts: {
        title: "キーボードショートカット",
        hintPress: "押す: ",
        hintSuffix: " でこのダイアログを表示",
        closeButton: "了解",
        cat: {
          general: "一般",
          pythonSandbox: "Python サンドボックス",
          javaSandbox: "Java サンドボックス",
          navigation: "ナビゲーション",
          study: "学習"
        },
        desc: {
          toggleTools: "ツールドロワーを開閉",
          closeDialogs: "ダイアログを閉じる/キャンセル",
          saveProgress: "進捗を保存（エクスポート）",
          runPython: "Pythonコードを実行",
          clearPython: "Pythonエディタをクリア",
          copyPython: "Pythonコードをコピー",
          togglePythonTmpl: "Pythonテンプレートを切り替え",
          runJava: "Javaコードを実行",
          clearJava: "Javaエディタをクリア",
          copyJava: "Javaコードをコピー",
          toggleJavaTmpl: "Javaテンプレートを切り替え",
          switchSql: "SQLモードに切り替え",
          switchItTerm: "IT用語モードに切り替え",
          switchPython: "Pythonモードに切り替え",
          switchJava: "Javaモードに切り替え",
          switchTyping: "タイピングモードに切り替え",
          nextLesson: "次のレッスン",
          prevLesson: "前のレッスン",
          startQuiz: "クイズを開始"
        }
      },
      auth: {
        localMode: "ローカルモード",
        notLoggedIn: "未ログイン",
        account: "アカウント",
        sync: "同期",
        syncInDev: "同期機能は開発中です",
        continueLocal: "ローカル使用を続ける",
        mockSignIn: "ログイン状態を模擬",
        mockSignOut: "模擬ログインを終了",
        exportSnapshot: "ローカルスナップショットをエクスポート",
        deviceId: "デバイス ID",
        pendingSync: "同期待ち",
        lastSync: "最終同期",
        noUpload: "データはアップロードされません",
        close: "閉じる",
        signedIn: "ログイン済み",
        signedOut: "ログアウト",
        status: "ステータス",
        anonymous: "匿名",
        loginTitle: "ログイン",
        registerTitle: "アカウント登録",
        displayName: "ニックネーム",
        email: "メールアドレス",
        password: "パスワード",
        confirmPassword: "パスワード確認",
        loginButton: "ログイン",
        registerButton: "アカウント登録",
        magicLinkOption: "メールリンクでログイン",
        usernameDisplayOnly: "表示専用です。ログインにはメールを使用します",
        emailRequired: "メールアドレスを入力してください",
        invalidEmail: "有効なメールアドレスを入力してください",
        credentialsRequired: "メールとパスワードを入力してください",
        passwordRequired: "パスワードを入力してください",
        passwordTooShort: "パスワードは6文字以上で入力してください",
        passwordMismatch: "パスワードが一致しません",
        usernameRequired: "ニックネームを入力してください",
        loginSuccess: "ログイン成功",
        loginFailed: "ログイン失敗",
        registerSuccess: "登録成功",
        registerFailed: "登録失敗",
        confirmEmailNotice: "登録成功！メールを確認して登録を完了してください",
        invalidCredentials: "メールまたはパスワードが正しくありません",
        emailNotConfirmed: "メールが未確認です。メールボックスを確認してください",
        userAlreadyExists: "このメールは既に登録されています",
        tooManyRequests: "リクエストが多すぎます。しばらくしてから再試行してください",
        signOut: "ログアウト",
        sendMagicLink: "ログインリンクを送信",
      }
    },

    "zh-CN": {
      home: {
        eyebrow: "学习工作台",
        title: "今天也继续加油",
        currentContext: "当前学习",
        startLearning: "开始学习",
        continueLearning: "继续学习",
        reviewCourse: "从头复习",
        courseDone: "已完成",
        goContinue: "继续",
        emptyHint: "还没有学习记录。建议从 SQL 第 1 课开始。",
        openDashboard: "学习统计",
        progressOverview: "课程进度",
        quickActions: "快捷操作",
        openGlossary: "术语词典"
      },
      lessonNav: {
        title: "课时导航",
        prev: "上一课",
        next: "下一课",
        completed: "本课已完成",
        notCompleted: "未完成 · 完成课题后自动标记"
      },
      common: {
        save: "保存",
        cancel: "取消",
        confirm: "确认",
        delete: "删除",
        reset: "重置",
        resetProgress: "重置进度",
        loading: "加载中...",
        themeToggle: "切换浅色/深色模式",
        close: "关闭",
        open: "打开",
        search: "搜索",
        random: "随机",
        restart: "重新开始",
        copy: "复制",
        maximize: "最大化",
        minimize: "最小化",
        challenging: "挑战中",
        lessonIndex: "第{id}节",
        changeQuestion: "换一题",
        stop: "结束",
        subtitle: "配套备考平台"
      },
      nav: {
        sql: "SQL 学习",
        java: "Java 学习",
        python: "Python 学习",
        itpass: "IT Passport 备考",
        sg: "SG 备考",
        typing: "日本语打字",
        codingTyping: "编程打字",
        sqlChallenge: "综合随机挑战"
      },
      moduleSwitch: {
        title: "选择学习版块",
        close: "关闭",
        triggerLabel: "打开版块选择"
      },
      moduleDesc: {
        sql: "数据库基础、查询、练习与沙盒",
        sg: "情報セキュリティマネジメント試験対策",
        itpass: "IT Passport 基础与真题练习",
        typing: "日本语输入与打字练习",
        codingTyping: "代码输入与语法练习",
        java: "Java 基础、语法与练习",
        python: "Python 基础、语法与练习"
      },
      codingTyping: {
        title: "编程打字练习",
        subtitle: "通过打字练习掌握编程语法。选择语言和类别开始练习。",
        language: "选择语言",
        category: "类别过滤",
        allCategories: "全部",
        difficulty: "难度",
        code: "目标代码",
        input: "输入区",
        inputPlaceholder: "在此输入目标代码...",
        focus: "练习要点",
        hint: "提示",
        speed: "CPM",
        accuracy: "正确率",
        progress: "进度",
        time: "用时",
        errors: "错误",
        complete: "完成！",
        result: "结果",
        totalTime: "用时",
        errorChars: "常错字符",
        recentHistory: "最近记录",
        noHistory: "暂无记录",
        next: "下一题 (Alt+N)",
        random: "随机 (Alt+S)",
        restart: "重来 (Alt+R)",
        continueRandom: "继续随机练习",
        python: "Python",
        java: "Java",
        sql: "SQL",
        symbols: "符号",
        normalizeFullWidth: "全角→半角",
        lenientIndent: "缩进宽容",
        sendToSandbox: "发送到沙盒",        runInSandbox: "在沙盒中运行",        sandboxUnavailable: "沙盒不可用",        sentToSandbox: "已发送到沙盒",        openSandbox: "打开沙盒",        shortcutHelp: "快捷键: Alt+N 下一题 / Alt+R 重来 / Alt+S 随机",
        summaryLine: "CPM: {cpm} | 正确率: {acc}% | 错误: {err} | 用时: {time}",
        examInsight: "考试理解卡片",
        examRelevance: "考试相关性",
        examTargets: "考试目标",
        relatedTerms: "相关术语",
        meaning: "含义",
        memoryHook: "记忆法",
        examPoint: "考点",
        commonMistake: "易错点",
        high: "高",
        medium: "中",
        low: "低",
        backToList: "返回练习列表",
        exerciseList: "练习列表"
      },
      subnav: {
        lessons: "教科书与演练沙盒",
        practicalExam: "实操模拟考试",
        study: "教科书章节学习",
        pastExams: "过去问道场 & 模拟考试",
        javaIntro: "入门编 (基础篇) — 第1〜8章",
        javaPractice: "实践编 (进阶篇) — 第1〜11章"
      },
      sidebar: {
        progress: "学习进度",
        unitList: "课程目录"
      },
      sandbox: {
        run: "执行",
        playground: "沙盒",
        runSql: "执行 SQL",
        resetDb: "重置 DB",
        hint: "提示",
        maximize: "最大化",
        output: "执行结果",
        sqlPlaceholder: "请在此处输入 SQL 语句...",
        currentMission: "当前任务",
        randomPractice: "随机指令练习",
        databaseStructure: "数据库表结构参考",
        schoolDb: "学校DB",
        bookstoreDb: "书店DB",
        javaSandboxTitle: "Java 编程沙盒",
        pythonSandboxTitle: "Python 编程沙盒",
        pythonSandboxDesc: "Python 3.12 · 10s · 离线沙盒",
        sqlPlayground: "SQL 演练沙盒",
        chooseLessonLoadCode: "— 选择课程后载入示例代码 —",
        toolbarTemplate: "模板",
        toolbarTemplateTitle: "加载当前课程模板",
        toolbarCopy: "复制",
        toolbarCopyTitle: "复制代码",
        toolbarClear: "重置",
        toolbarClearTitle: "清空并重置",
        javaEditorPlaceholder: "请在此处输入 Java 代码...",
        pythonEditorPlaceholder: "请在此处输入 Python 代码...",
        stdinHeader: "标准输入 / Standard Input (stdin)",
        stdinPlaceholder: "程序接收 of 输入内容写在这里...",
        runCode: "运行",
        readyStatus: "已就绪 / Ready",
        outputPlaceholder: "（点击运行按钮，结果将显示在这里）"
      },
      exam: {
        allExamTitle: "综合随机模拟考试",
        yearExamTitle: "{year} 公开问题 模拟考试",
        verify: "验证并保存答案",
        submit: "提交试卷并交卷",
        previous: "上一题",
        next: "下一题",
        exit: "中断退出",
        ruleTitle: "过去问道场 — CBT 模拟考试系统",
        ruleDesc: "基于近年 IT 考试真题，模拟真实计算机机考 (CBT) 环境。",
        ruleDetailTitle: "考试规则 (試験規則):",
        ruleDetail1: "题目均为实操编程题/数据库查询题，给日文文字任务或预期输出结果。",
        ruleDetail2: "支持在右侧的模拟沙盒编辑器中即时编写、运行、和调试代码。",
        ruleDetail3: "完成代码后，点击“验证并保存答案 (判定)”进行测试用例校验。",
        ruleDetail4: "测试成功（Passed ✓）计入得分，可以反复修改直到判定通过。",
        ruleDetail5: "请在倒计时结束前点击“结束考试并交卷”，超时系统将自动收卷。",
        selectYear: "选择考试年度/真题:",
        questionCount: "模拟考试题量:",
        launchBtn: "开始机考挑战 (試験開始)",
        status: "测试状态: ",
        notStarted: "未作答 (未着手)",
        cbtSimulator: "CBT 模拟系统",
        remainingTime: "剩余时间: ",
        exitCbt: "中断退出",
        submitCbt: "提交试卷",
        questionNumber: "第 {current} 题 / 問 {current}",
        flagReview: "标记此题复查",
        metaExpand: "题目信息 <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "收起 <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "出典：",
        metaYear: "年度",
        metaQNum: "题号",
        metaCategory: "分类",
        metaSubcategory: "子分类",
        metaTopic: "主题",
        ruleTitleCoding: "实操编程模拟考试 (実技模試)",
        ruleDescCoding: "模拟日本 IT 专门学校的编程考试环境。在限定时间内完成给定的编码或数据库任务。",
        launchBtnCoding: "开始实操考试 (試験開始)",
        cbtSimulatorCoding: "实操机考",
        expectedOutput: "期待される出力結果 (期望执行输出)",
        resultsTitle: "实操考试成绩单 (評価結果)",
        resultsTitleCbt: "模拟考试测评成绩单",
        backToHome: "返回主页",
        backToDojo: "返回道场",
        passLabel: "合格判定",
        passStatus: "合格",
        failStatus: "不合格",
        correctCountText: "{count} / {total} 题通过",
        totalScoreText: "{score} / 1000 点",
        correctRate: "得点率 (通过率)",
        timeSpent: "考试用时 (所要时间):",
        detailsTitle: "各题验证测试详情",
        reviewTitle: "真题回顾与错题分析",
        reviewTip: "点击任意条目展开查看详情。",
        reviewCollapse: "收起解析",
        reviewExplanation: "解析",
        resultCorrect: "正确",
        resultIncorrect: "错误",
        tableQNo: "题号",
        tableTitle: "考核知识点 (标题)",
        tableField: "领域",
        tableDifficulty: "难度",
        tableResult: "验证结果",
        tableYourAnswer: "您的答案",
        tableCorrectAnswer: "正确答案",
        tableAction: "操作",
        tableActionDetail: "查看",
        personalizeTitle: "机考个性化选项:",
        optShuffle: "随机打乱选择肢顺序 (ア〜エ)",
        optNocalc: "过滤计算题 (不显示含有公式计算的题目)",
        all: "随机混合真题 (推荐 - 综合模考)",
        r8: "令和8年度 (R8) 公开问题",
        r7: "令和7年度 (R7) 公开问题",
        r6: "令和6年度 (R6) 公开问题",
        r5: "令和5年度 (R5) 公开问题",
        r4: "令和4年度 (R4) 公开问题",
        r3: "令和3年度 (R3) 公开问题",
        cbtQ100: "100 题 (正式机考标准 - 限时120分钟)",
        cbtQ50: "50 题 (精简模考 - 限时60分钟)",
        cbtQ30: "30 题 (快速刷题 - 限时36分钟)",
        cbtQ10: "10 题 (微型小测 - 限时12分钟)",
        q5: "5 题 (快速测试 - 15 分钟)",
        q10: "10 题 (标准实操 - 30 分钟)",
        q15: "15 题 (全面挑战 - 45 分钟)",
        gridTitle: "答题卡看板",
        unanswered: "未答",
        answered: "已答",
        flagged: "标记",
        totalScoreLabel: "综合得点 (总分)",
        domainStrat: "ストラテジ系 (策略领域)",
        domainMan: "マネジメント系 (管理领域)",
        domainTech: "テクノロジ系 (技术领域)",
        domainSubb: "科目B (场景应用)"
      },
      toast: {
        alreadyFirst: "已经是第一题了。",
        alreadyLast: "已经是最后一题了。",
        saved: "已保存。",
        verifySuccess: "判定通过！已保存答案。",
        dbReset: "模拟数据库已重置回初始数据。",
        examStarted: "CBT 模拟考试已生成，考试开始！",
        lessonQuizPassed: "🏆 恭喜！{title} 课后小测已全部通过，本节标记为“已完成”！",
        dbSwitched: "🗄️ 已切换到《{dbName}》数据库。您现在可以查询 {tables} 等表。",
        sqlHint: "💡 提示：参考以下 SQL 结构：<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ 还没有已完成的课节！请先学习并完成至少一个课节。",
        challengeStarted: "🔥 综合挑战开始！共 {count} 道题来自已完成的 {lessonCount} 个课节。",
        challengeEnded: "✅ 综合挑战结束。",
        challengeQuestionCorrect: "🎉 正确！来自第 {sourceId} 节的题目已通过！",
        challengeSuccess: "综合挑战达成！",
        examEmpty: "⚠️ 符合条件的真题为空，请更改出题选项！",
        examTimeout: "⏰ 答题时间已到，系统自动交卷！",
        examPassed: "🎉 恭喜！您已成功通过本次模拟测试！",
        examFailed: "💪 未通过合格标准，请继续努力！",
        examDbNotFound: "⚠️ 没有找到该科目的考题数据库！",
        practicalExamStarted: "📝 实操模拟考试开始！请根据要求完成代码编写。",
        sqlSyntaxError: "❌ SQL 语法错误，判定未通过！",
        sqlOutputMismatch: "❌ 运行结果与期望数据不一致，请检查排序、过滤或选择的列数！",
        compileError: "❌ 编译错误，判定失败！",
        runtimeError: "❌ 运行错误，判定失败！",
        outputMismatch: "❌ 输出内容不一致，判定失败！",
        practicalExamSubmitted: "🎉 实操考试提交成功！已生成详细成绩单与评定。"
      },
      dialog: {
        resetDbConfirm: "模拟数据库将被重置回初始状态，继续吗？",
        exitExamConfirm: "确定要中断退出当前考试吗？进度将不被保存。",
        switchExamConfirm: "当前有正在运行的考试。切换科目会中断当前考试，确定要切换吗？",
        clearCodeConfirm: "确定要清空代码吗？",
        resetProgressConfirm: "确定要重置所有 {subject} 学习进度吗？",
        submitExamConfirm: "确定要交卷吗？",
        submitExamUnansweredConfirm: "还有 {count} 道题未作答，确定现在交卷吗？",
        submitCodingExamUnansweredConfirm: "还有 {count} 道题未作答或未判定成功，确定现在交卷吗？"
      },
      tools: {
        quickStart: "快速开始",
        quickStartTitle: "学习流程",
        quickStartDesc: "选择科目 → 学习课程 → 做练习 → 查看统计",
        tools: "工具",
        openMenu: "打开工具菜单",
        closeMenu: "关闭工具菜单",
        learningTools: "学习工具",
        glossary: "术语表",
        glossaryDesc: "查询 IT / SQL / 编程术语",
        settings: "设置",
        settingsDesc: "调整学习工具偏好",
        dangerous: "危险操作",
        resetProgress: "重置进度",
        resetProgressDesc: "清空当前学习进度，需要确认",
        confirmResetTitle: "确认重置进度？",
        confirmResetBody: "此操作会清空当前学习进度，是否继续？",
        cancel: "取消",
        confirmReset: "确认重置",
        examTools: "考试工具",
        examHistory: "考试历史",
        examHistoryDesc: "查看最近的模拟考试记录",
        dashboard: "学习统计",
        dashboardDesc: "查看学习进度与数据概览",
        exportData: "导出学习数据",
        exportDataDesc: "下载本地学习数据备份 (JSON)",
        continueLearning: "继续学习",
        continueLearningDesc: "从上次进度继续",
        exportSuccess: "导出成功",
        exportPartialSuccess: "已导出，已跳过部分损坏的数据",
        exportFailed: "导出失败",
        exportScope: "不含密码和密钥 — 仅学习数据",
        importPreview: "预览导入备份",
        importPreviewDesc: "只读预览，不会修改当前数据",
        importSelectFile: "选择备份文件",
        importParseFailed: "文件解析失败",
        importSensitiveBlocked: "检测到敏感字段，已阻止",
        importImportable: "可导入数据",
        importNew: "新增",
        importConflict: "冲突",
        importSkipped: "跳过",
        importIncompatibleVersion: "不兼容的备份版本",
        importFileTooLarge: "文件过大",
        importClosePreview: "关闭预览",
        importSummary: "导入摘要",
        importNoData: "无可导入数据",
        importSection: "数据类别",
        importSectionLessons: "已完成课程",
        importSectionQuiz: "测验结果",
        importSectionWrongBook: "错题本",
        importSectionBookmarks: "收藏",
        importSectionTyping: "打字记录",
        importSectionExam: "考试记录",
        importSectionGoals: "学习目标",
        importSectionSettings: "语言与主题",
        importSectionSync: "同步状态",
        importSyncBlocked: "不可导入 — 同步状态由本设备管理",
        updater: "检查更新",
        updaterDesc: "查看和更新程序版本",
        updaterCurrentVersion: "当前版本",
        updaterLatestVersion: "最新版本",
        updaterCheckBtn: "检查更新",
        updaterDownloadBtn: "下载更新",
        updaterApplyBtn: "立即重启并更新",
        updaterChecking: "正在检查更新...",
        updaterDownloading: "正在下载更新...",
        updaterIdle: "就绪",
        updaterUpToDate: "已是最新版本",
        updaterUpdateAvailable: "发现新版本 {version}",
        updaterDownloadComplete: "下载完成，准备更新",
        updaterAutoDownload: "自动下载更新",
        updaterCheckFailed: "检查更新失败",
        updaterDownloadFailed: "下载失败",
        updaterApplyFailed: "更新失败",
        updaterRolledBack: "已自动回滚至先前版本",
        updaterNoNetwork: "无法连接到更新服务器",
        updaterChangelog: "更新日志",
        updaterReleaseDate: "发布日期",
        updaterLastCheck: "上次检查",
        updaterAppUpdate: "软件更新",
        updaterSecureTitle: "更新检查暂时不可用",
        updaterSecureDesc: "此安装缺少安全验证组件，因此更新已安全停止。学习功能不受影响。你可以继续学习，或使用官方 Portable 包重新安装后再检查更新。",
        updaterSecureNote: "不影响任何学习功能。",
        updaterDiag: "诊断信息",
        updaterRetry: "重新检查",
        updaterBackToLearning: "返回学习"
      },
      dashboard: {
        dashboard: "学习统计",
        overview: "总览",
        completedLessons: "已完成课程",
        wrongBook: "错题",
        bookmarks: "收藏",
        typingPractice: "打字练习",
        examHistory: "模拟考试",
        quizCompleted: "测验完成",
        learningProgress: "学习进度",
        wrongBookAndBookmarks: "错题与收藏",
        wrongBookCount: "错题数",
        bookmarkCount: "收藏数",
        latestAdded: "最新添加",
        typingArticles: "打字文章",
        japaneseFavorites: "日语收藏",
        quizAndExam: "测验与考试",
        examCount: "考试次数",
        avgAccuracy: "平均正确率",
        latestExam: "最近考试",
        japaneseTyping: "日语打字",
        codingTyping: "编程打字",
        bestCpm: "最高 CPM",
        syncStatus: "同步状态",
        loginStatus: "登录状态",
        loggedIn: "已登录",
        notLoggedIn: "未登录",
        lastSync: "上次同步",
        lastSyncResult: "同步结果",
        syncSuccess: "同步成功",
        syncFailed: "同步失败",
        collectedAt: "数据采集时间",
        refreshStats: "刷新统计",
        noData: "暂无数据",
        loadError: "加载统计数据失败",
        infoSec: "信息安全",
        weeklyTrend: "周学习趋势",
        last7Days: "最近7天",
        noTrendData: "暂无趋势数据",
        learningGoals: "学习目标",
        goalActivities: "每周活动目标",
        goalTyping: "每周打字目标",
        goalExams: "每周考试目标",
        saveGoals: "保存目标",
        resetGoals: "重置目标",
        goalsSaved: "目标已保存",
        goalsReset: "目标已重置",
        noActivity: "无活动",
        examsShort: "考试",
        jpTypingShort: "日语",
        ctTypingShort: "编程",
        wbShort: "错题",
        thisWeek: "本周",
        today: "今日",
        dataIncompleteHint: "数据可能不完整，建议先同步。",
        syncAndRefresh: "同步并刷新",
        syncing: "正在同步…",
        syncComplete: "同步完成",
        syncFailedHint: "同步失败",
        loginToSyncHint: "登录后可同步多设备数据"
      },
      examHistory: {
        title: "最近考试历史",
        subtitle: "查看已保存的模拟考试结果",
        close: "关闭",
        filterAll: "全部",
        empty: "还没有考试历史",
        emptyHint: "完成一次模拟考试后，这里会显示记录",
        questions: "题量",
        correct: "正确",
        accuracy: "正确率",
        score: "分数",
        passed: "合格",
        failed: "未合格",
        time: "用时",
        viewDetails: "查看详情",
        hideDetails: "收起详情",
        examConfig: "考试配置",
        fieldScores: "分类子分",
        questionResults: "逐题结果",
        yearFilter: "年度筛选",
        submittedAt: "提交时间",
        yourAnswer: "用户答案",
        correctAnswer: "正确答案",
        unanswered: "未答",
        questionNotFound: "题目数据未找到"
      },
      wrongBook: {
        title: "错题本",
        subtitle: "复习答错的题目",
        emptyTitle: "还没有错题",
        emptyDesc: "答错的题目会自动收集到这里",
        all: "全部",
        unmastered: "未掌握",
        mastered: "已掌握",
        markMastered: "标记已掌握",
        unmarkMastered: "取消已掌握",
        remove: "移除",
        wrongCount: "答错次数",
        lastWrongAt: "最近答错",
        yourAnswer: "我的错误答案",
        correctAnswer: "正确答案",
        explanation: "解析",
        source: "来源",
        confirmRemove: "确定从错题本移除这道题吗？",
        viewDetails: "展开详情",
        hideDetails: "收起详情",
        choices: "选项",
        question: "题目",
        unknownAnswer: "未记录"
      },
      message: {
        inputSqlRequired: "请输入 SQL 语句."
      },
      settings: {
        language: "语言"
      },
      lesson: {
        locateBookPage: "定位原书页",
        explanationJa: "解説 (日本語)",
        explanationZh: "讲解 (中文)",
        compareMode: "左右对照 (日/中)",
        japaneseOnly: "日本語のみ",
        chineseOnly: "中文简体",
        analogy: "形象比喻",
        sqlExample: "SQL构文例 (SQL示例)",
        javaExample: "Java 代码例 (示例代码)",
        pythonExample: "Python 代码例 (示例代码)",
        glossary: "重要用語・公式 (核心术语 & 公式)",
        copyToEditor: "复制到编辑器",
        showPdf: "显示原书 PDF",
        translationZh: "中文翻译对照",
        checklist: "要点速记",
        calculator: "计算器工具箱",
        calculatorSelect: "选择工具:"
      },
      flashcard: {
        title: "词汇闪卡",
        flip: "翻转",
        clickToFlip: "点击闪卡即可翻转查看中日对照翻译及释义：",
        clickToFlipHint: "点击此处翻转",
        localizedDesc: "{title} 的核心术语卡片。"
      },
      glossary: {
        title: "术语表 / 用語集",
        open: "术语表 / 用語集",
        close: "关闭 / 閉じる",
        searchPlaceholder: "搜索术语... / 用語を検索...",
        allCategories: "全部 / すべて",
        filterDatabase: "数据库 / データベース",
        filterSecurity: "安全 / セキュリティ",
        filterNetwork: "网络 / ネットワーク",
        filterProgramming: "编程 / プログラミング",
        filterSystem: "系统 / システム",
        category: "分类 / カテゴリ",
        japanese: "日语 / 日本語",
        english: "英语 / 英語",
        currentLanguage: "当前语言",
        aliases: "别名 / 別名",
        relatedTerms: "相关术语 / 関連用語",
        example: "代码示例 / コード例",
        source: "来源 / 出典",
        needsReview: "⚠ 翻译待校对 / 翻訳確認中",
        noResults: "没有找到相关术语 / 該当する用語はありません",
        keepEnglishNote: "保留英文技术术语 / 英語の技術用語を保持",
        resultCount: "共 {count} 个术语 / {count} 件の用語"
      },
      quiz: {
        title: "理解度测试 (章节小测)",
        questionCount: "问题 {current} / 共 {total} 题",
        previous: "上一题",
        next: "下一题",
        submitAnswer: "提交回答",
        empty: "本章暂无课后练习小测。",
        unavailable: "{title} 的小测暂未完成当前显示语言本地化。",
        thisLesson: "本章"
      },
      typing: {
        library: "打字文库",
        libraryDesc: "从 N3 到 N1 实战日本語输入，支持生活、职场、长文练习，自由调整节奏。",
        search: "文库搜索",
        searchPlaceholder: "标题、内容或标签",
        articleList: "文章列表",
        randomArticle: "随机一篇",
        restart: "重新开始",
        timeLimit: "限时",
        imeMode: "IME 实战模式",
        speed: "速度",
        accuracy: "正确率",
        progress: "有效进度",
        edits: "修改次数",
        time: "时间",
        recentScores: "最近成绩",
        personalReview: "个人复练",
        reviewDesc: "正确率低于 98% 的文章会自动加入错句复练；满分完成后自动移除。",
        reviewCount: "{count} 篇待复练",
        emptyReview: "无需要复练的文章",
        imePlaceholder: "日语输入法をオンにして、这里から输入してください。",
        typingHelp: "系统日语 IME 输入。全角与半角会统一比较；删除和改写会计入修改次数。",
        resultTitle: "练习完成",
        continueRandom: "继续随机练习",
        favoriteBtn: "收藏",
        favoriteTitle: "收藏当前文章",
        noLimit: "不限时",
        t1min: "1分钟",
        t3min: "3分钟",
        t5min: "5分钟",
        t10min: "10分钟"
      },
      console: {
        output: "执行结果 / 実行結果",
        rows: "{count}行",
        emptyResult: "查询结果表格将在此处显示。",
        emptyResultSub: "(执行 SQL 后，查询结果表格将在此处显示)",
        maximize: "最大化",
        minimize: "最小化"
      },
      auth: {
        localMode: "本地模式",
        notLoggedIn: "未登录",
        account: "账号",
        sync: "同步",
        syncInDev: "同步功能开发中",
        continueLocal: "继续本地使用",
        mockSignIn: "模拟登录状态",
        mockSignOut: "退出模拟登录",
        exportSnapshot: "导出本地快照",
        deviceId: "设备 ID",
        pendingSync: "待同步",
        lastSync: "上次同步",
        noUpload: "不会上传数据",
        close: "关闭",
        signedIn: "已登录",
        signedOut: "未登录",
        status: "状态",
        anonymous: "匿名",
        loginTitle: "登录",
        registerTitle: "注册账号",
        displayName: "昵称",
        email: "邮箱",
        password: "密码",
        confirmPassword: "确认密码",
        loginButton: "登录",
        registerButton: "注册账号",
        magicLinkOption: "使用邮件登录链接",
        usernameDisplayOnly: "仅用于显示，登录使用邮箱",
        emailRequired: "请输入邮箱",
        invalidEmail: "请输入有效的邮箱地址",
        credentialsRequired: "请输入邮箱和密码",
        passwordRequired: "请输入密码",
        passwordTooShort: "密码至少 6 位",
        passwordMismatch: "两次密码不一致",
        usernameRequired: "请输入昵称",
        loginSuccess: "登录成功",
        loginFailed: "登录失败",
        registerSuccess: "注册成功",
        registerFailed: "注册失败",
        confirmEmailNotice: "注册成功！请前往邮箱确认注册，然后登录。",
        invalidCredentials: "邮箱或密码不正确",
        emailNotConfirmed: "邮箱尚未确认，请检查邮箱",
        userAlreadyExists: "该邮箱已注册",
        tooManyRequests: "请求过于频繁，请稍后再试",
        signOut: "登出",
        sendMagicLink: "发送登录链接",
      }
    },

    "en-US": {
      home: {
        eyebrow: "Learning Workspace",
        title: "Keep the momentum going",
        currentContext: "Current course",
        startLearning: "Start learning",
        continueLearning: "Continue learning",
        reviewCourse: "Review from start",
        courseDone: "Done",
        goContinue: "Continue",
        emptyHint: "No learning history yet. Starting with SQL Lesson 1 is recommended.",
        openDashboard: "Statistics",
        progressOverview: "Course progress",
        quickActions: "Quick actions",
        openGlossary: "Glossary"
      },
      lessonNav: {
        title: "Lesson navigation",
        prev: "Previous lesson",
        next: "Next lesson",
        completed: "Lesson completed",
        notCompleted: "Not completed · finish the task to complete"
      },
      common: {
        save: "Save",
        cancel: "Cancel",
        confirm: "Confirm",
        delete: "Delete",
        reset: "Reset",
        resetProgress: "Reset Progress",
        loading: "Loading...",
        themeToggle: "Toggle light/dark mode",
        close: "Close",
        open: "Open",
        search: "Search",
        random: "Random",
        restart: "Restart",
        copy: "Copy",
        maximize: "Maximize",
        minimize: "Minimize",
        challenging: "Challenging",
        lessonIndex: "Lesson {id}",
        changeQuestion: "Change Question",
        stop: "End",
        subtitle: "Study Support Platform"
      },
      nav: {
        sql: "SQL",
        java: "Java",
        python: "Python",
        itpass: "IT Passport",
        sg: "SG Exam",
        typing: "Japanese Typing",
        codingTyping: "Coding Typing",
        sqlChallenge: "Random Challenge"
      },
      moduleSwitch: {
        title: "Select learning section",
        close: "Close",
        triggerLabel: "Open section selector"
      },
      moduleDesc: {
        sql: "Database basics, queries, practice, and sandbox",
        sg: "Information Security Management exam prep",
        itpass: "IT Passport basics and past questions",
        typing: "Japanese input and typing practice",
        codingTyping: "Code input and syntax practice",
        java: "Java basics, syntax, and practice",
        python: "Python basics, syntax, and practice"
      },
      codingTyping: {
        title: "Coding Typing Practice",
        subtitle: "Master programming syntax through typing practice. Select a language and category to begin.",
        language: "Language",
        category: "Category",
        allCategories: "All",
        difficulty: "Difficulty",
        code: "Target Code",
        input: "Input",
        inputPlaceholder: "Type the target code here...",
        focus: "Focus Points",
        hint: "Hint",
        speed: "CPM",
        accuracy: "Accuracy",
        progress: "Progress",
        time: "Time",
        errors: "Errors",
        complete: "Complete!",
        result: "Result",
        totalTime: "Time",
        errorChars: "Common Errors",
        recentHistory: "Recent History",
        noHistory: "No history yet",
        next: "Next (Alt+N)",
        random: "Random (Alt+S)",
        restart: "Restart (Alt+R)",
        continueRandom: "Continue Random",
        python: "Python",
        java: "Java",
        sql: "SQL",
        symbols: "Symbols",
        normalizeFullWidth: "Fullwidth→Halfwidth",
        lenientIndent: "Lenient Indent",
        sendToSandbox: "Send to Sandbox",        runInSandbox: "Run in Sandbox",        sandboxUnavailable: "Sandbox unavailable",        sentToSandbox: "Sent to sandbox",        openSandbox: "Open sandbox",        shortcutHelp: "Shortcuts: Alt+N Next / Alt+R Restart / Alt+S Random",
        summaryLine: "CPM: {cpm} | Accuracy: {acc}% | Errors: {err} | Time: {time}",
        examInsight: "Exam Insight",
        examRelevance: "Exam Relevance",
        examTargets: "Exam Targets",
        relatedTerms: "Related Terms",
        meaning: "Meaning",
        memoryHook: "Memory Hook",
        examPoint: "Exam Point",
        commonMistake: "Common Mistake",
        high: "High",
        medium: "Medium",
        low: "Low",
        backToList: "Back to Practice List",
        exerciseList: "Practice List"
      },
      subnav: {
        lessons: "Textbook & Practice Sandbox",
        practicalExam: "Practical Mock Exam",
        study: "Textbook Lessons",
        pastExams: "Past Questions & Mock Exam",
        javaIntro: "Introductory — Chapters 1-8",
        javaPractice: "Practical — Chapters 1-11"
      },
      sidebar: {
        progress: "Progress",
        unitList: "Unit List"
      },
      sandbox: {
        run: "Run",
        playground: "Sandbox",
        runSql: "Run SQL",
        resetDb: "Reset DB",
        hint: "Hint",
        maximize: "Maximize",
        output: "Output",
        sqlPlaceholder: "Please enter an SQL query here...",
        currentMission: "Current Mission",
        randomPractice: "Random Practice",
        databaseStructure: "Database Structure Reference",
        schoolDb: "School DB",
        bookstoreDb: "Bookstore DB",
        javaSandboxTitle: "Java Programming Sandbox",
        pythonSandboxTitle: "Python Programming Sandbox",
        pythonSandboxDesc: "Python 3.12 · 10s · Offline Sandbox",
        sqlPlayground: "SQL Playground",
        chooseLessonLoadCode: "— Select a lesson to load sample code —",
        toolbarTemplate: "Template",
        toolbarTemplateTitle: "Load lesson template",
        toolbarCopy: "Copy",
        toolbarCopyTitle: "Copy code",
        toolbarClear: "Clear",
        toolbarClearTitle: "Clear and reset",
        javaEditorPlaceholder: "Type Java code here...",
        pythonEditorPlaceholder: "Type Python code here...",
        stdinHeader: "Standard Input (stdin)",
        stdinPlaceholder: "Input data for the program...",
        runCode: "Run",
        readyStatus: "Ready",
        outputPlaceholder: "(Execution output will appear here)"
      },
      exam: {
        allExamTitle: "Comprehensive Random Mock Exam",
        yearExamTitle: "{year} Published Questions Mock Exam",
        verify: "Check and Save",
        submit: "Submit Exam",
        previous: "Previous",
        next: "Next",
        exit: "Exit Exam",
        ruleTitle: "Past Question Dojo — CBT Mock Exam System",
        ruleDesc: "Simulates the computer-based testing (CBT) environment using past questions.",
        ruleDetailTitle: "Exam Rules:",
        ruleDetail1: "Questions are database query tasks or coding tasks with expected output.",
        ruleDetail2: "You can write, run, and debug code instantly in the sandbox editor.",
        ruleDetail3: "Click \"Check and Save\" to verify your code against test cases.",
        ruleDetail4: "Passed tasks earn score points. You can modify code as needed.",
        ruleDetail5: "Make sure to submit before timer expires; timeout triggers auto-submission.",
        selectYear: "Select Year:",
        questionCount: "Number of Questions:",
        launchBtn: "Start Test",
        status: "Test Status: ",
        notStarted: "Not Attempted",
        cbtSimulator: "CBT Simulator",
        remainingTime: "Time Remaining: ",
        exitCbt: "Exit Exam",
        submitCbt: "Submit Exam",
        questionNumber: "Q {current}",
        flagReview: "Flag for Review",
        metaExpand: "Question info <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "Collapse <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "Source: ",
        metaYear: "Year",
        metaQNum: "Question No.",
        metaCategory: "Category",
        metaSubcategory: "Subcategory",
        metaTopic: "Topic",
        ruleTitleCoding: "Practical Programming Mock Exam",
        launchBtnCoding: "Start Exam",
        cbtSimulatorCoding: "Practical Exam",
        expectedOutput: "Expected Output",
        resultsTitle: "Evaluation Scorecard",
        resultsTitleCbt: "CBT Scorecard Report",
        backToHome: "Return Home",
        backToDojo: "Return Dojo",
        passLabel: "Judgement",
        passStatus: "Passed",
        failStatus: "Failed",
        correctCountText: "{count} / {total} Passed",
        totalScoreText: "{score} / 1000 pts",
        correctRate: "Accuracy Rate",
        timeSpent: "Time Spent:",
        detailsTitle: "Question Evaluation Details",
        reviewTitle: "Question Review & Error Analysis",
        reviewTip: "Click any entry to expand and view details.",
        reviewCollapse: "Hide explanation",
        reviewExplanation: "Explanation",
        resultCorrect: "Correct",
        resultIncorrect: "Incorrect",
        tableQNo: "No.",
        tableTitle: "Topic / Title",
        tableField: "Domain",
        tableDifficulty: "Difficulty",
        tableResult: "Result",
        tableYourAnswer: "Your Answer",
        tableCorrectAnswer: "Correct Answer",
        tableAction: "Action",
        tableActionDetail: "View",
        personalizeTitle: "Exam Custom Options:",
        optShuffle: "Shuffle choices order (ア〜エ)",
        optNocalc: "Filter calculation tasks (do not show math/formula tasks)",
        all: "Randomly mixed past questions (Recommended)",
        r8: "Reiwa 8 (R8) Past Questions",
        r7: "Reiwa 7 (R7) Past Questions",
        r6: "Reiwa 6 (R6) Past Questions",
        r5: "Reiwa 5 (R5) Past Questions",
        r4: "Reiwa 4 (R4) Past Questions",
        r3: "Reiwa 3 (R3) Past Questions",
        cbtQ100: "100 Questions (Standard CBT - 120 mins)",
        cbtQ50: "50 Questions (Shorter Exam - 60 mins)",
        cbtQ30: "30 Questions (Quick Review - 36 mins)",
        cbtQ10: "10 Questions (Mini Quiz - 12 mins)",
        q5: "5 Questions (Quick Test - 15 min)",
        q10: "10 Questions (Standard - 30 min)",
        q15: "15 Questions (Full Challenge - 45 min)",
        gridTitle: "Answer Sheet Grid",
        unanswered: "Unanswered",
        answered: "Answered",
        flagged: "Flagged",
        totalScoreLabel: "Total Score",
        domainStrat: "Strategy Domain",
        domainMan: "Management Domain",
        domainTech: "Technology Domain",
        domainSubb: "Subject B"
      },
      toast: {
        alreadyFirst: "This is the first question.",
        alreadyLast: "This is the last question.",
        saved: "Saved.",
        verifySuccess: "Passed. Your answer has been saved.",
        dbReset: "The mock database has been reset.",
        examStarted: "The CBT mock exam has started.",
        lessonQuizPassed: "🏆 Congratulations! You have passed all quizzes for {title}, and this lesson is marked as Completed!",
        dbSwitched: "🗄️ Switched to <{dbName}> database. You can now query tables: {tables}.",
        sqlHint: "💡 Hint: Refer to the following SQL syntax:<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ No completed lessons yet! Please study and complete at least one lesson first.",
        challengeStarted: "🔥 Random challenge started! Total {count} questions from {lessonCount} completed lessons.",
        challengeEnded: "✅ Random challenge ended.",
        challengeQuestionCorrect: "🎉 Correct! Passed the question from lesson {sourceId}!",
        challengeSuccess: "Challenge Completed!",
        examEmpty: "⚠️ No past questions match the criteria, please change the options!",
        examTimeout: "⏰ Time is up! The exam is automatically submitted.",
        examPassed: "🎉 Congratulations! You have successfully passed this mock test!",
        examFailed: "💪 You did not pass the standard. Keep trying!",
        examDbNotFound: "⚠️ Question database for this subject not found!",
        practicalExamStarted: "📝 Practical mock exam started! Please complete the code according to the requirements.",
        sqlSyntaxError: "❌ SQL syntax error. Verification failed!",
        sqlOutputMismatch: "❌ The query result does not match the expected output. Please check ordering, filtering, or columns!",
        compileError: "❌ Compilation error. Verification failed!",
        runtimeError: "❌ Runtime error. Verification failed!",
        outputMismatch: "❌ Output content mismatch. Verification failed!",
        practicalExamSubmitted: "🎉 Practical exam submitted successfully! Detailed scorecard generated."
      },
      dialog: {
        resetDbConfirm: "The mock database will be reset. Continue?",
        exitExamConfirm: "Exit the current exam? Your progress will not be saved.",
        switchExamConfirm: "An exam is currently running. Switching subjects will interrupt it. Continue?",
        clearCodeConfirm: "Clear the code?",
        resetProgressConfirm: "Are you sure you want to reset all {subject} learning progress?",
        submitExamConfirm: "Are you sure you want to submit the exam?",
        submitExamUnansweredConfirm: "There are still {count} unanswered questions. Are you sure you want to submit now?",
        submitCodingExamUnansweredConfirm: "There are still {count} unanswered or failed questions. Are you sure you want to submit now?"
      },
      tools: {
        quickStart: "Quick Start",
        quickStartTitle: "Learning Flow",
        quickStartDesc: "Select a subject → Study lessons → Practice → Check statistics",
        tools: "Tools",
        openMenu: "Open tools menu",
        closeMenu: "Close tools menu",
        learningTools: "Learning Tools",
        glossary: "Glossary",
        glossaryDesc: "Search IT / SQL / programming terms",
        settings: "Settings",
        settingsDesc: "Adjust learning tool preferences",
        dangerous: "Dangerous Actions",
        resetProgress: "Reset Progress",
        resetProgressDesc: "Clear current learning progress. Confirmation required",
        confirmResetTitle: "Reset progress?",
        confirmResetBody: "This will clear your current learning progress. Continue?",
        cancel: "Cancel",
        confirmReset: "Confirm reset",
        examTools: "Exam Tools",
        examHistory: "Exam History",
        examHistoryDesc: "View recent mock exam records",
        dashboard: "Learning Stats",
        dashboardDesc: "View learning progress and data overview",
        exportData: "Export Learning Data",
        exportDataDesc: "Download local learning data backup (JSON)",
        continueLearning: "Continue Learning",
        continueLearningDesc: "Continue from last progress",
        exportSuccess: "Export successful",
        exportPartialSuccess: "Exported with some damaged data skipped",
        exportFailed: "Export failed",
        exportScope: "No passwords or keys — learning data only",
        importPreview: "Preview Backup Import",
        importPreviewDesc: "Read-only preview — no data will be modified",
        importSelectFile: "Select backup file",
        importParseFailed: "File parse failed",
        importSensitiveBlocked: "Sensitive fields detected, blocked",
        importImportable: "Importable data",
        importNew: "New",
        importConflict: "Conflict",
        importSkipped: "Skipped",
        importIncompatibleVersion: "Incompatible backup version",
        importFileTooLarge: "File too large",
        importClosePreview: "Close preview",
        importSummary: "Import summary",
        importNoData: "No importable data",
        importSection: "Section",
        importSectionLessons: "Completed Lessons",
        importSectionQuiz: "Quiz Results",
        importSectionWrongBook: "Wrong Book",
        importSectionBookmarks: "Bookmarks",
        importSectionTyping: "Typing History",
        importSectionExam: "Exam History",
        importSectionGoals: "Learning Goals",
        importSectionSettings: "Language & Theme",
        importSectionSync: "Sync Status",
        importSyncBlocked: "Not imported — sync state is device-managed"
      },
      dashboard: {
        dashboard: "Learning Stats",
        overview: "Overview",
        completedLessons: "Completed Lessons",
        wrongBook: "Wrong Answers",
        bookmarks: "Bookmarks",
        typingPractice: "Typing Practice",
        examHistory: "Mock Exams",
        quizCompleted: "Quizzes Done",
        learningProgress: "Learning Progress",
        wrongBookAndBookmarks: "Wrong Answers & Bookmarks",
        wrongBookCount: "Wrong Answer Count",
        bookmarkCount: "Bookmark Count",
        latestAdded: "Latest Added",
        typingArticles: "Typing Articles",
        japaneseFavorites: "Japanese Favorites",
        quizAndExam: "Quizzes & Exams",
        examCount: "Exam Count",
        avgAccuracy: "Average Accuracy",
        latestExam: "Latest Exam",
        japaneseTyping: "Japanese Typing",
        codingTyping: "Coding Typing",
        bestCpm: "Best CPM",
        syncStatus: "Sync Status",
        loginStatus: "Login Status",
        loggedIn: "Logged In",
        notLoggedIn: "Not Logged In",
        lastSync: "Last Sync",
        lastSyncResult: "Sync Result",
        syncSuccess: "Sync Success",
        syncFailed: "Sync Failed",
        collectedAt: "Collected At",
        refreshStats: "Refresh Stats",
        noData: "No Data",
        loadError: "Failed to load statistics",
        infoSec: "Info Security",
        weeklyTrend: "Weekly Trend",
        last7Days: "Last 7 Days",
        noTrendData: "No trend data",
        learningGoals: "Learning Goals",
        goalActivities: "Weekly Activities",
        goalTyping: "Weekly Typing",
        goalExams: "Weekly Exams",
        saveGoals: "Save Goals",
        resetGoals: "Reset Goals",
        goalsSaved: "Goals saved",
        goalsReset: "Goals reset",
        noActivity: "No activity",
        examsShort: "Exams",
        jpTypingShort: "JP",
        ctTypingShort: "Code",
        wbShort: "Wrong",
        thisWeek: "This week",
        today: "Today",
        dataIncompleteHint: "Your data may be incomplete. Sync first to update this device.",
        syncAndRefresh: "Sync & Refresh",
        syncing: "Syncing\u2026",
        syncComplete: "Sync complete",
        syncFailedHint: "Sync failed",
        loginToSyncHint: "Login to sync data across devices"
      },
      examHistory: {
        title: "Recent Exam History",
        subtitle: "View saved mock exam results",
        close: "Close",
        filterAll: "All",
        empty: "No exam history yet",
        emptyHint: "Complete a mock exam and records will appear here",
        questions: "Questions",
        correct: "Correct",
        accuracy: "Accuracy",
        score: "Score",
        passed: "Passed",
        failed: "Failed",
        time: "Time spent",
        viewDetails: "View details",
        hideDetails: "Hide details",
        examConfig: "Exam config",
        fieldScores: "Field scores",
        questionResults: "Question results",
        yearFilter: "Year filter",
        submittedAt: "Submitted at",
        yourAnswer: "Your answer",
        correctAnswer: "Correct answer",
        unanswered: "Unanswered",
        questionNotFound: "Question data not found"
      },
      wrongBook: {
        title: "Wrong Book",
        subtitle: "Review questions you answered incorrectly",
        emptyTitle: "No wrong answers yet",
        emptyDesc: "Incorrect answers will be collected here automatically",
        all: "All",
        unmastered: "Unmastered",
        mastered: "Mastered",
        markMastered: "Mark mastered",
        unmarkMastered: "Mark unmastered",
        remove: "Remove",
        wrongCount: "Wrong count",
        lastWrongAt: "Last wrong",
        yourAnswer: "Your wrong answer",
        correctAnswer: "Correct answer",
        explanation: "Explanation",
        source: "Source",
        confirmRemove: "Remove this question from the wrong book?",
        viewDetails: "View details",
        hideDetails: "Hide details",
        choices: "Choices",
        question: "Question",
        unknownAnswer: "Not recorded"
      },
      message: {
        inputSqlRequired: "Please enter an SQL query."
      },
      settings: {
        language: "Language"
      },
      lesson: {
        locateBookPage: "Locate Book Page",
        explanationJa: "Explanation (Japanese)",
        explanationZh: "Explanation (Chinese)",
        compareMode: "Bilingual (Ja/Zh)",
        japaneseOnly: "Japanese Only",
        chineseOnly: "Chinese Only",
        analogy: "Real-world Analogy",
        sqlExample: "SQL Example Syntax",
        javaExample: "Java Code Example",
        pythonExample: "Python Code Example",
        glossary: "Key Terms & Formulas",
        copyToEditor: "Copy to Sandbox",
        showPdf: "Show Textbook PDF",
        translationZh: "Chinese Translation",
        checklist: "Checklist",
        calculator: "Calculator Tools",
        calculatorSelect: "Select tool:"
      },
      flashcard: {
        title: "Vocabulary Flashcard",
        flip: "Flip Card",
        clickToFlip: "Click the card to view translation and definitions:",
        clickToFlipHint: "Click to flip",
        localizedDesc: "Localized term card for {title}."
      },
      glossary: {
        title: "IT Glossary",
        open: "Glossary",
        close: "Close",
        searchPlaceholder: "Search terms...",
        allCategories: "All",
        filterDatabase: "Database",
        filterSecurity: "Security",
        filterNetwork: "Network",
        filterProgramming: "Programming",
        filterSystem: "System",
        category: "Category",
        japanese: "Japanese",
        english: "English",
        currentLanguage: "Current Language",
        aliases: "Aliases",
        relatedTerms: "Related Terms",
        example: "Example",
        source: "Source",
        needsReview: "⚠ Translation pending review",
        noResults: "No terms found",
        keepEnglishNote: "English technical terms are preserved",
        resultCount: "{count} terms"
      },
      quiz: {
        title: "Section Quiz",
        questionCount: "Question {current} / {total}",
        previous: "Previous",
        next: "Next",
        submitAnswer: "Submit Answer",
        empty: "No quiz is available for this lesson.",
        unavailable: "The quiz for {title} is not localized for this display language yet.",
        thisLesson: "this lesson"
      },
      typing: {
        library: "Typing Library",
        libraryDesc: "Practice practical Japanese input from N3 to N1 at your own pace.",
        search: "Search library",
        searchPlaceholder: "Title, content or tag",
        articleList: "Articles",
        randomArticle: "Random",
        restart: "Restart",
        timeLimit: "Time Limit",
        imeMode: "IME Mode",
        speed: "Speed",
        accuracy: "Accuracy",
        progress: "Progress",
        edits: "Corrections",
        time: "Time",
        recentScores: "Recent Scores",
        personalReview: "Review List",
        reviewDesc: "Articles below 98% accuracy will be added here for practice.",
        reviewCount: "{count} to review",
        emptyReview: "No articles to review",
        imePlaceholder: "Turn on Japanese IME and start typing...",
        typingHelp: "System Japanese IME input. Full-width and half-width unified; edits count.",
        resultTitle: "Typing Completed",
        continueRandom: "Continue Random Practice",
        favoriteBtn: "Favorite",
        favoriteTitle: "Add to favorites",
        noLimit: "No limit",
        t1min: "1 Minute",
        t3min: "3 Minutes",
        t5min: "5 Minutes",
        t10min: "10 Minutes"
      },
      console: {
        output: "Execution Output",
        rows: "{count} rows",
        emptyResult: "The query result table will be displayed here.",
        emptyResultSub: "(Execute SQL to see results)",
        maximize: "Maximize",
        minimize: "Minimize"
      },
      auth: {
        localMode: "Local Mode",
        notLoggedIn: "Not Logged In",
        account: "Account",
        sync: "Sync",
        syncInDev: "Sync feature is under development",
        continueLocal: "Continue Locally",
        mockSignIn: "Simulate Sign-In",
        mockSignOut: "Exit Simulation",
        exportSnapshot: "Export Local Snapshot",
        deviceId: "Device ID",
        pendingSync: "Pending Sync",
        lastSync: "Last Sync",
        noUpload: "No data is uploaded",
        close: "Close",
        signedIn: "Signed In",
        signedOut: "Signed Out",
        status: "Status",
        anonymous: "Anonymous",
        loginTitle: "Sign In",
        registerTitle: "Create Account",
        displayName: "Display Name",
        email: "Email",
        password: "Password",
        confirmPassword: "Confirm Password",
        loginButton: "Sign In",
        registerButton: "Create Account",
        magicLinkOption: "Use magic link instead",
        usernameDisplayOnly: "For display only. Sign in with email.",
        emailRequired: "Please enter your email",
        invalidEmail: "Please enter a valid email address",
        credentialsRequired: "Please enter email and password",
        passwordRequired: "Please enter a password",
        passwordTooShort: "Password must be at least 6 characters",
        passwordMismatch: "Passwords do not match",
        usernameRequired: "Please enter a display name",
        loginSuccess: "Signed in successfully",
        loginFailed: "Sign in failed",
        registerSuccess: "Account created successfully",
        registerFailed: "Registration failed",
        confirmEmailNotice: "Account created! Please check your email to confirm, then sign in.",
        invalidCredentials: "Invalid email or password",
        emailNotConfirmed: "Email not confirmed. Please check your inbox.",
        userAlreadyExists: "This email is already registered",
        tooManyRequests: "Too many requests. Please try again later.",
        signOut: "Sign Out",
        sendMagicLink: "Send Magic Link",
      }
    },

    "my-MM": {
      common: {
        save: "သိမ်းမည်",
        cancel: "မလုပ်တော့ပါ",
        confirm: "အတည်ပြုမည်",
        delete: "ဖျက်မည်",
        reset: "ပြန်လည်စတင်မည်",
        resetProgress: "တိုးတက်မှုကို ပြန်လည်စတင်မည်",
        loading: "ဖတ်နေသည်...",
        themeToggle: "အလင်း/အမှောင် မုဒ်ပြောင်းမည်",
        close: "ပိတ်မည်",
        open: "ဖွင့်မည်",
        search: "ရှာဖွေမည်",
        random: "ကျပန်း",
        restart: "ပြန်လည်စတင်မည်",
        copy: "ကူးယူမည်",
        maximize: "ချဲ့မည်",
        minimize: "ချုံ့မည်",
        challenging: "စိန်ခေါ်မှုပြုလုပ်နေဆဲ",
        lessonIndex: "သင်ခန်းစာ {id}",
        changeQuestion: "မေးခွန်းပြောင်းရန်",
        stop: "ရပ်တန့်မည်",
        subtitle: "သင်ယူမှုပံ့ပိုးရေးပလက်ဖောင်း"
      },
      nav: {
        sql: "SQL လေ့လာမှု",
        java: "Java လေ့လာမှု",
        python: "Python လေ့လာမှု",
        itpass: "IT Passport ပြင်ဆင်မှု",
        sg: "SG စာမေးပွဲ ပြင်ဆင်မှု",
        typing: "ဂျပန်စာ Typing",
        codingTyping: "ကိုဒ်ရိုကြင်း",
        sqlChallenge: "ကျပန်းစိန်ခေါ်မှု"
      },
      moduleSwitch: {
        title: "သင်ယူမှုကဏ္ဍကိုရွေးချယ်ပါ",
        close: "ပိတ်မည်",
        triggerLabel: "ကဏ္ဍရွေးချယ်မှုဖွင့်ရန်"
      },
      moduleDesc: {
        sql: "ဒေတာဘေ့စ်အခြေခံ၊ မေးခွန်းထုတ်ခြင်း၊ လေ့ကျင့်ခြင်းနှင့် ဆန်းဘောက်စ်",
        sg: "သတင်းအချက်အလက်လုံခြုံရေးစီမံခန့်ခွဲမှုစာမေးပွဲပြင်ဆင်မှု",
        itpass: "IT Passport အခြေခံနှင့် မေးခွန်းဟောင်းလေ့ကျင့်ခန်း",
        typing: "ဂျပန်စာရိုက်ခြင်းနှင့် လေ့ကျင့်ခန်း",
        codingTyping: "ကုဒ်ရိုက်ခြင်းနှင့် syntax လေ့ကျင့်ခန်း",
        java: "Java အခြေခံ၊ syntax နှင့် လေ့ကျင့်ခန်း",
        python: "Python အခြေခံ၊ syntax နှင့် လေ့ကျင့်ခန်း"
      },
      codingTyping: {
        title: "ကုဒ်ရိုက်ခြင်းလေ့ကျင့်ခန်း",
        subtitle: "ကုဒ်ရေးနည်းကို ရိုက်ခြင်းဖြင့် လေ့လာပါ။",
        language: "ဘာသာစကား", category: "အမျိုးအစား", allCategories: "အားလုံး",
        difficulty: "အခက်အခဲ", code: "ပစ်မှတ်ကုဒ်", input: "ထည့်သွင်းရန်",
        inputPlaceholder: "ကုဒ်ကို ရိုက်ပါ...", focus: "အာရုံစိုက်ရန်", hint: "အကြံပြုချက်",
        speed: "CPM", accuracy: "တိကျမှု", progress: "တိုးတက်မှု",
        time: "အချိန်", errors: "အမှား", complete: "ပြီးဆုံး!",
        result: "ရလဒ်", totalTime: "အချိန်", errorChars: "မကြာခဏမှားသောစာလုံးများ",
        recentHistory: "မှတ်တမ်း", noHistory: "မှတ်တမ်းမရှိသေး",
        next: "နောက် (Alt+N)", random: "ကျပန်း (Alt+S)", restart: "ပြန်စ (Alt+R)",
        continueRandom: "ဆက်လုပ်ရန်",
        python: "Python", java: "Java", sql: "SQL", symbols: "သင်္ကေတ",
        normalizeFullWidth: "အပြည့်→တစ်ဝက်", lenientIndent: "အင်တင်အာလျော့",
        sendToSandbox: "Sandbox သို့ပို့ရန်",        runInSandbox: "Sandbox တွင် run ရန်",        sandboxUnavailable: "Sandbox မရနိုင်ပါ",        sentToSandbox: "Sandbox သို့ပို့ပြီး",        openSandbox: "Sandbox ဖွင့်ရန်",        shortcutHelp: "Alt+N နောက် / Alt+R ပြန်စ / Alt+S ကျပန်း",
        summaryLine: "CPM: {cpm} | တိကျမှု: {acc}% | အမှား: {err} | အချိန်: {time}",
        backToList: "လေ့ကျင့်ခန်းစာရင်းသို့ပြန်ရန်"
      },
      subnav: {
        lessons: "စာအုပ်နှင့် လေ့ကျင့်ခန်း",
        practicalExam: "လက်တွေ့ Mock Exam",
        study: "သင်ခန်းစာလေ့လာမှု",
        pastExams: "မေးခွန်းဟောင်းနှင့် Mock Exam",
        javaIntro: "မိတ်ဆက်အပိုင်း — အခန်း ၁ မှ ၈",
        javaPractice: "လက်တွေ့အပိုင်း — အခန်း ၁ မှ ၁၁"
      },
      sidebar: {
        progress: "တိုးတက်မှု",
        unitList: "သင်ခန်းစာစာရင်း"
      },
      sandbox: {
        run: "Run",
        playground: "Sandbox",
        runSql: "SQL Run",
        resetDb: "DB ပြန်လည်စတင်",
        hint: "အရိပ်အမြွက်",
        maximize: "ချဲ့မည်",
        output: "ရလဒ်",
        sqlPlaceholder: "SQL query ကို ဤနေရာတွင် ထည့်ပါ...",
        currentMission: "လက်ရှိမစ်ရှင်",
        randomPractice: "ကျပန်းလေ့ကျင့်မှု",
        databaseStructure: "ဒေတာဘေ့စ်တည်ဆောက်ပုံ",
        schoolDb: "ကျောင်း DB",
        bookstoreDb: "စာအုပ်ဆိုင် DB",
        javaSandboxTitle: "Java Programming Sandbox",
        pythonSandboxTitle: "Python Programming Sandbox",
        pythonSandboxDesc: "Python 3.12 · 10s · Offline Sandbox",
        sqlPlayground: "SQL Playground",
        chooseLessonLoadCode: "— ကုဒ်နမူနာကို ယူရန် သင်ခန်းစာတစ်ခု ရွေးချယ်ပါ —",
        toolbarTemplate: "ပုံစံ",
        toolbarTemplateTitle: "သင်ခန်းစာပုံစံကို ယူရန်",
        toolbarCopy: "ကူးယူမည်",
        toolbarCopyTitle: "ကုဒ်ကူးယူရန်",
        toolbarClear: "ရှင်းလင်းမည်",
        toolbarClearTitle: "ကုဒ်အားလုံးဖျက်ရန်",
        javaEditorPlaceholder: "Java ကုဒ်ကို ဤနေရာတွင် ရေးပါ...",
        pythonEditorPlaceholder: "Python ကုဒ်ကို ဤနေရာတွင် ရေးပါ...",
        stdinHeader: "စံထည့်သွင်းမှု (stdin)",
        stdinPlaceholder: "ပရိုဂရမ်အတွက် လိုအပ်သော ဒေတာများထည့်ပါ...",
        runCode: "Run",
        readyStatus: "အဆင်သင့်ဖြစ်ပါပြီ",
        outputPlaceholder: "(ပရိုဂရမ်ရလဒ် ဤနေရာတွင် ပြသမည်)"
      },
      exam: {
        verify: "စစ်ဆေးပြီး သိမ်းမည်",
        submit: "စာမေးပွဲတင်မည်",
        previous: "ရှေ့မေးခွန်း",
        next: "နောက်မေးခွန်း",
        exit: "စာမေးပွဲမှထွက်မည်",
        ruleTitle: "Past Question Dojo — CBT စာမေးပွဲစနစ်",
        ruleDesc: "မေးခွန်းဟောင်းများကို အသုံးပြု၍ CBT စာမေးပွဲပတ်ဝန်းကျင်ကို ဖန်တီးပေးသည်။",
        ruleDetailTitle: "စာမေးပွဲစည်းကမ်းချက်များ:",
        ruleDetail1: "မေးခွန်းများသည် ဒေတာဘေ့စ် မေးမြန်းမှုများ သို့မဟုတ် ကုဒ်ရေးသားခြင်း လုပ်ငန်းများ ဖြစ်သည်။",
        ruleDetail2: "သင်သည် sandbox အယ်ဒီတာတွင် ကုဒ်များကို ချက်ချင်းရေးသား၊ run ပြီး စစ်ဆေးနိုင်သည်။",
        ruleDetail3: "ကုဒ်များကို စစ်ဆေးရန် \"စစ်ဆေးပြီး သိမ်းမည်\" ကို နှိပ်ပါ။",
        ruleDetail4: "စမ်းသပ်မှုအောင်မြင်ပါက အမှတ်ရရှိမည်ဖြစ်ပြီး လိုအပ်သလို ကုဒ်များကို ပြင်ဆင်နိုင်သည်။",
        ruleDetail5: "အချိန်မကုန်မီ တင်သွင်းပါ၊ အချိန်ကုန်ပါက အလိုအလျောက် တင်သွင်းသွားမည်ဖြစ်သည်။",
        selectYear: "ခုနှစ် ရွေးချယ်ရန်:",
        questionCount: "မေးခွန်းအရေအတွက်:",
        launchBtn: "စာမေးပွဲစတင်မည်",
        status: "စစ်ဆေးမှုအခြေအနေ: ",
        notStarted: "မလုပ်ရသေးပါ",
        cbtSimulator: "CBT Simulator",
        remainingTime: "ကျန်ရှိချိန်: ",
        exitCbt: "စာမေးပွဲမှထွက်မည်",
        submitCbt: "စာမေးပွဲတင်မည်",
        questionNumber: "မေးခွန်း {current}",
        flagReview: "မှတ်သားထားရန်",
        metaExpand: "မေးခွန်းအချက်အလက် <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "ပိတ်ရန် <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "ရင်းမြစ်：",
        metaYear: "ခုနှစ်",
        metaQNum: "မေးခွန်းအမှတ်",
        metaCategory: "အမျိုးအစား",
        metaSubcategory: "အမျိုးအစားခွဲ",
        metaTopic: "အကြောင်းအရာ",
        ruleTitleCoding: "လက်တွေ့ပရိုဂရမ်မင်း Mock Exam",
        ruleDescCoding: "အိုင်တီပရိုဂရမ်မင်းစာမေးပွဲများကို တုပသည်။ အချိန်အတွင်း ကုဒ်ရေးရမည်။",
        launchBtnCoding: "စာမေးပွဲစတင်ရန်",
        cbtSimulatorCoding: "လက်တွေ့စာမေးပွဲ",
        expectedOutput: "မျှော်မှန်းရလဒ်",
        resultsTitle: "ရမှတ်မှတ်တမ်း",
        resultsTitleCbt: "CBT ရမှတ်မှတ်တမ်း",
        backToHome: "မူလစာမျက်နှာသို့",
        backToDojo: "道場သို့ပြန်သွားရန်",
        passLabel: "အောင်/ရှုံး အကဲဖြတ်မှု",
        passStatus: "အောင်မြင်သည်",
        failStatus: "ကျရှုံးသည်",
        correctCountText: "{count} / {total} ပုဒ် အောင်မြင်သည်",
        totalScoreText: "{score} / ၁၀၀၀ မှတ်",
        correctRate: "မှန်ကန်မှုနှုန်း",
        timeSpent: "ကြာချိန်:",
        detailsTitle: "မေးခွန်းအသေးစိတ် အကဲဖြတ်ချက်",
        reviewTitle: "မေးခွန်းများ ပြန်လည်သုံးသပ်ခြင်း",
        reviewTip: "အောက်ပါစာရင်းမှ မည်သည့်အကြောင်းအရာကိုမဆို နှိပ်၍ အသေးစိတ်ကြည့်ရှုနိုင်ပါသည်။",
        reviewCollapse: "ရှင်းလင်းချက်ကို ပိတ်မည်",
        reviewExplanation: "ရှင်းလင်းချက်",
        resultCorrect: "မှန်သည်",
        resultIncorrect: "မှားသည်",
        tableQNo: "နံပါတ်",
        tableTitle: "အကြောင်းအရာ / ခေါင်းစဉ်",
        tableField: "နယ်ပယ်",
        tableDifficulty: "ခက်ခဲမှု",
        tableResult: "ရလဒ်",
        tableYourAnswer: "သင့်အဖြေ",
        tableCorrectAnswer: "မှန်ကန်သောအဖြေ",
        tableAction: "လုပ်ဆောင်ချက်",
        tableActionDetail: "ကြည့်ရန်",
        personalizeTitle: "စိတ်ကြိုက်ရွေးချယ်စရာများ:",
        optShuffle: "အဖြေရွေးချယ်စရာများကို ရောနှောမည် (ア〜エ)",
        optNocalc: "တွက်ချက်မှုမေးခွန်းများကို ဖယ်ထုတ်မည်",
        all: "မေးခွန်းဟောင်းများ ကျပန်းရောနှောခြင်း (အကြံပြုသည်)",
        r8: "Reiwa 8 (R8) မေးခွန်းဟောင်းများ",
        r7: "Reiwa 7 (R7) မေးခွန်းဟောင်းများ",
        r6: "Reiwa 6 (R6) မေးခွန်းဟောင်းများ",
        r5: "Reiwa 5 (R5) မေးခွန်းဟောင်းများ",
        r4: "Reiwa 4 (R4) မေးခွန်းဟောင်းများ",
        r3: "Reiwa 3 (R3) မေးခွန်းဟောင်းများ",
        cbtQ100: "၁၀၀ ပုဒ် (ပုံမှန် CBT စံနှုန်း - မိနစ် ၁၂၀)",
        cbtQ50: "၅၀ ပုဒ် (အတိုချုပ် Mock - မိနစ် ၆၀)",
        cbtQ30: "၃၀ ပုဒ် (အမြန်ဖြေဆိုခြင်း - မိနစ် ၃၆)",
        cbtQ10: "၁၀ ပုဒ် (အသေးစား စာမေးပွဲ - မိနစ် ၁၂)",
        q5: "၅ ပုဒ် (အမြန်စမ်းသပ်မှု - ၁၅ မိနစ်)",
        q10: "၁၀ ပုဒ် (စံလက်တွေ့ - ၃၀ မိနစ်)",
        q15: "၁၅ ပုဒ် (အပြည့်အဝစိန်ခေါ်မှု - ၄၅ မိနစ်)",
        gridTitle: "အဖြေလွှာဇယား",
        unanswered: "မဖြေရသေး",
        answered: "ဖြေပြီး",
        flagged: "မှတ်သားထား",
        totalScoreLabel: "စုစုပေါင်းရမှတ်",
        domainStrat: "မဟာဗျူဟာနယ်ပယ် (Strategy)",
        domainMan: "စီမံခန့်ခွဲမှုနယ်ပယ် (Management)",
        domainTech: "နည်းပညာနယ်ပယ် (Technology)",
        domainSubb: "ဘာသာရပ် B (အသုံးချ)"
      },
      toast: {
        alreadyFirst: "ပထမဆုံးမေးခွန်းဖြစ်သည်။",
        alreadyLast: "နောက်ဆုံးမေးခွန်းဖြစ်သည်။",
        saved: "သိမ်းပြီးပါပြီ။",
        verifySuccess: "အောင်မြင်ပါသည်။ အဖြေကို သိမ်းပြီးပါပြီ။",
        dbReset: "Mock database ကို ပြန်လည်စတင်ပြီးပါပြီ။",
        examStarted: "CBT Mock Exam စတင်ပါပြီ။",
        lessonQuizPassed: "🏆 ဂုဏ်ယူပါသည်။ သင်ခန်းစာ {title} အတွက် မေးခွန်းများအားလုံး ဖြေဆိုအောင်မြင်ပြီးဖြစ်သောကြောင့် သင်ခန်းစာ ပြီးဆုံးကြောင်း မှတ်သားလိုက်ပါပြီ။",
        dbSwitched: "🗄️ <{dbName}> database သို့ ပြောင်းလဲပြီးပါပြီ။ ယခု {tables} ဇယားများကို မေးမြန်းနိုင်ပါပြီ။",
        sqlHint: "💡 အရိပ်အမြွက်- အောက်ပါ SQL တည်ဆောက်ပုံကို ကိုးကားပါ-<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ ပြီးဆုံးထားသော သင်ခန်းစာ မရှိသေးပါ။ ကျေးဇူးပြု၍ အနည်းဆုံး သင်ခန်းစာတစ်ခုကို လေ့လာပြီး အောင်မြင်အောင် ဖြေဆိုပါ။",
        challengeStarted: "🔥 ကျပန်းစိန်ခေါ်မှု စတင်ပါပြီ။ အောင်မြင်ပြီး သင်ခန်းစာ {lessonCount} ခုမှ စုစုပေါင်း မေးခွန်း {count} ခု ရှိပါမည်။",
        challengeEnded: "✅ ကျပန်းစိန်ခေါ်မှု ပြီးဆုံးပါပြီ။",
        challengeQuestionCorrect: "🎉 မှန်ကန်ပါသည်။ သင်ခန်းစာ {sourceId} မှ မေးခွန်းကို အောင်မြင်ပါပြီ။",
        challengeSuccess: "စိန်ခေါ်မှု အောင်မြင်ပါသည်!",
        examEmpty: "⚠️ သတ်မှတ်ချက်နှင့် ကိုက်ညီသော မေးခွန်းဟောင်းများ မရှိပါ၊ ကျေးဇူးပြု၍ ရွေးချယ်စရာများကို ပြောင်းလဲပါ။",
        examTimeout: "⏰ အချိန်ပြည့်သွားပါပြီ။ အလိုအလျောက် အဖြေလွှာတင်သွင်းပြီးပါပြီ။",
        examPassed: "🎉 ဂုဏ်ယူပါသည်။ ဤ Mock Test ကို အောင်မြင်စွာ ဖြေဆိုနိုင်ခဲ့ပါသည်။",
        examFailed: "💪 သတ်မှတ်ထားသော အမှတ်မမီပါ၊ ကျေးဇူးပြု၍ ဆက်လက်ကြိုးစားပါ။",
        examDbNotFound: "⚠️ ဤဘာသာရပ်အတွက် မေးခွန်းဒေတာဘေ့စ် မတွေ့ရှိပါ!",
        practicalExamStarted: "📝 လက်တွေ့ Mock Exam စတင်ပါပြီ။ သတ်မှတ်ချက်အတိုင်း ကုဒ်ရေးသားပါ။",
        sqlSyntaxError: "❌ SQL တည်ဆောက်ပုံ မှားယွင်းနေသဖြင့် စစ်ဆေးမှု မအောင်မြင်ပါ။",
        sqlOutputMismatch: "❌ ထွက်ပေါ်လာသော ရလဒ်သည် မျှော်မှန်းရလဒ်နှင့် ကွဲလွဲနေပါသည်၊ စီစဉ်မှု သို့မဟုတ် ကော်လံများကို စစ်ဆေးပါ။",
        compileError: "❌ Compile မှားယွင်းနေသဖြင့် စစ်ဆေးမှု မအောင်မြင်ပါ။",
        runtimeError: "❌ Runtime မှားယွင်းနေသဖြင့် စစ်ဆေးမှု မအောင်မြင်ပါ။",
        outputMismatch: "❌ ရလဒ်များ ကွဲလွဲနေသဖြင့် စစ်ဆေးမှု မအောင်မြင်ပါ။",
        practicalExamSubmitted: "🎉 လက်တွေ့စာမေးပွဲ အောင်မြင်စွာ တင်သွင်းပြီးပါပြီ။ အသေးစိတ် ရမှတ်မှတ်တမ်း ထွက်ပေါ်လာပါပြီ။"
      },
      dialog: {
        resetDbConfirm: "Mock database ကို မူလအခြေအနေသို့ ပြန်လည်စတင်မည်။ ဆက်လုပ်မလား？",
        exitExamConfirm: "လက်ရှိစာမေးပွဲမှ ထွက်မလား？ တိုးတက်မှုကို သိမ်းမည်မဟုတ်ပါ။",
        switchExamConfirm: "လက်ရှိစာမေးပွဲတစ်ခု လုပ်ဆောင်နေသည်။ ဘာသာပြောင်းပါက စာမေးပွဲရပ်သွားမည်။ ဆက်လုပ်မလား？",
        clearCodeConfirm: "ကုဒ်ကို ရှင်းမလား？",
        resetProgressConfirm: "{subject} သင်ယူမှုတိုးတက်မှု အားလုံးကို ပြန်လည်စတင်ရန် သေချာပါသလား။",
        submitExamConfirm: "အဖြေလွှာ တင်သွင်းရန် သေချာပါသလား။",
        submitExamUnansweredConfirm: "မဖြေရသေးသော မေးခွန်း {count} ခု ကျန်ရှိနေပါသေးသည်။ တင်သွင်းရန် သေချာပါသလား။",
        submitCodingExamUnansweredConfirm: "မဖြေရသေးသော သို့မဟုတ် မအောင်မြင်သော မေးခွန်း {count} ခု ကျန်ရှိနေပါသေးသည်။ တင်သွင်းရန် သေချာပါသလား။"
      },
      tools: {
        quickStart: "အမြန်စတင်ရန်",
        quickStartTitle: "သင်ယူမှု လုပ်ငန်းစဉ်",
        quickStartDesc: "ဘာသာရပ်ရွေးရန် → သင်ခန်းစာလေ့လာရန် → လေ့ကျင့်ရန် → စာရင်းဇယားကြည့်ရန်",
        tools: "ကိရိယာများ",
        openMenu: "ကိရိယာမီနူးဖွင့်ရန်",
        closeMenu: "ကိရိယာမီနူးပိတ်ရန်",
        learningTools: "သင်ယူမှုကိရိယာများ",
        glossary: "ဝေါဟာရစာရင်း",
        glossaryDesc: "IT / SQL / ပရိုဂရမ်းမင်းဝေါဟာရများ ရှာဖွေရန်",
        settings: "ဆက်တင်များ",
        settingsDesc: "သင်ယူမှုကိရိယာ ဦးစားပေးမှုများ ပြင်ဆင်ရန်",
        dangerous: "အန္တရာယ်ရှိသော လုပ်ဆောင်ချက်များ",
        resetProgress: "တိုးတက်မှုပြန်လည်သတ်မှတ်ရန်",
        resetProgressDesc: "လက်ရှိသင်ယူမှုတိုးတက်မှုကို ရှင်းလင်းမည်။ အတည်ပြုရန်လိုအပ်သည်",
        confirmResetTitle: "တိုးတက်မှုပြန်လည်သတ်မှတ်ရန်？",
        confirmResetBody: "ဤလုပ်ဆောင်ချက်သည် သင်၏လက်ရှိသင်ယူမှုတိုးတက်မှုကို ရှင်းလင်းမည်။ ဆက်လုပ်မည်လား？",
        cancel: "မလုပ်တော့ပါ",
        confirmReset: "ပြန်လည်သတ်မှတ်ရန် အတည်ပြုမည်",
        examTools: "စာမေးပွဲ ကိရိယာများ",
        examHistory: "စာမေးပွဲ မှတ်တမ်း",
        examHistoryDesc: "မကြာသေးမီ စာမေးပွဲမှတ်တမ်းများ ကြည့်ရှုရန်",
        dashboard: "သင်ယူမှု စာရင်းဇယားများ",
        dashboardDesc: "သင်ယူမှုတိုးတက်မှုနှင့် ဒေတာ အကျဉ်းချုပ်ကြည့်ရန်",
        exportData: "သင်ယူမှုဒေတာ ထုတ်ယူရန်",
        exportDataDesc: "ဒေသနဲ့ဆိုင်သော သင်ယူမှုဒေတာ backup ဒေါင်းလုဒ်ဆွဲရန် (JSON)",
        continueLearning: "သင်ယူမှုဆက်လုပ်ရန်",
        continueLearningDesc: "ယခင်တိုးတက်မှုမှ ဆက်လုပ်ရန်",
        exportSuccess: "ထုတ်ယူမှု အောင်မြင်သည်",
        exportPartialSuccess: "ပျက်စီးနေသောဒေတာကို ကျော်သွားပြီး ထုတ်ယူပြီးပါပြီ",
        exportFailed: "ထုတ်ယူမှု မအောင်မြင်ပါ",
        exportScope: "စကားဝှက်နှင့် key မပါ — သင်ယူမှုဒေတာသာ",
        importPreview: "Backup ထည့်သွင်းမှု ကြိုကြည့်ရန်",
        importPreviewDesc: "ဖတ်ရှုသာ ကြိုကြည့်မှု — ဒေတာများ မပြောင်းလဲပါ",
        importSelectFile: "Backup ဖိုင်ရွေးချယ်ရန်",
        importParseFailed: "ဖိုင် parse မအောင်မြင်ပါ",
        importSensitiveBlocked: "အထိခိုက်မခံသော ကွက်လပ်များ တွေ့ရှိ၍ ပိတ်ဆို့ခဲ့သည်",
        importImportable: "ထည့်သွင်းနိုင်သော ဒေတာ",
        importNew: "အသစ်",
        importConflict: "ဆန့်ကျင်မှု",
        importSkipped: "ကျော်သွားသည်",
        importIncompatibleVersion: "ကိုက်ညီမှုမရှိသော Backup ဗားရှင်း",
        importFileTooLarge: "ဖိုင်သည် အလွန်ကြီးသည်",
        importClosePreview: "ကြိုကြည့်မှု ပိတ်ရန်",
        importSummary: "ထည့်သွင်းမှု အကျဉ်းချုပ်",
        importNoData: "ထည့်သွင်းနိုင်သော ဒေတာ မရှိပါ",
        importSection: "အပိုင်း",
        importSectionLessons: "ပြီးဆုံးသော သင်ခန်းစာများ",
        importSectionQuiz: "Quiz ရလဒ်များ",
        importSectionWrongBook: "မှားယွင်းမှုစာအုပ်",
        importSectionBookmarks: "Bookmark များ",
        importSectionTyping: "ရိုက်ခြင်း မှတ်တမ်း",
        importSectionExam: "စာမေးပွဲ မှတ်တမ်း",
        importSectionGoals: "သင်ယူမှု ပန်းတိုင်များ",
        importSectionSettings: "ဘာသာစကားနှင့် Theme",
        importSectionSync: "ချိတ်ဆက်မှု အခြေအနေ",
        importSyncBlocked: "ထည့်သွင်းမဝင် — ချိတ်ဆက်မှုအခြေအနေသည် စက်ပစ္စည်းမှ စီမံသည်"
      },
      examHistory: {
        title: "Recent Exam History",
        subtitle: "View saved mock exam results",
        close: "Close",
        filterAll: "All",
        empty: "No exam history yet",
        emptyHint: "Complete a mock exam and records will appear here",
        questions: "Questions",
        correct: "Correct",
        accuracy: "Accuracy",
        score: "Score",
        passed: "Passed",
        failed: "Failed",
        time: "Time spent",
        viewDetails: "View details",
        hideDetails: "Hide details",
        examConfig: "Exam config",
        fieldScores: "Field scores",
        questionResults: "Question results",
        yearFilter: "Year filter",
        submittedAt: "Submitted at",
        yourAnswer: "Your answer",
        correctAnswer: "Correct answer",
        unanswered: "Unanswered",
        questionNotFound: "Question data not found"
      },
      wrongBook: {
        title: "Wrong Book",
        subtitle: "Review questions you answered incorrectly",
        emptyTitle: "No wrong answers yet",
        emptyDesc: "Incorrect answers will be collected here automatically",
        all: "All",
        unmastered: "Unmastered",
        mastered: "Mastered",
        markMastered: "Mark mastered",
        unmarkMastered: "Mark unmastered",
        remove: "Remove",
        wrongCount: "Wrong count",
        lastWrongAt: "Last wrong",
        yourAnswer: "Your wrong answer",
        correctAnswer: "Correct answer",
        explanation: "Explanation",
        source: "Source",
        confirmRemove: "Remove this question from the wrong book?",
        viewDetails: "View details",
        hideDetails: "Hide details",
        choices: "Choices",
        question: "Question",
        unknownAnswer: "Not recorded"
      },
      message: {
        inputSqlRequired: "SQL query ထည့်ပါ။"
      },
      settings: {
        language: "ဘာသာစကား"
      },
      lesson: {
        locateBookPage: "မူရင်းစာမျက်နှာသို့ သွားရန်",
        explanationJa: "ရှင်းလင်းချက် (ဂျပန်)",
        explanationZh: "ရှင်းလင်းချက် (တရုတ်)",
        compareMode: "ယှဉ်တွဲကြည့်ရန် (ဂျပန်/တရုတ်)",
        japaneseOnly: "ဂျပန်ဘာသာ သီးသန့်",
        chineseOnly: "တရုတ်ဘာသာ သီးသန့်",
        analogy: "နှိုင်းယှဉ်ချက်",
        sqlExample: "SQL နမူနာ",
        javaExample: "Java နမူနာကုဒ်",
        pythonExample: "Python နမူနာကုဒ်",
        glossary: "အဓိကဝေါဟာရများနှင့် ညီမျှခြင်းများ",
        copyToEditor: "Editor ထဲသို့ ကူးယူမည်",
        showPdf: "မူရင်း PDF ကို ပြမည်",
        translationZh: "တရုတ်ဘာသာပြန်",
        checklist: "အချက်အလက်စစ်ဆေးရန်",
        calculator: "တွက်ချက်ရေးကိရိယာ",
        calculatorSelect: "ကိရိယာကို ရွေးပါ:"
      },
      flashcard: {
        title: "ဝေါဟာရကတ်များ",
        flip: "လှန်မည်",
        clickToFlip: "ဂျပန်-တရုတ် ဘာသာပြန်နှင့် အဓိပ္ပာယ်ကို ကြည့်ရန် ကတ်ကို နှိပ်ပါ -",
        clickToFlipHint: "အနောက်ဘက်ကို ကြည့်ရန် နှိပ်ပါ",
        localizedDesc: "{title} အတွက် အဓိကဝေါဟာရကတ်။"
      },
      glossary: {
        title: "IT ဝေါဟာရစာရင်း",
        open: "ဝေါဟာရစာရင်း",
        close: "ပိတ်မည်",
        searchPlaceholder: "ဝေါဟာရရှာဖွေရန်...",
        allCategories: "အားလုံး",
        filterDatabase: "ဒေတာဘေ့စ်",
        filterSecurity: "လုံခြုံရေး",
        filterNetwork: "ကွန်ရက်",
        filterProgramming: "ပရိုဂရမ်မင်း",
        filterSystem: "စနစ်",
        category: "အမျိုးအစား",
        japanese: "ဂျပန်",
        english: "အင်္ဂလိပ်",
        currentLanguage: "လက်ရှိဘာသာစကား",
        aliases: "အမည်ကွဲများ",
        relatedTerms: "ဆက်စပ်ဝေါဟာရများ",
        example: "နမူနာ",
        source: "ရင်းမြစ်",
        needsReview: "⚠ ဘာသာပြန်စစ်ဆေးရန်",
        noResults: "ဝေါဟာရမတွေ့ပါ",
        keepEnglishNote: "အင်္ဂလိပ်နည်းပညာဝေါဟာရများ ထိန်းသိမ်းထားသည်",
        resultCount: "{count} ခု"
      },
      quiz: {
        title: "နားလည်မှု စစ်ဆေးခြင်း",
        questionCount: "မေးခွန်း {current} / စုစုပေါင်း {total}",
        previous: "ယခင်မေးခွန်း",
        next: "နောက်မေးခွန်း",
        submitAnswer: "အဖြေတင်မည်",
        empty: "ဤသင်ခန်းစာတွင် quiz မရှိသေးပါ။",
        unavailable: "{title} အတွက် quiz သည် ဤဘာသာစကားတွင် မပြင်ဆင်ရသေးပါ။",
        thisLesson: "ဤသင်ခန်းစာ"
      },
      typing: {
        library: "Typing စာကြည့်တိုက်",
        libraryDesc: "N3 မှ N1 အထိ လက်တွေ့သုံး ဂျပန်စာရိုက်လေ့ကျင့်ခန်း။ မိမိစိတ်ကြိုက် လေ့ကျင့်နိုင်ပါသည်။",
        search: "စာကြည့်တိုက် ရှာဖွေရန်",
        searchPlaceholder: "ခေါင်းစဉ် သို့မဟုတ် အကြောင်းအရာ",
        articleList: "ဆောင်းပါးစာရင်း",
        randomArticle: "ကျပန်းဆောင်းပါး",
        restart: "ပြန်လည်စတင်မည်",
        timeLimit: "အချိန်ကန့်သတ်ချက်",
        imeMode: "IME စာရိုက်မုဒ်",
        speed: "မြန်နှုန်း",
        accuracy: "မှန်ကန်မှုနှုန်း",
        progress: "တိုးတက်မှု",
        edits: "ပြင်ဆင်မှုအကြိမ်ရေ",
        time: "အချိန်",
        recentScores: "လတ်တလောရလဒ်များ",
        personalReview: "ကိုယ်ပိုင်ပြန်လည်လေ့ကျင့်မှု",
        reviewDesc: "မှန်ကန်မှု ၉၈% အောက်ရှိသော ဆောင်းပါးများကို ပြန်လည်လေ့ကျင့်ရန်စာရင်းထဲသို့ အလိုအလျောက် ထည့်သွင်းပေးမည်ဖြစ်ပါသည်။",
        reviewCount: "ပြန်လည်လေ့ကျင့်ရန် {count} ပုဒ်ကျန်ပါသည်",
        emptyReview: "ပြန်လည်လေ့ကျင့်ရန် ဆောင်းပါးမရှိပါ",
        imePlaceholder: "ဂျပန်စာရိုက်စနစ် (IME) ကို ဖွင့်ပြီး ဤနေရာတွင် ရိုက်ထည့်ပါ။",
        typingHelp: "ဂျပန်စာရိုက်စနစ်ဖြင့် စာရိုက်ပါ။ ပြင်ဆင်မှုများကို မှတ်တမ်းတင်သွားမည်ဖြစ်ပါသည်။",
        resultTitle: "လေ့ကျင့်မှု ပြီးဆုံးပါပြီ",
        continueRandom: "ကျပန်းလေ့ကျင့်မှု ဆက်လုပ်မည်",
        favoriteBtn: "အကြိုက်ဆုံး",
        favoriteTitle: "ဤဆောင်းပါးကို သိမ်းဆည်းရန်",
        noLimit: "အချိန်ကန့်သတ်ချက်မရှိ",
        t1min: "၁ မိနစ်",
        t3min: "၃ မိနစ်",
        t5min: "၅ မိနစ်",
        t10min: "၁၀ မိနစ်"
      },
      console: {
        output: "ရလဒ်",
        rows: "{count} ကြောင်း",
        emptyResult: "SQL ကို run ပြီးနောက် ရလဒ်ဇယားကို ဤနေရာတွင် ပြသမည်ဖြစ်ပါသည်။",
        emptyResultSub: "(SQL run ပြီးပါက ရလဒ်များ ဤနေရာတွင် ပေါ်လာမည်)",
        maximize: "ချဲ့မည်",
        minimize: "ချုံ့မည်"
      },
      auth: {
        localMode: "ဒေသတွင်းမုဒ်",
        notLoggedIn: "အကောင့်မဝင်ရသေး",
        account: "အကောင့်",
        sync: "စင့်ခ်",
        syncInDev: "စင့်ခ်လုပ်ဆောင်ချက်ကို ဖော်ဆောင်နေဆဲ",
        continueLocal: "ဒေသတွင်းသုံးဆက်ရန်",
        mockSignIn: "အကောင့်ဝင်ရောက်ခြင်းကို အတုခိုးရန်",
        mockSignOut: "အကောင့်အတုခိုးခြင်းမှ ထွက်ရန်",
        exportSnapshot: "ဒေသတွင်းဒေတာကို ထုတ်ယူရန်",
        deviceId: "စက်ပစ္စည်း ID",
        pendingSync: "စင့်ခ်လုပ်ရန်ကျန်နေ",
        lastSync: "နောက်ဆုံးစင့်ခ်",
        noUpload: "ဒေတာကို upload မလုပ်ပါ",
        close: "ပိတ်မည်",
        signedIn: "အကောင့်ဝင်ထားသည်",
        signedOut: "အကောင့်ထွက်ထားသည်",
        status: "အခြေအနေ",
        anonymous: "အမည်မသိ",
        loginTitle: "အကောင့်ဝင်ရန်",
        registerTitle: "အကောင့်ဖွင့်ရန်",
        displayName: "ပြသမည့်အမည်",
        email: "အီးမေးလ်",
        password: "စကားဝှက်",
        confirmPassword: "စကားဝှက်အတည်ပြုရန်",
        loginButton: "အကောင့်ဝင်ရန်",
        registerButton: "အကောင့်ဖွင့်ရန်",
        magicLinkOption: "အီးမေးလ်လင့်ခ်ဖြင့်ဝင်ရန်",
        usernameDisplayOnly: "ပြသရန်သာဖြစ်သည်။ အီးမေးလ်ဖြင့်ဝင်ရန်။",
        emailRequired: "အီးမေးလ်ထည့်သွင်းပါ",
        invalidEmail: "မှန်ကန်သောအီးမေးလ်လိပ်စာထည့်သွင်းပါ",
        credentialsRequired: "အီးမေးလ်နှင့်စကားဝှက်ထည့်သွင်းပါ",
        passwordRequired: "စကားဝှက်ထည့်သွင်းပါ",
        passwordTooShort: "စကားဝှက်သည် အနည်းဆုံး ၆ လုံးရှိရပါမည်",
        passwordMismatch: "စကားဝှက်မကိုက်ညီပါ",
        usernameRequired: "ပြသမည့်အမည်ထည့်သွင်းပါ",
        loginSuccess: "အကောင့်ဝင်ရောက်မှုအောင်မြင်သည်",
        loginFailed: "အကောင့်ဝင်ရောက်မှုမအောင်မြင်ပါ",
        registerSuccess: "အကောင့်ဖွင့်ခြင်းအောင်မြင်သည်",
        registerFailed: "အကောင့်ဖွင့်ခြင်းမအောင်မြင်ပါ",
        confirmEmailNotice: "အကောင့်ဖွင့်ခြင်းအောင်မြင်သည်။ အီးမေးလ်ကိုစစ်ဆေးပြီး အတည်ပြုပါ။",
        invalidCredentials: "အီးမေးလ်သို့မဟုတ်စကားဝှက်မှားယွင်းသည်",
        emailNotConfirmed: "အီးမေးလ်အတည်မပြုရသေးပါ",
        userAlreadyExists: "ဤအီးမေးလ်သည် မှတ်ပုံတင်ပြီးဖြစ်သည်",
        tooManyRequests: "တောင်းဆိုမှုများလွန်းသည်။ နောက်မှထပ်ကြိုးစားပါ",
        signOut: "အကောင့်ထွက်ရန်",
        sendMagicLink: "လော့ဂ်အင်လင့်ခ်ပို့ရန်",
      }
    },

    "vi-VN": {
      common: {
        save: "Lưu",
        cancel: "Hủy",
        confirm: "Xác nhận",
        delete: "Xóa",
        reset: "Đặt lại",
        resetProgress: "Đặt lại tiến độ",
        loading: "Đang tải...",
        themeToggle: "Chuyển chế độ sáng/tối",
        close: "Đóng",
        open: "Mở",
        search: "Tìm kiếm",
        random: "Ngẫu nhiên",
        restart: "Bắt đầu lại",
        copy: "Sao chép",
        maximize: "Phóng to",
        minimize: "Thu nhỏ",
        challenging: "Đang thử thách",
        lessonIndex: "Bài {id}",
        changeQuestion: "Đổi câu hỏi",
        stop: "Kết thúc",
        subtitle: "Nền tảng hỗ trợ học tập"
      },
      nav: {
        sql: "Học SQL",
        java: "Học Java",
        python: "Học Python",
        itpass: "Luyện thi IT Passport",
        sg: "Luyện thi SG",
        typing: "Gõ tiếng Nhật",
        codingTyping: "Gõ mã nguồn",
        sqlChallenge: "Thử thách ngẫu nhiên"
      },
      moduleSwitch: {
        title: "Chọn khu vực học tập",
        close: "Đóng",
        triggerLabel: "Mở bảng chọn khu vực"
      },
      moduleDesc: {
        sql: "Cơ sở dữ liệu, truy vấn, thực hành và sandbox",
        sg: "Luyện thi Quản lý An toàn Thông tin",
        itpass: "IT Passport cơ bản và đề thi cũ",
        typing: "Nhập và luyện gõ tiếng Nhật",
        codingTyping: "Nhập mã và luyện cú pháp",
        java: "Java cơ bản, cú pháp và thực hành",
        python: "Python cơ bản, cú pháp và thực hành"
      },
      codingTyping: {
        title: "Luyện gõ mã nguồn",
        subtitle: "Nắm vững cú pháp lập trình qua luyện gõ. Chọn ngôn ngữ và danh mục để bắt đầu.",
        language: "Ngôn ngữ", category: "Danh mục", allCategories: "Tất cả",
        difficulty: "Độ khó", code: "Mã nguồn", input: "Nhập",
        inputPlaceholder: "Gõ mã tại đây...", focus: "Trọng tâm", hint: "Gợi ý",
        speed: "CPM", accuracy: "Độ chính xác", progress: "Tiến độ",
        time: "Thời gian", errors: "Lỗi", complete: "Hoàn thành!",
        result: "Kết quả", totalTime: "Thời gian", errorChars: "Lỗi thường gặp",
        recentHistory: "Lịch sử", noHistory: "Chưa có lịch sử",
        next: "Tiếp (Alt+N)", random: "Ngẫu nhiên (Alt+S)", restart: "Làm lại (Alt+R)",
        continueRandom: "Tiếp tục ngẫu nhiên",
        python: "Python", java: "Java", sql: "SQL", symbols: "Ký hiệu",
        normalizeFullWidth: "Toàn→Bán sừng", lenientIndent: "Thụt lề linh hoạt",
        sendToSandbox: "Gửi đến Sandbox",        runInSandbox: "Chạy trong Sandbox",        sandboxUnavailable: "Sandbox không khả dụng",        sentToSandbox: "Đã gửi đến Sandbox",        openSandbox: "Mở Sandbox",        shortcutHelp: "Phím tắt: Alt+N Tiếp / Alt+R Làm lại / Alt+S Ngẫu nhiên",
        summaryLine: "CPM: {cpm} | Độ chính xác: {acc}% | Lỗi: {err} | TG: {time}",
        backToList: "Quay lại danh sách",
        exerciseList: "Danh sách bài tập"
      },
      subnav: {
        lessons: "Sách giáo khoa & luyện tập",
        practicalExam: "Bài thi mô phỏng thực hành",
        study: "Học theo chương",
        pastExams: "Đề cũ & thi mô phỏng",
        javaIntro: "Nhập môn (Cơ bản) — Chương 1〜8",
        javaPractice: "Thực hành (Nâng cao) — Chương 1〜11"
      },
      sidebar: {
        progress: "Tiến độ học tập",
        unitList: "Danh sách bài học"
      },
      sandbox: {
        run: "Chạy",
        playground: "Sandbox",
        runSql: "Chạy SQL",
        resetDb: "Đặt lại DB",
        hint: "Gợi ý",
        maximize: "Phóng to",
        output: "Kết quả",
        sqlPlaceholder: "Vui lòng nhập câu lệnh SQL vào đây...",
        currentMission: "Nhiệm vụ hiện tại",
        randomPractice: "Luyện tập ngẫu nhiên",
        databaseStructure: "Cấu trúc cơ sở dữ liệu tham khảo",
        schoolDb: "Trường học DB",
        bookstoreDb: "Nhà sách DB",
        javaSandboxTitle: "Java Programming Sandbox",
        pythonSandboxTitle: "Python Programming Sandbox",
        pythonSandboxDesc: "Python 3.12 · 10s · Offline Sandbox",
        sqlPlayground: "SQL Playground",
        chooseLessonLoadCode: "— Chọn một bài học để tải mã mẫu —",
        toolbarTemplate: "Mẫu",
        toolbarTemplateTitle: "Tải mẫu bài học",
        toolbarCopy: "Sao chép",
        toolbarCopyTitle: "Sao chép mã",
        toolbarClear: "Đặt lại",
        toolbarClearTitle: "Xóa sạch và đặt lại",
        javaEditorPlaceholder: "Nhập mã Java tại đây...",
        pythonEditorPlaceholder: "Nhập mã Python tại đây...",
        stdinHeader: "Đầu vào tiêu chuẩn (stdin)",
        stdinPlaceholder: "Dữ liệu đầu vào cho chương trình...",
        runCode: "Chạy",
        readyStatus: "Sẵn sàng",
        outputPlaceholder: "(Kết quả chạy chương trình sẽ xuất hiện ở đây)"
      },
      exam: {
        verify: "Kiểm tra và lưu",
        submit: "Nộp bài",
        previous: "Câu trước",
        next: "Câu tiếp",
        exit: "Thoát bài thi",
        ruleTitle: "Past Question Dojo — Hệ thống thi thử CBT",
        ruleDesc: "Mô phỏng môi trường thi trên máy tính (CBT) bằng cách sử dụng các câu hỏi cũ.",
        ruleDetailTitle: "Quy tắc thi:",
        ruleDetail1: "Các câu hỏi là các nhiệm vụ truy vấn cơ sở dữ liệu hoặc lập trình thực hành.",
        ruleDetail2: "Bạn có thể viết, chạy và gỡ lỗi mã ngay lập tức trong trình soạn thảo sandbox.",
        ruleDetail3: "Nhấp \"Kiểm tra và lưu\" để xác minh mã của bạn với các trường hợp kiểm thử.",
        ruleDetail4: "Các nhiệm vụ đã vượt qua sẽ được tính điểm. Bạn có thể sửa đổi mã nếu cần.",
        ruleDetail5: "Hãy chắc chắn nộp bài trước khi hết giờ; hết giờ sẽ tự động nộp bài.",
        selectYear: "Chọn năm thi:",
        questionCount: "Số lượng câu hỏi:",
        launchBtn: "Bắt đầu thi thử",
        status: "Trạng thái kiểm tra: ",
        notStarted: "Chưa thử sức",
        cbtSimulator: "Bộ mô phỏng CBT",
        remainingTime: "Thời gian còn lại: ",
        exitCbt: "Thoát bài thi",
        submitCbt: "Nộp bài thi",
        questionNumber: "Câu {current}",
        flagReview: "Đánh dấu xem lại",
        metaExpand: "Thông tin câu hỏi <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "Thu gọn <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "Nguồn：",
        metaYear: "Năm",
        metaQNum: "Số câu hỏi",
        metaCategory: "Danh mục",
        metaSubcategory: "Danh mục phụ",
        metaTopic: "Chủ đề",
        ruleTitleCoding: "Bài thi thử lập trình thực hành",
        ruleDescCoding: "Mô phỏng các kỳ thi lập trình thực tế. Viết mã trong thời gian giới hạn.",
        launchBtnCoding: "Bắt đầu làm bài",
        cbtSimulatorCoding: "Thi thực hành",
        expectedOutput: "Kết quả mong đợi",
        resultsTitle: "Bảng điểm đánh giá",
        resultsTitleCbt: "Báo cáo bảng điểm CBT",
        backToHome: "Quay lại trang chủ",
        backToDojo: "Quay lại võ đường",
        passLabel: "Đánh giá kết quả",
        passStatus: "Đạt",
        failStatus: "Chưa đạt",
        correctCountText: "{count} / {total} câu đạt",
        totalScoreText: "{score} / 1000 điểm",
        correctRate: "Tỷ lệ chính xác",
        timeSpent: "Thời gian làm bài:",
        detailsTitle: "Chi tiết đánh giá câu hỏi",
        reviewTitle: "Xem lại câu hỏi & Phân tích lỗi",
        reviewTip: "Nhấp vào bất kỳ mục nào để mở rộng xem chi tiết.",
        reviewCollapse: "Thu gọn giải thích",
        reviewExplanation: "Giải thích",
        resultCorrect: "Đúng",
        resultIncorrect: "Sai",
        tableQNo: "STT",
        tableTitle: "Chủ đề / Tiêu đề",
        tableField: "Lĩnh vực",
        tableDifficulty: "Độ khó",
        tableResult: "Kết quả",
        tableYourAnswer: "Câu trả lời của bạn",
        tableCorrectAnswer: "Đáp án đúng",
        tableAction: "Thao tác",
        tableActionDetail: "Xem",
        personalizeTitle: "Tùy chọn cá nhân hóa:",
        optShuffle: "Tráo đổi thứ tự câu trả lời (ア〜エ)",
        optNocalc: "Lọc câu hỏi tính toán (không hiển thị câu hỏi có công thức)",
        all: "Hỗn hợp ngẫu nhiên các đề cũ (Khuyến nghị)",
        r8: "Đề thi năm Reiwa 8 (R8)",
        r7: "Đề thi năm Reiwa 7 (R7)",
        r6: "Đề thi năm Reiwa 6 (R6)",
        r5: "Đề thi năm Reiwa 5 (R5)",
        r4: "Đề thi năm Reiwa 4 (R4)",
        r3: "Đề thi năm Reiwa 3 (R3)",
        cbtQ100: "100 câu (Chuẩn CBT chính thức - 120 phút)",
        cbtQ50: "50 câu (Thi thử rút gọn - 60 phút)",
        cbtQ30: "30 câu (Luyện nhanh - 36 phút)",
        cbtQ10: "10 câu (Trắc nghiệm nhỏ - 12 phút)",
        q5: "5 câu (Kiểm tra nhanh - 15 phút)",
        q10: "10 câu (Tiêu chuẩn - 30 phút)",
        q15: "15 câu (Thử thách đầy đủ - 45 phút)",
        gridTitle: "Bảng tiến độ câu hỏi",
        unanswered: "Chưa trả lời",
        answered: "Đã trả lời",
        flagged: "Đã đánh dấu",
        totalScoreLabel: "Tổng điểm",
        domainStrat: "Lĩnh vực chiến lược (Strategy)",
        domainMan: "Lĩnh vực quản lý (Management)",
        domainTech: "Lĩnh vực công nghệ (Technology)",
        domainSubb: "Môn B (Ứng dụng)"
      },
      toast: {
        alreadyFirst: "Đây là câu hỏi đầu tiên.",
        alreadyLast: "Đây là câu hỏi cuối cùng.",
        saved: "Đã lưu.",
        verifySuccess: "Đạt. Câu trả lời đã được lưu.",
        dbReset: "Cơ sở dữ liệu mô phỏng đã được đặt lại.",
        examStarted: "Bài thi mô phỏng CBT đã bắt đầu.",
        lessonQuizPassed: "🏆 Xin chúc mừng! Bạn đã vượt qua tất cả bài kiểm tra của bài học {title}, bài học này đã được đánh dấu là Hoàn thành!",
        dbSwitched: "🗄️ Đã chuyển sang cơ sở dữ liệu <{dbName}>. Hiện tại bạn có thể truy vấn các bảng: {tables}.",
        sqlHint: "💡 Gợi ý: Tham khảo cấu trúc SQL dưới đây:<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ Chưa có bài học nào được hoàn thành! Vui lòng học và hoàn thành ít nhất một bài trước.",
        challengeStarted: "🔥 Bắt đầu thử thách ngẫu nhiên! Tổng cộng có {count} câu hỏi từ {lessonCount} bài học đã hoàn thành.",
        challengeEnded: "✅ Thử thách ngẫu nhiên kết thúc.",
        challengeQuestionCorrect: "🎉 Chính xác! Bạn đã vượt qua câu hỏi từ bài học {sourceId}!",
        challengeSuccess: "Thử thách hoàn thành!",
        examEmpty: "⚠️ Không tìm thấy đề cũ phù hợp với tiêu chí, vui lòng thay đổi tùy chọn!",
        examTimeout: "⏰ Đã hết giờ làm bài! Bài thi sẽ tự động được nộp.",
        examPassed: "🎉 Xin chúc mừng! Bạn đã vượt qua bài thi thử này thành công!",
        examFailed: "💪 Chưa đạt tiêu chuẩn, hãy tiếp tục cố gắng nhé!",
        examDbNotFound: "⚠️ Không tìm thấy cơ sở dữ liệu câu hỏi cho môn học này!",
        practicalExamStarted: "📝 Bắt đầu bài thi thử thực hành! Vui lòng viết mã theo yêu cầu.",
        sqlSyntaxError: "❌ Lỗi cú pháp SQL. Xác minh không đạt!",
        sqlOutputMismatch: "❌ Kết quả thực thi khác với kết quả mong đợi. Vui lòng kiểm tra lại thứ tự, bộ lọc hoặc số cột!",
        compileError: "❌ Lỗi biên dịch. Xác minh không đạt!",
        runtimeError: "❌ Lỗi thực thi. Xác minh không đạt!",
        outputMismatch: "❌ Nội dung đầu ra khác biệt. Xác minh không đạt!",
        practicalExamSubmitted: "🎉 Bài thi thực hành đã nộp thành công! Báo cáo kết quả chi tiết đã được tạo."
      },
      dialog: {
        resetDbConfirm: "Cơ sở dữ liệu mô phỏng sẽ được đặt lại. Tiếp tục?",
        exitExamConfirm: "Thoát khỏi bài thi hiện tại? Tiến độ sẽ không được lưu.",
        switchExamConfirm: "Hiện có một bài thi đang chạy. Chuyển môn sẽ làm gián đoạn bài thi hiện tại. Tiếp tục?",
        clearCodeConfirm: "Xóa mã hiện tại?",
        resetProgressConfirm: "Bạn có chắc chắn muốn đặt lại toàn bộ tiến độ học tập môn {subject} không?",
        submitExamConfirm: "Bạn có chắc chắn muốn nộp bài không?",
        submitExamUnansweredConfirm: "Vẫn còn {count} câu hỏi chưa trả lời. Bạn có chắc chắn muốn nộp bài bây giờ không?",
        submitCodingExamUnansweredConfirm: "Vẫn còn {count} câu hỏi chưa trả lời hoặc chưa đạt. Bạn có chắc chắn muốn nộp bài bây giờ không?"
      },
      tools: {
        quickStart: "Bắt đầu nhanh",
        quickStartTitle: "Quy trình học tập",
        quickStartDesc: "Chọn môn → Học bài → Luyện tập → Xem thống kê",
        tools: "Công cụ",
        openMenu: "Mở menu công cụ",
        closeMenu: "Đóng menu công cụ",
        learningTools: "Công cụ học tập",
        glossary: "Thuật ngữ",
        glossaryDesc: "Tra cứu thuật ngữ IT / SQL / lập trình",
        settings: "Cài đặt",
        settingsDesc: "Điều chỉnh tùy chọn công cụ học tập",
        dangerous: "Thao tác nguy hiểm",
        resetProgress: "Đặt lại tiến trình",
        resetProgressDesc: "Xóa tiến trình học hiện tại. Cần xác nhận",
        confirmResetTitle: "Đặt lại tiến trình?",
        confirmResetBody: "Thao tác này sẽ xóa tiến trình học hiện tại. Tiếp tục?",
        cancel: "Hủy",
        confirmReset: "Xác nhận đặt lại",
        examTools: "Công cụ thi",
        examHistory: "Lịch sử thi",
        examHistoryDesc: "Xem kết quả thi thử gần đây",
        dashboard: "Thống kê học tập",
        dashboardDesc: "Xem tiến trình học và tổng quan dữ liệu",
        exportData: "Xuất dữ liệu học tập",
        exportDataDesc: "Tải bản sao lưu dữ liệu học cục bộ (JSON)",
        continueLearning: "Tiếp tục học",
        continueLearningDesc: "Tiếp tục từ lần học trước"
      },
      examHistory: {
        title: "Lịch sử thi gần đây",
        subtitle: "Xem kết quả thi thử đã lưu",
        close: "Đóng",
        filterAll: "Tất cả",
        empty: "Chưa có lịch sử thi",
        emptyHint: "Hoàn thành bài thi thử để xem kết quả tại đây",
        questions: "Số câu",
        correct: "Đúng",
        accuracy: "Tỉ lệ đúng",
        score: "Điểm",
        passed: "Đậu",
        failed: "Rớt",
        time: "Thời gian",
        viewDetails: "Xem chi tiết",
        hideDetails: "Ẩn chi tiết",
        examConfig: "Cấu hình thi",
        fieldScores: "Điểm theo lĩnh vực",
        questionResults: "Kết quả từng câu",
        yearFilter: "Lọc theo năm",
        submittedAt: "Thời gian nộp",
        yourAnswer: "Câu trả lời của bạn",
        correctAnswer: "Đáp án đúng",
        unanswered: "Chưa trả lời",
        questionNotFound: "Không tìm thấy dữ liệu câu hỏi"
      },
      wrongBook: {
        title: "Wrong Book",
        subtitle: "Review questions you answered incorrectly",
        emptyTitle: "No wrong answers yet",
        emptyDesc: "Incorrect answers will be collected here automatically",
        all: "All",
        unmastered: "Unmastered",
        mastered: "Mastered",
        markMastered: "Mark mastered",
        unmarkMastered: "Mark unmastered",
        remove: "Remove",
        wrongCount: "Wrong count",
        lastWrongAt: "Last wrong",
        yourAnswer: "Your wrong answer",
        correctAnswer: "Correct answer",
        explanation: "Explanation",
        source: "Source",
        confirmRemove: "Remove this question from the wrong book?",
        viewDetails: "View details",
        hideDetails: "Hide details",
        choices: "Choices",
        question: "Question",
        unknownAnswer: "Not recorded"
      },
      message: {
        inputSqlRequired: "Vui lòng nhập câu lệnh SQL."
      },
      settings: {
        language: "Ngôn ngữ"
      },
      lesson: {
        locateBookPage: "Định vị trang sách gốc",
        explanationJa: "Giải thích (Tiếng Nhật)",
        explanationZh: "Giải thích (Tiếng Trung)",
        compareMode: "So sánh song song (Nhật/Trung)",
        japaneseOnly: "Chỉ tiếng Nhật",
        chineseOnly: "Chỉ tiếng Trung",
        analogy: "Ví dụ thực tế",
        sqlExample: "Ví dụ cú pháp SQL",
        javaExample: "Ví dụ mã Java",
        pythonExample: "Ví dụ mã Python",
        glossary: "Thuật ngữ & Công thức cốt lõi",
        copyToEditor: "Sao chép vào trình biên tập",
        showPdf: "Hiển thị PDF gốc",
        translationZh: "Bản dịch tiếng Trung",
        checklist: "Danh sách kiểm tra",
        calculator: "Công cụ tính toán",
        calculatorSelect: "Chọn công cụ:"
      },
      flashcard: {
        title: "Thẻ từ vựng",
        flip: "Lật thẻ",
        clickToFlip: "Nhấp vào thẻ để lật xem nghĩa và cách đọc Nhật-Trung:",
        clickToFlipHint: "Nhấp để lật xem mặt sau",
        localizedDesc: "Thẻ thuật ngữ chính cho {title}."
      },
      glossary: {
        title: "Bảng thuật ngữ IT",
        open: "Thuật ngữ",
        close: "Đóng",
        searchPlaceholder: "Tìm thuật ngữ...",
        allCategories: "Tất cả",
        filterDatabase: "Cơ sở dữ liệu",
        filterSecurity: "Bảo mật",
        filterNetwork: "Mạng",
        filterProgramming: "Lập trình",
        filterSystem: "Hệ thống",
        category: "Danh mục",
        japanese: "Tiếng Nhật",
        english: "Tiếng Anh",
        currentLanguage: "Ngôn ngữ hiện tại",
        aliases: "Tên khác",
        relatedTerms: "Thuật ngữ liên quan",
        example: "Ví dụ",
        source: "Nguồn",
        needsReview: "⚠ Bản dịch cần xem lại",
        noResults: "Không tìm thấy thuật ngữ",
        keepEnglishNote: "Giữ nguyên thuật ngữ kỹ thuật tiếng Anh",
        resultCount: "{count} thuật ngữ"
      },
      quiz: {
        title: "Bài kiểm tra nhanh",
        questionCount: "Câu hỏi {current} / Tổng số {total}",
        previous: "Câu trước",
        next: "Câu tiếp",
        submitAnswer: "Nộp câu trả lời",
        empty: "Bài này chưa có câu hỏi kiểm tra.",
        unavailable: "Bài kiểm tra của {title} chưa được bản địa hóa cho ngôn ngữ hiển thị này.",
        thisLesson: "bài này"
      },
      typing: {
        library: "Kho tài liệu gõ chữ",
        libraryDesc: "Luyện gõ tiếng Nhật thực tế từ N3 đến N1. Tốc độ, công việc, văn bản dài tự chọn.",
        search: "Tìm kiếm tài liệu",
        searchPlaceholder: "Tiêu đề, nội dung hoặc thẻ",
        articleList: "Danh sách bài viết",
        randomArticle: "Ngẫu nhiên",
        restart: "Bắt đầu lại",
        timeLimit: "Giới hạn thời gian",
        imeMode: "Chế độ IME thực tế",
        speed: "Tốc độ",
        accuracy: "Độ chính xác",
        progress: "Tiến độ hiệu quả",
        edits: "Số lần sửa",
        time: "Thời gian",
        recentScores: "Kết quả gần đây",
        personalReview: "Luyện tập lại",
        reviewDesc: "Bài viết có độ chính xác dưới 98% sẽ tự động được thêm vào danh sách luyện tập lại.",
        reviewCount: "{count} bài chờ luyện lại",
        emptyReview: "Không có bài viết nào cần luyện lại",
        imePlaceholder: "Bật bộ gõ tiếng Nhật IME và nhập vào đây.",
        typingHelp: "Nhập bằng bộ gõ IME tiếng Nhật. Định dạng toàn sừng và bán sừng được chuẩn hóa; sửa đổi được tính vào số lần sửa.",
        resultTitle: "Hoàn thành luyện tập",
        continueRandom: "Tiếp tục luyện tập ngẫu nhiên",
        favoriteBtn: "Yêu thích",
        favoriteTitle: "Thêm bài viết này vào danh sách yêu thích",
        noLimit: "Không giới hạn",
        t1min: "1 phút",
        t3min: "3 phút",
        t5min: "5 phút",
        t10min: "10 phút"
      },
      console: {
        output: "Kết quả thực thi",
        rows: "{count} dòng",
        emptyResult: "Bảng kết quả truy vấn sẽ hiển thị ở đây sau khi thực thi.",
        emptyResultSub: "(Kết quả truy vấn SQL sẽ hiển thị ở đây)",
        maximize: "Phóng to",
        minimize: "Thu nhỏ"
      },
      auth: {
        localMode: "Chế độ ngoại tuyến",
        notLoggedIn: "Chưa đăng nhập",
        account: "Tài khoản",
        sync: "Đồng bộ",
        syncInDev: "Tính năng đồng bộ đang phát triển",
        continueLocal: "Tiếp tục sử dụng ngoại tuyến",
        mockSignIn: "Mô phỏng đăng nhập",
        mockSignOut: "Thoát mô phỏng",
        exportSnapshot: "Xuất dữ liệu ngoại tuyến",
        deviceId: "ID thiết bị",
        pendingSync: "Chờ đồng bộ",
        lastSync: "Lần đồng bộ cuối",
        noUpload: "Dữ liệu không được tải lên",
        close: "Đóng",
        signedIn: "Đã đăng nhập",
        signedOut: "Chưa đăng nhập",
        status: "Trạng thái",
        anonymous: "Ẩn danh",
        loginTitle: "Đăng nhập",
        registerTitle: "Tạo tài khoản",
        displayName: "Tên hiển thị",
        email: "Email",
        password: "Mật khẩu",
        confirmPassword: "Xác nhận mật khẩu",
        loginButton: "Đăng nhập",
        registerButton: "Tạo tài khoản",
        magicLinkOption: "Sử dụng liên kết email",
        usernameDisplayOnly: "Chỉ để hiển thị. Đăng nhập bằng email.",
        emailRequired: "Vui lòng nhập email",
        invalidEmail: "Vui lòng nhập địa chỉ email hợp lệ",
        credentialsRequired: "Vui lòng nhập email và mật khẩu",
        passwordRequired: "Vui lòng nhập mật khẩu",
        passwordTooShort: "Mật khẩu phải có ít nhất 6 ký tự",
        passwordMismatch: "Mật khẩu không khớp",
        usernameRequired: "Vui lòng nhập tên hiển thị",
        loginSuccess: "Đăng nhập thành công",
        loginFailed: "Đăng nhập thất bại",
        registerSuccess: "Tạo tài khoản thành công",
        registerFailed: "Đăng ký thất bại",
        confirmEmailNotice: "Tạo tài khoản thành công! Vui lòng kiểm tra email để xác nhận.",
        invalidCredentials: "Email hoặc mật khẩu không đúng",
        emailNotConfirmed: "Email chưa được xác nhận",
        userAlreadyExists: "Email này đã được đăng ký",
        tooManyRequests: "Quá nhiều yêu cầu. Vui lòng thử lại sau.",
        signOut: "Đăng xuất",
        sendMagicLink: "Gửi liên kết đăng nhập",
      }
    },

    "fr-FR": {
      home: {
        eyebrow: "Espace d'apprentissage",
        title: "Continuez sur votre lancée",
        currentContext: "Cours actuel",
        startLearning: "Commencer",
        continueLearning: "Reprendre l'apprentissage",
        reviewCourse: "Réviser depuis le début",
        courseDone: "Terminé",
        goContinue: "Continuer",
        emptyHint: "Aucun historique d'apprentissage. Commencez par la leçon 1 de SQL.",
        openDashboard: "Statistiques",
        progressOverview: "Progression des cours",
        quickActions: "Actions rapides",
        openGlossary: "Glossaire"
      },
      lessonNav: {
        title: "Navigation des leçons",
        prev: "Leçon précédente",
        next: "Leçon suivante",
        completed: "Leçon terminée",
        notCompleted: "Non terminée · validez l'exercice pour terminer"
      },
      common: {
        save: "Enregistrer",
        cancel: "Annuler",
        confirm: "Confirmer",
        delete: "Supprimer",
        reset: "Réinitialiser",
        resetProgress: "Réinitialiser la progression",
        loading: "Chargement...",
        themeToggle: "Basculer le mode clair/sombre",
        close: "Fermer",
        open: "Ouvrir",
        search: "Rechercher",
        random: "Aléatoire",
        restart: "Recommencer",
        copy: "Copier",
        maximize: "Agrandir",
        minimize: "Réduire",
        challenging: "En défi",
        lessonIndex: "Leçon {id}",
        changeQuestion: "Changer de question",
        stop: "Fin",
        subtitle: "Plateforme d'aide à l'apprentissage"
      },
      nav: {
        sql: "SQL",
        java: "Java",
        python: "Python",
        itpass: "IT Passport",
        sg: "Examen SG",
        typing: "Saisie japonaise",
        codingTyping: "Frappe de code",
        sqlChallenge: "Défi aléatoire"
      },
      moduleSwitch: {
        title: "Choisir une section d'apprentissage",
        close: "Fermer",
        triggerLabel: "Ouvrir le sélecteur de section"
      },
      moduleDesc: {
        sql: "Bases de données, requêtes, pratique et bac à sable",
        sg: "Préparation à l'examen de gestion de la sécurité",
        itpass: "IT Passport : bases et annales",
        typing: "Saisie japonaise et pratique",
        codingTyping: "Saisie de code et pratique de syntaxe",
        java: "Java : bases, syntaxe et pratique",
        python: "Python : bases, syntaxe et pratique"
      },
      codingTyping: {
        title: "Exercice de frappe de code",
        subtitle: "Maîtrisez la syntaxe en tapant du code. Choisissez un langage.",
        language: "Langage", category: "Catégorie", allCategories: "Tout",
        difficulty: "Difficulté", code: "Code cible", input: "Saisie",
        inputPlaceholder: "Tapez le code ici...", focus: "Points d'attention", hint: "Indice",
        speed: "CPM", accuracy: "Précision", progress: "Progrès",
        time: "Temps", errors: "Erreurs", complete: "Terminé !",
        result: "Résultat", totalTime: "Temps", errorChars: "Erreurs fréquentes",
        recentHistory: "Historique", noHistory: "Pas d'historique",
        next: "Suivant (Alt+N)", random: "Aléatoire (Alt+S)", restart: "Rec. (Alt+R)",
        continueRandom: "Continuer aléatoire",
        python: "Python", java: "Java", sql: "SQL", symbols: "Symboles",
        normalizeFullWidth: "Pleine→Demi-chasse", lenientIndent: "Indentation souple",
        sendToSandbox: "Send to Sandbox",        runInSandbox: "Run in Sandbox",        sandboxUnavailable: "Sandbox unavailable",        sentToSandbox: "Sent to sandbox",        openSandbox: "Open sandbox",        shortcutHelp: "Raccourcis: Alt+N Suivant / Alt+R Reco. / Alt+S Aléatoire",
        summaryLine: "CPM: {cpm} | Précision: {acc}% | Err: {err} | Temps: {time}",
        backToList: "Retour à la liste",
        exerciseList: "Liste des exercices"
      },
      subnav: {
        lessons: "Manuel & exercices",
        practicalExam: "Examen pratique simulé",
        study: "Leçons",
        pastExams: "Annales & examen blanc",
        javaIntro: "Introduction — Chapitres 1-8",
        javaPractice: "Pratique — Chapitres 1-11"
      },
      sidebar: {
        progress: "Progression",
        unitList: "Liste des unités"
      },
      sandbox: {
        run: "Exécuter",
        playground: "Bac à sable",
        runSql: "Exécuter SQL",
        resetDb: "Réinitialiser la DB",
        hint: "Indice",
        maximize: "Agrandir",
        output: "Résultat",
        sqlPlaceholder: "Veuillez saisir une requête SQL ici...",
        currentMission: "Mission actuelle",
        randomPractice: "Pratique aléatoire",
        databaseStructure: "Structure de la base de données",
        schoolDb: "DB École",
        bookstoreDb: "DB Librairie",
        javaSandboxTitle: "Sandbox de programmation Java",
        pythonSandboxTitle: "Sandbox de programmation Python",
        pythonSandboxDesc: "Python 3.12 · 10s · Sandbox hors ligne",
        sqlPlayground: "Playground SQL",
        chooseLessonLoadCode: "— Sélectionnez une leçon pour charger le code —",
        toolbarTemplate: "Modèle",
        toolbarTemplateTitle: "Charger le modèle de leçon",
        toolbarCopy: "Copier",
        toolbarCopyTitle: "Copier le code",
        toolbarClear: "Vider",
        toolbarClearTitle: "Effacer et réinitialiser",
        javaEditorPlaceholder: "Saisissez votre code Java ici...",
        pythonEditorPlaceholder: "Saisissez votre code Python ici...",
        stdinHeader: "Entrée standard (stdin)",
        stdinPlaceholder: "Données d'entrée pour le programme...",
        runCode: "Exécuter",
        readyStatus: "Prêt",
        outputPlaceholder: "(Les sorties du programme s'afficheront ici)"
      },
      exam: {
        verify: "Vérifier et enregistrer",
        submit: "Rendre l'examen",
        previous: "Précédent",
        next: "Suivant",
        exit: "Quitter l'examen",
        ruleTitle: "Past Question Dojo — Système d'examen blanc CBT",
        ruleDesc: "Simule l'environnement d'examen sur ordinateur (CBT) à l'aide des annales.",
        ruleDetailTitle: "Règles de l'examen :",
        ruleDetail1: "Les questions sont des requêtes de base de données ou des exercices de code.",
        ruleDetail2: "Vous pouvez écrire, exécuter et déboguer le code instantanément dans l'éditeur.",
        ruleDetail3: "Cliquez sur \"Vérifier et enregistrer\" pour valider votre code avec les tests.",
        ruleDetail4: "Les tâches réussies rapportent des points. Vous pouvez modifier le code à volonté.",
        ruleDetail5: "Soumettez votre copie avant la fin du temps ; la fin du temps valide l'envoi automatique.",
        selectYear: "Sélectionner l'année :",
        questionCount: "Nombre de questions :",
        launchBtn: "Démarrer l'examen",
        status: "Statut du test : ",
        notStarted: "Non tenté",
        cbtSimulator: "Simulateur CBT",
        remainingTime: "Temps restant : ",
        exitCbt: "Quitter l'examen",
        submitCbt: "Soumettre l'examen",
        questionNumber: "Question {current}",
        flagReview: "Marquer pour révision",
        metaExpand: "Infos question <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "Réduire <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "Source : ",
        metaYear: "Année",
        metaQNum: "N° question",
        metaCategory: "Catégorie",
        metaSubcategory: "Sous-catégorie",
        metaTopic: "Sujet",
        ruleTitleCoding: "Examen blanc pratique de programmation",
        ruleDescCoding: "Simule les examens de programmation réels. Écrivez le code dans le temps imparti.",
        launchBtnCoding: "Démarrer l'examen",
        cbtSimulatorCoding: "Examen pratique",
        expectedOutput: "Sortie attendue",
        resultsTitle: "Relevé de notes",
        resultsTitleCbt: "Rapport d'examen CBT",
        backToHome: "Retour à l'accueil",
        backToDojo: "Retour au Dojo",
        passLabel: "Verdict",
        passStatus: "Admis",
        failStatus: "Échoué",
        correctCountText: "{count} / {total} réussies",
        totalScoreText: "{score} / 1000 pts",
        correctRate: "Taux de réussite",
        timeSpent: "Temps écoulé :",
        detailsTitle: "Détails d'évaluation des questions",
        reviewTitle: "Examen des questions & Analyse d'erreurs",
        reviewTip: "Cliquez sur n'importe quel élément pour afficher les détails.",
        reviewCollapse: "Masquer l'explication",
        reviewExplanation: "Explication",
        resultCorrect: "Correct",
        resultIncorrect: "Incorrect",
        tableQNo: "N°",
        tableTitle: "Sujet / Titre",
        tableField: "Domaine",
        tableDifficulty: "Difficulté",
        tableResult: "Résultat",
        tableYourAnswer: "Votre réponse",
        tableCorrectAnswer: "Bonne réponse",
        tableAction: "Action",
        tableActionDetail: "Voir",
        personalizeTitle: "Options personnalisées :",
        optShuffle: "Mélanger l'ordre des choix (ア〜エ)",
        optNocalc: "Filtrer les calculs (ne pas afficher les questions avec formules)",
        all: "Mélange aléatoire d'annales (Recommandé)",
        r8: "Annales Reiwa 8 (R8)",
        r7: "Annales Reiwa 7 (R7)",
        r6: "Annales Reiwa 6 (R6)",
        r5: "Annales Reiwa 5 (R5)",
        r4: "Annales Reiwa 4 (R4)",
        r3: "Annales Reiwa 3 (R3)",
        cbtQ100: "100 questions (Standard CBT - 120 min)",
        cbtQ50: "50 questions (Examen abrégé - 60 min)",
        cbtQ30: "30 questions (Révision rapide - 36 min)",
        cbtQ10: "10 questions (Mini quiz - 12 min)",
        q5: "5 questions (Test rapide - 15 min)",
        q10: "10 questions (Standard - 30 min)",
        q15: "15 questions (Défi complet - 45 min)",
        gridTitle: "Feuille de réponses",
        unanswered: "Non répondu",
        answered: "Répondu",
        flagged: "Marqué",
        totalScoreLabel: "Note globale",
        domainStrat: "Domaine Stratégie",
        domainMan: "Domaine Management",
        domainTech: "Domaine Technologie",
        domainSubb: "Épreuve B"
      },
      toast: {
        alreadyFirst: "C'est la première question.",
        alreadyLast: "C'est la dernière question.",
        saved: "Enregistré.",
        verifySuccess: "Réussi. Votre réponse a été enregistrée.",
        dbReset: "La base de données de simulation a été réinitialisée.",
        examStarted: "L'examen blanc CBT a commencé.",
        lessonQuizPassed: "🏆 Félicitations ! Vous avez réussi tous les quiz pour {title}, et cette leçon est marquée comme Terminée !",
        dbSwitched: "🗄️ Base de données changée pour <{dbName}>. Vous pouvez interroger les tables : {tables}.",
        sqlHint: "💡 Indice : Référez-vous à la syntaxe SQL suivante :<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ Aucune leçon terminée pour le moment ! Veuillez d'abord en terminer au moins une.",
        challengeStarted: "🔥 Défi aléatoire commencé ! Un total de {count} questions issues de {lessonCount} leçons terminées.",
        challengeEnded: "✅ Défi aléatoire terminé.",
        challengeQuestionCorrect: "🎉 Correct ! Question de la leçon {sourceId} réussie !",
        challengeSuccess: "Défi réussi !",
        examEmpty: "⚠️ Aucune question trouvée avec ces critères, veuillez modifier les options !",
        examTimeout: "⏰ Le temps est écoulé ! L'examen est soumis automatiquement.",
        examPassed: "🎉 Félicitations ! Vous avez réussi cet examen blanc !",
        examFailed: "💪 Vous n'avez pas atteint la note minimale, continuez vos efforts !",
        examDbNotFound: "⚠️ Base de données de questions introuvable pour cette matière !",
        practicalExamStarted: "📝 L'examen blanc pratique a commencé ! Veuillez compléter le code selon les consignes.",
        sqlSyntaxError: "❌ Erreur de syntaxe SQL. Évaluation échouée !",
        sqlOutputMismatch: "❌ Le résultat de la requête ne correspond pas à la sortie attendue. Veuillez vérifier l'ordre, les filtres ou les colonnes !",
        compileError: "❌ Erreur de compilation. Évaluation échouée !",
        runtimeError: "❌ Erreur d'exécution. Évaluation échouée !",
        outputMismatch: "❌ Différence dans les sorties. Évaluation échouée !",
        practicalExamSubmitted: "🎉 Examen pratique soumis avec succès ! Relevé de notes détaillé généré."
      },
      dialog: {
        resetDbConfirm: "La base de données de simulation sera réinitialisée. Continuer ?",
        exitExamConfirm: "Quitter l'examen en cours ? La progression ne sera pas enregistrée.",
        switchExamConfirm: "Un examen est en cours. Changer de matière l'interrompra. Continuer ?",
        clearCodeConfirm: "Effacer le code ?",
        resetProgressConfirm: "Voulez-vous vraiment réinitialiser toute la progression pour {subject} ?",
        submitExamConfirm: "Voulez-vous vraiment rendre votre copie ?",
        submitExamUnansweredConfirm: "Il reste {count} questions sans réponse. Voulez-vous vraiment soumettre maintenant ?",
        submitCodingExamUnansweredConfirm: "Il reste {count} questions sans réponse ou échouées. Voulez-vous vraiment soumettre maintenant ?"
      },
      tools: {
        quickStart: "Démarrage rapide",
        quickStartTitle: "Flux d'apprentissage",
        quickStartDesc: "Sélectionner une matière → Étudier les leçons → Pratiquer → Consulter les statistiques",
        tools: "Outils",
        openMenu: "Ouvrir le menu des outils",
        closeMenu: "Fermer le menu des outils",
        learningTools: "Outils d'apprentissage",
        glossary: "Glossaire",
        glossaryDesc: "Rechercher des termes IT / SQL / programmation",
        settings: "Paramètres",
        settingsDesc: "Ajuster les préférences des outils d'apprentissage",
        dangerous: "Actions dangereuses",
        resetProgress: "Réinitialiser la progression",
        resetProgressDesc: "Effacer la progression actuelle. Confirmation requise",
        confirmResetTitle: "Réinitialiser la progression ?",
        confirmResetBody: "Cette action effacera votre progression actuelle. Continuer ?",
        cancel: "Annuler",
        confirmReset: "Confirmer la réinitialisation",
        examTools: "Outils d'examen",
        examHistory: "Historique d'examens",
        examHistoryDesc: "Voir les résultats d'examens récents",
        dashboard: "Statistiques d'apprentissage",
        dashboardDesc: "Voir la progression et l'aperçu des données",
        exportData: "Exporter les données d'apprentissage",
        exportDataDesc: "Télécharger une sauvegarde locale des données (JSON)",
        continueLearning: "Continuer l'apprentissage",
        continueLearningDesc: "Reprendre depuis la dernière progression"
      },
      examHistory: {
        title: "Historique récent",
        subtitle: "Voir les résultats d'examens simulés enregistrés",
        close: "Fermer",
        filterAll: "Tout",
        empty: "Aucun historique d'examen",
        emptyHint: "Terminez un examen simulé pour voir les résultats ici",
        questions: "Questions",
        correct: "Correct",
        accuracy: "Précision",
        score: "Score",
        passed: "Réussi",
        failed: "Échoué",
        time: "Durée",
        viewDetails: "Voir les détails",
        hideDetails: "Masquer les détails",
        examConfig: "Configuration",
        fieldScores: "Scores par domaine",
        questionResults: "Résultats par question",
        yearFilter: "Filtre par année",
        submittedAt: "Soumis le",
        yourAnswer: "Votre réponse",
        correctAnswer: "Bonne réponse",
        unanswered: "Sans réponse",
        questionNotFound: "Données de question introuvables"
      },
      wrongBook: {
        title: "Wrong Book",
        subtitle: "Review questions you answered incorrectly",
        emptyTitle: "No wrong answers yet",
        emptyDesc: "Incorrect answers will be collected here automatically",
        all: "All",
        unmastered: "Unmastered",
        mastered: "Mastered",
        markMastered: "Mark mastered",
        unmarkMastered: "Mark unmastered",
        remove: "Remove",
        wrongCount: "Wrong count",
        lastWrongAt: "Last wrong",
        yourAnswer: "Your wrong answer",
        correctAnswer: "Correct answer",
        explanation: "Explanation",
        source: "Source",
        confirmRemove: "Remove this question from the wrong book?",
        viewDetails: "View details",
        hideDetails: "Hide details",
        choices: "Choices",
        question: "Question",
        unknownAnswer: "Not recorded"
      },
      message: {
        inputSqlRequired: "Vui lòng nhập câu lệnh SQL."
      },
      settings: {
        language: "Langue"
      },
      lesson: {
        locateBookPage: "Aller à la page d'origine",
        explanationJa: "Explication (Japonais)",
        explanationZh: "Explication (Chinois)",
        compareMode: "Comparaison côte à côte (Ja/Zh)",
        japaneseOnly: "Japonais uniquement",
        chineseOnly: "Chinois uniquement",
        analogy: "Analogie simple",
        sqlExample: "Exemple de syntaxe SQL",
        javaExample: "Exemple de code Java",
        pythonExample: "Exemple de code Python",
        glossary: "Termes clés & Formules",
        copyToEditor: "Copier dans l'éditeur",
        showPdf: "Afficher le PDF d'origine",
        translationZh: "Traduction en chinois",
        checklist: "Liste de contrôle",
        calculator: "Outils de calcul",
        calculatorSelect: "Sélectionnez un outil:"
      },
      flashcard: {
        title: "Cartes mémoire",
        flip: "Retourner",
        clickToFlip: "Cliquez sur la carte pour la retourner et voir la traduction et définition :",
        clickToFlipHint: "Cliquez pour retourner",
        localizedDesc: "Carte des termes clés pour {title}."
      },
      glossary: {
        title: "Glossaire IT",
        open: "Glossaire",
        close: "Fermer",
        searchPlaceholder: "Rechercher des termes...",
        allCategories: "Tout",
        filterDatabase: "Base de données",
        filterSecurity: "Sécurité",
        filterNetwork: "Réseau",
        filterProgramming: "Programmation",
        filterSystem: "Système",
        category: "Catégorie",
        japanese: "Japonais",
        english: "Anglais",
        currentLanguage: "Langue actuelle",
        aliases: "Alias",
        relatedTerms: "Termes associés",
        example: "Exemple",
        source: "Source",
        needsReview: "⚠ Traduction à vérifier",
        noResults: "Aucun terme trouvé",
        keepEnglishNote: "Les termes techniques anglais sont conservés",
        resultCount: "{count} termes"
      },
      quiz: {
        title: "Quiz de compréhension",
        questionCount: "Question {current} / {total}",
        previous: "Précédent",
        next: "Suivant",
        submitAnswer: "Soumettre la réponse",
        empty: "Aucun quiz n'est disponible pour cette leçon.",
        unavailable: "Le quiz de {title} n'est pas encore localisé pour cette langue d'affichage.",
        thisLesson: "cette leçon"
      },
      typing: {
        library: "Bibliothèque de dactylographie",
        libraryDesc: "Pratique de la saisie en japonais du niveau N3 au N1. Travail, vie quotidienne, textes longs.",
        search: "Rechercher un texte",
        searchPlaceholder: "Titre, contenu ou mot-clé",
        articleList: "Liste des articles",
        randomArticle: "Aléatoire",
        restart: "Recommencer",
        timeLimit: "Limite de temps",
        imeMode: "Mode IME réel",
        speed: "Vitesse",
        accuracy: "Précision",
        progress: "Progression effective",
        edits: "Corrections",
        time: "Temps",
        recentScores: "Scores récents",
        personalReview: "Révisions personnelles",
        reviewDesc: "Les textes avec une précision inférieure à 98 % sont automatiquement ajoutés aux révisions.",
        reviewCount: "{count} textes à réviser",
        emptyReview: "Aucun texte à réviser",
        imePlaceholder: "Activez votre méthode de saisie japonaise (IME) et écrivez ici.",
        typingHelp: "Saisie système avec IME japonais. Les caractères pleine chasse et demi-chasse sont unifiés ; les retours arrière comptent comme corrections.",
        resultTitle: "Entraînement terminé",
        continueRandom: "Continuer l'entraînement aléatoire",
        favoriteBtn: "Favoris",
        favoriteTitle: "Ajouter cet article aux favoris",
        noLimit: "Pas de limite",
        t1min: "1 minute",
        t3min: "3 minutes",
        t5min: "5 minutes",
        t10min: "10 minutes"
      },
      console: {
        output: "Résultat d'exécution",
        rows: "{count} lignes",
        emptyResult: "Exécutez une requête pour afficher le tableau des résultats ici.",
        emptyResultSub: "(Les résultats de la requête SQL s'afficheront ici après exécution)",
        maximize: "Agrandir",
        minimize: "Réduire"
      },
      auth: {
        localMode: "Mode local",
        notLoggedIn: "Non connecté",
        account: "Compte",
        sync: "Synchronisation",
        syncInDev: "La synchronisation est en cours de développement",
        continueLocal: "Continuer en local",
        mockSignIn: "Simuler la connexion",
        mockSignOut: "Quitter la simulation",
        exportSnapshot: "Exporter les données locales",
        deviceId: "ID de l'appareil",
        pendingSync: "En attente de synchronisation",
        lastSync: "Dernière synchronisation",
        noUpload: "Aucune donnée n'est téléchargée",
        close: "Fermer",
        signedIn: "Connecté",
        signedOut: "Déconnecté",
        status: "Statut",
        anonymous: "Anonyme",
        loginTitle: "Connexion",
        registerTitle: "Créer un compte",
        displayName: "Nom d'affichage",
        email: "E-mail",
        password: "Mot de passe",
        confirmPassword: "Confirmer le mot de passe",
        loginButton: "Se connecter",
        registerButton: "Créer un compte",
        magicLinkOption: "Utiliser un lien magique",
        usernameDisplayOnly: "Pour l'affichage uniquement. Connectez-vous avec l'e-mail.",
        emailRequired: "Veuillez entrer votre e-mail",
        invalidEmail: "Veuillez entrer une adresse e-mail valide",
        credentialsRequired: "Veuillez entrer l'e-mail et le mot de passe",
        passwordRequired: "Veuillez entrer un mot de passe",
        passwordTooShort: "Le mot de passe doit contenir au moins 6 caractères",
        passwordMismatch: "Les mots de passe ne correspondent pas",
        usernameRequired: "Veuillez entrer un nom d'affichage",
        loginSuccess: "Connexion réussie",
        loginFailed: "Échec de la connexion",
        registerSuccess: "Compte créé avec succès",
        registerFailed: "Échec de l'inscription",
        confirmEmailNotice: "Compte créé ! Veuillez vérifier votre e-mail pour confirmer.",
        invalidCredentials: "E-mail ou mot de passe incorrect",
        emailNotConfirmed: "E-mail non confirmé",
        userAlreadyExists: "Cet e-mail est déjà enregistré",
        tooManyRequests: "Trop de requêtes. Veuillez réessayer plus tard.",
        signOut: "Se déconnecter",
        sendMagicLink: "Envoyer le lien magique",
      }
    },

    "ko-KR": {
      common: {
        save: "저장",
        cancel: "취소",
        confirm: "확인",
        delete: "삭제",
        reset: "초기화",
        resetProgress: "진행도 초기화",
        loading: "로딩 중...",
        themeToggle: "라이트/다크 모드 전환",
        close: "닫기",
        open: "열기",
        search: "검색",
        random: "랜덤",
        restart: "처음부터",
        copy: "복사",
        maximize: "최대화",
        minimize: "최소화",
        challenging: "도전 중",
        lessonIndex: "제{id}과",
        changeQuestion: "문제 변경",
        stop: "종료",
        subtitle: "학습 지원 플랫폼"
      },
      nav: {
        sql: "SQL 학습",
        java: "Java 학습",
        python: "Python 학습",
        itpass: "IT 패스포트 대비",
        sg: "SG 대비",
        typing: "일본어 타자",
        codingTyping: "코딩 타자",
        sqlChallenge: "랜덤 도전"
      },
      moduleSwitch: {
        title: "학습 섹션 선택",
        close: "닫기",
        triggerLabel: "섹션 선택 열기"
      },
      moduleDesc: {
        sql: "데이터베이스 기초, 쿼리, 연습, 샌드박스",
        sg: "정보보안 관리 시험 대비",
        itpass: "IT Passport 기초와 기출문제 연습",
        typing: "일본어 입력 및 타자 연습",
        codingTyping: "코드 입력 및 구문 연습",
        java: "Java 기초, 구문, 연습",
        python: "Python 기초, 구문, 연습"
      },
      codingTyping: {
        title: "코딩 타자 연습",
        subtitle: "타자 연습으로 프로그래밍 문법을 마스터하세요.",
        language: "언어 선택", category: "카테고리", allCategories: "전체",
        difficulty: "난이도", code: "목표 코드", input: "입력",
        inputPlaceholder: "코드를 입력하세요...", focus: "연습 포인트", hint: "힌트",
        speed: "CPM", accuracy: "정확도", progress: "진행률",
        time: "시간", errors: "오류", complete: "완료!",
        result: "결과", totalTime: "시간", errorChars: "자주 틀리는 문자",
        recentHistory: "최근 기록", noHistory: "기록 없음",
        next: "다음 (Alt+N)", random: "랜덤 (Alt+S)", restart: "다시 (Alt+R)",
        continueRandom: "랜덤 연습 계속",
        python: "Python", java: "Java", sql: "SQL", symbols: "기호",
        normalizeFullWidth: "전각→반각", lenientIndent: "들여쓰기 허용",
        sendToSandbox: "Sandbox로 보내기",        runInSandbox: "Sandbox에서 실행",        sandboxUnavailable: "Sandbox를 사용할 수 없음",        sentToSandbox: "Sandbox로 전송됨",        openSandbox: "Sandbox 열기",        shortcutHelp: "단축키: Alt+N 다음 / Alt+R 다시 / Alt+S 랜덤",
        summaryLine: "CPM: {cpm} | 정확도: {acc}% | 오류: {err} | 시간: {time}",
        backToList: "연습 목록으로 돌아가기",
        exerciseList: "연습 목록"
      },
      subnav: {
        lessons: "교재 및 실습 샌드박스",
        practicalExam: "실전 모의시험",
        study: "교재 학습",
        pastExams: "기출문제 & 모의시험",
        javaIntro: "입문편 (기초) — 제1~8장",
        javaPractice: "실천편 (응용) — 제1~11장"
      },
      sidebar: {
        progress: "진행 상황",
        unitList: "단원 목록"
      },
      sandbox: {
        run: "실행",
        playground: "샌드박스",
        runSql: "SQL 실행",
        resetDb: "DB 초기화",
        hint: "힌트",
        maximize: "최대화",
        output: "실행 결과",
        sqlPlaceholder: "SQL 문을 입력하세요...",
        currentMission: "현재 미션",
        randomPractice: "랜덤 지시 연습",
        databaseStructure: "데이터베이스 구조",
        schoolDb: "학교 DB",
        bookstoreDb: "서점 DB",
        javaSandboxTitle: "Java 프로그래밍 샌드박스",
        pythonSandboxTitle: "Python 프로그래밍 샌드박스",
        pythonSandboxDesc: "Python 3.12 · 10s · 오프라인 샌드박스",
        sqlPlayground: "SQL 플레이그라운드",
        chooseLessonLoadCode: "— 과목을 선택하여 코드 불러오기 —",
        toolbarTemplate: "템플릿",
        toolbarTemplateTitle: "현재 과목 템플릿 불러오기",
        toolbarCopy: "복사",
        toolbarCopyTitle: "코드 복사",
        toolbarClear: "초기화",
        toolbarClearTitle: "코드 지우기 및 초기화",
        javaEditorPlaceholder: "Java 코드를 입력하세요...",
        pythonEditorPlaceholder: "Python 코드를 입력하세요...",
        stdinHeader: "표준 입력 (stdin)",
        stdinPlaceholder: "프로그램에 전달할 입력 데이터...",
        runCode: "실행",
        readyStatus: "준비 완료",
        outputPlaceholder: "(실행 결과가 여기에 표시됩니다)"
      },
      exam: {
        verify: "확인 및 저장",
        submit: "시험 제출",
        previous: "이전 문제",
        next: "다음 문제",
        exit: "시험 중단",
        ruleTitle: "기출문제 도장 — CBT 모의시험 시스템",
        ruleDesc: "최신 IT 시험 기출문제를 기반으로 실제 CBT 환경을 시뮬레이션합니다.",
        ruleDetailTitle: "시험 규칙:",
        ruleDetail1: "문제는 모두 실전형으로, 일본어 태스크 또는 예상 출력 결과가 주어집니다.",
        ruleDetail2: "우측의 샌드박스 에디터에서 즉시 코드를 작성, 실행, 디버깅할 수 있습니다.",
        ruleDetail3: "코드 작성 후 \"확인 및 저장\"을 클릭하여 테스트 케이스 검증을 수행합니다.",
        ruleDetail4: "테스트 통과(Passed ✓) 시 점수가 부여되며, 통과할 때까지 여러 번 수정 가능합니다.",
        ruleDetail5: "제한 시간 내에 \"시험 제출\"을 클릭하세요. 시간 초과 시 자동 회수됩니다.",
        selectYear: "시험 연도 선택:",
        questionCount: "모의시험 문제 수:",
        launchBtn: "시험 시작",
        status: "테스트 상태: ",
        notStarted: "미응시",
        cbtSimulator: "CBT 모의 시스템",
        remainingTime: "남은 시간: ",
        exitCbt: "중단 종료",
        submitCbt: "시험 제출",
        questionNumber: "제{current}문",
        flagReview: "이 문제 체크",
        metaExpand: "문제 정보 <i class=\"fa-solid fa-chevron-down\"></i>",
        metaCollapse: "접기 <i class=\"fa-solid fa-chevron-up\"></i>",
        metaSource: "출처：",
        metaYear: "연도",
        metaQNum: "문제 번호",
        metaCategory: "분류",
        metaSubcategory: "하위 분류",
        metaTopic: "주제",
        ruleTitleCoding: "실전 프로그래밍 모의시험",
        ruleDescCoding: "IT 전문학교의 프로그래밍 시험 환경을 재현합니다. 제한 시간 안에 코드를 작성합니다.",
        launchBtnCoding: "시험 시작",
        cbtSimulatorCoding: "실전 모의시험",
        expectedOutput: "예상 출력 결과",
        resultsTitle: "평가 결과",
        resultsTitleCbt: "모의시험 성적서",
        backToHome: "홈으로 돌아가기",
        backToDojo: "도장으로 돌아가기",
        passLabel: "합격 판정",
        passStatus: "합격",
        failStatus: "불합격",
        correctCountText: "{count} / {total} 문제 통과",
        totalScoreText: "{score} / 1000 점",
        correctRate: "득점률",
        timeSpent: "소요 시간:",
        detailsTitle: "각 문제 검증 상세",
        reviewTitle: "문제 복기 및 오답 분석",
        reviewTip: "항목을 클릭하면 상세 내용을 펼쳐볼 수 있습니다.",
        reviewCollapse: "해설 닫기",
        reviewExplanation: "해설",
        resultCorrect: "정답",
        resultIncorrect: "오답",
        tableQNo: "문항",
        tableTitle: "평가 항목",
        tableField: "분야",
        tableDifficulty: "난이도",
        tableResult: "검증 결과",
        tableYourAnswer: "내 답안",
        tableCorrectAnswer: "정답",
        tableAction: "조작",
        tableActionDetail: "상세",
        personalizeTitle: "시험 개인화 옵션:",
        optShuffle: "선택지 순서 섞기 (ア〜エ)",
        optNocalc: "계산 문제 필터 (계산식 포함 문제 표시 안 함)",
        all: "기출문제 랜덤 혼합 (추천 - 종합 모의시험)",
        r8: "레이와 8년도 (R8) 공개 문제",
        r7: "레이와 7년도 (R7) 공개 문제",
        r6: "레이와 6년도 (R6) 공개 문제",
        r5: "레이와 5년도 (R5) 공개 문제",
        r4: "레이와 4년도 (R4) 공개 문제",
        r3: "레이와 3년도 (R3) 공개 문제",
        cbtQ100: "100문 (정식 CBT 기준 - 제한시간 120분)",
        cbtQ50: "50문 (단축 모의시험 - 제한시간 60분)",
        cbtQ30: "30문 (퀵 연습 - 제한시간 36분)",
        cbtQ10: "10문 (미니 테스트 - 제한시간 12분)",
        q5: "5문 (퀵 테스트 - 15분)",
        q10: "10문 (표준 실전 - 30분)",
        q15: "15문 (풀 챌린지 - 45분)",
        gridTitle: "답안지 현황",
        unanswered: "미답",
        answered: "답함",
        flagged: "체크",
        totalScoreLabel: "총점",
        domainStrat: "전략 영역",
        domainMan: "관리 영역",
        domainTech: "기술 영역",
        domainSubb: "과목 B"
      },
      toast: {
        alreadyFirst: "첫 번째 문제입니다.",
        alreadyLast: "마지막 문제입니다.",
        saved: "저장했습니다.",
        verifySuccess: "판정 통과! 답안을 저장했습니다.",
        dbReset: "모의 데이터베이스를 초기 상태로 되돌렸습니다.",
        examStarted: "CBT 모의시험을 시작했습니다.",
        lessonQuizPassed: "🏆 {title} — 퀴즈 합격! 과목 완료로 표시되었습니다!",
        dbSwitched: "🗄️ 《{dbName}》데이터베이스로 전환했습니다. 현재 {tables} 등의 테이블을 조회할 수 있습니다.",
        sqlHint: "💡 힌트: 다음 SQL 구문을 참고하세요:<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
        noCompletedLessons: "⚠️ 완료된 과목이 없습니다! 먼저 최소 1개의 과목을 학습하고 완료하세요.",
        challengeStarted: "🔥 종합 랜덤 도전 시작! 완료한 {lessonCount}개 과목에서 총 {count}문항이 출제됩니다.",
        challengeEnded: "✅ 종합 랜덤 도전이 종료되었습니다.",
        challengeQuestionCorrect: "🎉 정답입니다! {sourceId}과의 문제를 통과했습니다!",
        challengeSuccess: "종합 랜덤 도전 달성!",
        examEmpty: "⚠️ 조건에 맞는 기출문제가 없습니다. 옵션을 변경해 주세요!",
        examTimeout: "⏰ 제한 시간이 되었습니다. 자동으로 답안을 제출합니다!",
        examPassed: "🎉 축하합니다! 이번 모의 테스트에 합격했습니다!",
        examFailed: "💪 합격 기준에 도달하지 못했습니다. 계속 노력하세요!",
        examDbNotFound: "⚠️ 이 과목의 문제 데이터베이스를 찾을 수 없습니다!",
        practicalExamStarted: "📝 실전 모의시험이 시작되었습니다! 태스크에 따라 코드를 작성하세요.",
        sqlSyntaxError: "❌ SQL 구문 오류. 테스트 불합격!",
        sqlOutputMismatch: "❌ 실행 결과가 예상 결과와 다릅니다. 정렬, 필터링 또는 열 수를 확인하세요!",
        compileError: "❌ 컴파일 오류. 판정 실패!",
        runtimeError: "❌ 실행 오류. 판정 실패!",
        outputMismatch: "❌ 출력 결과가 일치하지 않습니다. 판정 실패!",
        practicalExamSubmitted: "🎉 실기 시험 제출 성공! 성적표와 평가 결과가 생성되었습니다."
      },
      dialog: {
        resetDbConfirm: "모의 데이터베이스를 초기 상태로 되돌립니다. 계속하시겠습니까?",
        exitExamConfirm: "현재 시험을 중단하고 종료하시겠습니까? 진행 상황은 저장되지 않습니다.",
        switchExamConfirm: "현재 실행 중인 시험이 있습니다. 과목을 전환하면 현재 시험이 중단됩니다. 전환하시겠습니까?",
        clearCodeConfirm: "코드를 지우시겠습니까?",
        resetProgressConfirm: "모든 {subject} 학습 진행도를 초기화하시겠습니까?",
        submitExamConfirm: "시험을 종료하고 제출하시겠습니까?",
        submitExamUnansweredConfirm: "미답 문제가 {count}문 있습니다. 정말 지금 제출하시겠습니까?",
        submitCodingExamUnansweredConfirm: "미답 또는 판정 미통과 문제가 {count}문 있습니다. 정말 지금 제출하시겠습니까?"
      },
      tools: {
        quickStart: "빠른 시작",
        quickStartTitle: "학습 흐름",
        quickStartDesc: "과목 선택 → 학습하기 → 연습하기 → 통계 확인",
        tools: "도구",
        openMenu: "도구 메뉴 열기",
        closeMenu: "도구 메뉴 닫기",
        learningTools: "학습 도구",
        glossary: "용어집",
        glossaryDesc: "IT / SQL / 프로그래밍 용어 검색",
        settings: "설정",
        settingsDesc: "학습 도구 환경 설정 조정",
        dangerous: "위험 작업",
        resetProgress: "진행도 초기화",
        resetProgressDesc: "현재 학습 진행도를 지웁니다. 확인 필요",
        confirmResetTitle: "진행도를 초기화하시겠습니까?",
        confirmResetBody: "현재 학습 진행도가 지워집니다. 계속하시겠습니까?",
        cancel: "취소",
        confirmReset: "초기화 확인",
        examTools: "시험 도구",
        examHistory: "시험 기록",
        examHistoryDesc: "최근 모의고사 기록 보기",
        dashboard: "학습 통계",
        dashboardDesc: "학습 진행 상황 및 데이터 개요 보기",
        exportData: "학습 데이터 내보내기",
        exportDataDesc: "로컬 학습 데이터 백업 다운로드 (JSON)",
        continueLearning: "학습 계속",
        continueLearningDesc: "이전 진도에서 계속",
        exportSuccess: "내보내기 성공",
        exportPartialSuccess: "일부 손상된 데이터를 건너뛰고 내보냈습니다",
        exportFailed: "내보내기 실패",
        exportScope: "비밀번호나 키 미포함 — 학습 데이터만",
        importPreview: "백업 가져오기 미리보기",
        importPreviewDesc: "읽기 전용 미리보기 — 데이터가 수정되지 않음",
        importSelectFile: "백업 파일 선택",
        importParseFailed: "파일 구문 분석 실패",
        importSensitiveBlocked: "민감한 필드가 감지되어 차단됨",
        importImportable: "가져오기 가능한 데이터",
        importNew: "신규",
        importConflict: "충돌",
        importSkipped: "건너뜀",
        importIncompatibleVersion: "호환되지 않는 백업 버전",
        importFileTooLarge: "파일이 너무 큼",
        importClosePreview: "미리보기 닫기",
        importSummary: "가져오기 요약",
        importNoData: "가져오기 가능한 데이터 없음",
        importSection: "섹션",
        importSectionLessons: "완료된 레슨",
        importSectionQuiz: "퀴즈 결과",
        importSectionWrongBook: "오답 노트",
        importSectionBookmarks: "북마크",
        importSectionTyping: "타이핑 기록",
        importSectionExam: "시험 기록",
        importSectionGoals: "학습 목표",
        importSectionSettings: "언어 및 테마",
        importSectionSync: "동기화 상태",
        importSyncBlocked: "가져오기 불가 — 동기화 상태는 디바이스에서 관리"
      },
      dashboard: {
        dashboard: "학습 통계",
        overview: "개요",
        completedLessons: "완료한 학습",
        wrongBook: "오답",
        bookmarks: "즐겨찾기",
        typingPractice: "타이핑 연습",
        examHistory: "모의고사",
        quizCompleted: "퀴즈 완료",
        learningProgress: "학습 진행도",
        wrongBookAndBookmarks: "오답 및 즐겨찾기",
        wrongBookCount: "오답 수",
        bookmarkCount: "즐겨찾기 수",
        latestAdded: "최근 추가",
        typingArticles: "타이핑 글",
        japaneseFavorites: "일본어 즐겨찾기",
        quizAndExam: "퀴즈 및 시험",
        examCount: "시험 횟수",
        avgAccuracy: "평균 정답률",
        latestExam: "최근 시험",
        japaneseTyping: "일본어 타이핑",
        codingTyping: "코딩 타이핑",
        bestCpm: "최고 CPM",
        syncStatus: "동기화 상태",
        loginStatus: "로그인 상태",
        loggedIn: "로그인됨",
        notLoggedIn: "로그인 안 됨",
        lastSync: "마지막 동기화",
        lastSyncResult: "동기화 결과",
        syncSuccess: "동기화 성공",
        syncFailed: "동기화 실패",
        collectedAt: "데이터 수집 시간",
        refreshStats: "통계 새로고침",
        noData: "데이터 없음",
        loadError: "통계 데이터 로드 실패",
        infoSec: "정보 보안",
        weeklyTrend: "주간 학습 추세",
        last7Days: "최근 7일",
        noTrendData: "추세 데이터 없음",
        learningGoals: "학습 목표",
        goalActivities: "주간 학습 활동",
        goalTyping: "주간 타이핑",
        goalExams: "주간 모의고사",
        saveGoals: "목표 저장",
        resetGoals: "목표 초기화",
        goalsSaved: "목표가 저장되었습니다",
        goalsReset: "목표가 초기화되었습니다",
        noActivity: "활동 없음",
        examsShort: "시험",
        jpTypingShort: "일본어",
        ctTypingShort: "코딩",
        wbShort: "오답",
        thisWeek: "이번 주",
        today: "오늘",
        dataIncompleteHint: "데이터가 완전하지 않을 수 있습니다. 먼저 동기화하는 것을 권장합니다.",
        syncAndRefresh: "동기화 및 새로고침",
        syncing: "동기화 중…",
        syncComplete: "동기화 완료",
        syncFailedHint: "동기화 실패",
        loginToSyncHint: "로그인하여 여러 기기에서 데이터를 동기화하세요"
      },
      examHistory: {
        title: "최근 시험 기록",
        subtitle: "저장된 모의고사 결과 보기",
        close: "닫기",
        filterAll: "전체",
        empty: "시험 기록이 없습니다",
        emptyHint: "모의고사를 완료하면 여기에 기록이 표시됩니다",
        questions: "문제 수",
        correct: "정답",
        accuracy: "정답률",
        score: "점수",
        passed: "합격",
        failed: "불합격",
        time: "소요 시간",
        viewDetails: "상세 보기",
        hideDetails: "상세 숨기기",
        examConfig: "시험 설정",
        fieldScores: "분야별 점수",
        questionResults: "문제별 결과",
        yearFilter: "연도 필터",
        submittedAt: "제출 일시",
        yourAnswer: "내 답안",
        correctAnswer: "정답",
        unanswered: "미답변",
        questionNotFound: "문제 데이터를 찾을 수 없습니다"
      },
      wrongBook: {
        title: "오답 노트",
        subtitle: "틀린 문제 복습",
        emptyTitle: "아직 오답이 없습니다",
        emptyDesc: "틀린 문제는 자동으로 여기에 저장됩니다",
        all: "전체",
        unmastered: "미숙달",
        mastered: "숙달",
        markMastered: "숙달로 표시",
        unmarkMastered: "미숙달로 변경",
        remove: "삭제",
        wrongCount: "오답 횟수",
        lastWrongAt: "최근 오답",
        yourAnswer: "내 오답",
        correctAnswer: "정답",
        explanation: "해설",
        source: "출처",
        confirmRemove: "이 문제를 오답 노트에서 삭제할까요?",
        viewDetails: "상세 보기",
        hideDetails: "상세 숨기기",
        choices: "선택지",
        question: "문제",
        unknownAnswer: "기록 없음"
      },
      message: {
        inputSqlRequired: "SQL 쿼리를 입력해 주세요."
      },
      settings: {
        language: "언어"
      },
      lesson: {
        locateBookPage: "원서 페이지로 이동",
        explanationJa: "해설 (일본어)",
        explanationZh: "해설 (중국어)",
        compareMode: "좌우 대조 (일/중)",
        japaneseOnly: "일본어만",
        chineseOnly: "중국어만",
        analogy: "일상적인 비유",
        sqlExample: "SQL 구문 예",
        javaExample: "Java 코드 예",
        pythonExample: "Python 코드 예",
        glossary: "중요 용어 및 공식",
        copyToEditor: "에디터로 복사",
        showPdf: "원서 PDF 보기",
        translationZh: "중국어 번역",
        checklist: "요점 체크",
        calculator: "계산 도구",
        calculatorSelect: "도구 선택:"
      },
      flashcard: {
        title: "단어 카드",
        flip: "뒤집기",
        clickToFlip: "카드를 클릭하여 앞뒷면을 전환합니다:",
        clickToFlipHint: "클릭하여 뒷면 보기",
        localizedDesc: "{title}의 핵심 용어 카드입니다."
      },
      glossary: {
        title: "IT 용어집",
        open: "용어집",
        close: "닫기",
        searchPlaceholder: "용어 검색...",
        allCategories: "전체",
        filterDatabase: "데이터베이스",
        filterSecurity: "보안",
        filterNetwork: "네트워크",
        filterProgramming: "프로그래밍",
        filterSystem: "시스템",
        category: "카테고리",
        japanese: "일본어",
        english: "영어",
        currentLanguage: "현재 언어",
        aliases: "별칭",
        relatedTerms: "관련 용어",
        example: "코드 예",
        source: "출처",
        needsReview: "⚠ 번역 검토 중",
        noResults: "해당 용어가 없습니다",
        keepEnglishNote: "영어 기술 용어 유지",
        resultCount: "{count}개의 용어"
      },
      quiz: {
        title: "이해도 테스트",
        questionCount: "문제 {current} / 전체 {total}문",
        previous: "이전",
        next: "다음",
        submitAnswer: "답변 제출",
        empty: "이 단원에는 아직 퀴즈가 없습니다.",
        unavailable: "{title}의 퀴즈는 아직 이 표시 언어로 현지화되지 않았습니다.",
        thisLesson: "이 단원"
      },
      typing: {
        library: "타이핑 라이브러리",
        libraryDesc: "N3부터 N1까지 폭넓게 사용 가능한 실용 일본어 입력. 생활, 직장, 장문을 자신의 페이스로 연습할 수 있습니다.",
        search: "라이브러리 검색",
        searchPlaceholder: "제목, 내용 또는 태그",
        articleList: "기사 목록",
        randomArticle: "랜덤",
        restart: "처음부터",
        timeLimit: "제한 시간",
        imeMode: "IME 모드",
        speed: "속도",
        accuracy: "정확률",
        progress: "유효 진행",
        edits: "수정 횟수",
        time: "시간",
        recentScores: "최근 성적",
        personalReview: "개인 복습",
        reviewDesc: "정확률 98% 미만의 기사는 자동으로 복습 목록에 추가되며, 100% 달성 시 삭제됩니다.",
        reviewCount: "{count}건의 복습 대기",
        emptyReview: "복습 대기 기사가 없습니다",
        imePlaceholder: "일본어 IME를 켜고 여기에서 입력하세요.",
        typingHelp: "시스템 일본어 IME 입력. 전각/반각은 통일되어 비교됩니다. 백스페이스 등의 수정도 카운트됩니다.",
        resultTitle: "연습 완료",
        continueRandom: "랜덤 연습 계속",
        favoriteBtn: "즐겨찾기",
        favoriteTitle: "이 기사를 즐겨찾기에 추가",
        noLimit: "제한 없음",
        t1min: "1분",
        t3min: "3분",
        t5min: "5분",
        t10min: "10분"
      },
      console: {
        output: "실행 결과",
        rows: "{count}행",
        emptyResult: "쿼리를 실행하면 결과 테이블이 여기에 표시됩니다.",
        emptyResultSub: "(SQL 실행 후 결과가 여기에 표시됩니다)",
        maximize: "최대화",
        minimize: "최소화"
      },
      auth: {
        localMode: "로컬 모드",
        notLoggedIn: "로그인하지 않음",
        account: "계정",
        sync: "동기화",
        syncInDev: "동기화 기능은 개발 중입니다",
        continueLocal: "로컬 사용 계속",
        mockSignIn: "로그인 상태 시뮬레이션",
        mockSignOut: "시뮬레이션 종료",
        exportSnapshot: "로컬 스냅샷 내보내기",
        deviceId: "기기 ID",
        pendingSync: "동기화 대기",
        lastSync: "마지막 동기화",
        noUpload: "데이터가 업로드되지 않습니다",
        close: "닫기",
        signedIn: "로그인됨",
        signedOut: "로그아웃됨",
        status: "상태",
        anonymous: "익명",
        loginTitle: "로그인",
        registerTitle: "계정 만들기",
        displayName: "표시 이름",
        email: "이메일",
        password: "비밀번호",
        confirmPassword: "비밀번호 확인",
        loginButton: "로그인",
        registerButton: "계정 만들기",
        magicLinkOption: "이메일 링크로 로그인",
        usernameDisplayOnly: "표시 전용입니다. 이메일로 로그인하세요.",
        emailRequired: "이메일을 입력해 주세요",
        invalidEmail: "유효한 이메일 주소를 입력해 주세요",
        credentialsRequired: "이메일과 비밀번호를 입력해 주세요",
        passwordRequired: "비밀번호를 입력해 주세요",
        passwordTooShort: "비밀번호는 최소 6자 이상이어야 합니다",
        passwordMismatch: "비밀번호가 일치하지 않습니다",
        usernameRequired: "표시 이름을 입력해 주세요",
        loginSuccess: "로그인 성공",
        loginFailed: "로그인 실패",
        registerSuccess: "계정 생성 성공",
        registerFailed: "등록 실패",
        confirmEmailNotice: "계정이 생성되었습니다! 이메일을 확인하여 인증해 주세요.",
        invalidCredentials: "이메일 또는 비밀번호가 올바르지 않습니다",
        emailNotConfirmed: "이메일이 아직 인증되지 않았습니다",
        userAlreadyExists: "이 이메일은 이미 등록되어 있습니다",
        tooManyRequests: "요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.",
        signOut: "로그아웃",
        sendMagicLink: "로그인 링크 보내기",
      }
    },
    "th-TH": {
      common: {
        save: "บันทึก", cancel: "ยกเลิก", confirm: "ตกลง", delete: "ลบ",
        reset: "รีเซ็ต", resetProgress: "รีเซ็ตความคืบหน้า", loading: "กำลังโหลด...",
        themeToggle: "สลับโหมดสว่าง/มืด", close: "ปิด", open: "เปิด", search: "ค้นหา",
        random: "สุ่ม", restart: "เริ่มใหม่", copy: "คัดลอก", maximize: "ขยาย",
        minimize: "ย่อ", challenging: "กำลังท้าทาย", lessonIndex: "บทที่ {id}",
        changeQuestion: "เปลี่ยนคำถาม", stop: "หยุด", subtitle: "แพลตฟอร์มสนับสนุนการเรียน"
      },
      nav: {
        sql: "เรียน SQL", java: "เรียน Java", python: "เรียน Python",
        itpass: "เตรียมสอบ IT Passport", sg: "เตรียมสอบ SG",
        typing: "พิมพ์ภาษาญี่ปุ่น", codingTyping: "พิมพ์โค้ด", sqlChallenge: "สุ่มท้าทาย"
      },
      moduleSwitch: { title: "เลือกส่วนเรียน", close: "ปิด", triggerLabel: "เปิดเลือกส่วน" },
      moduleDesc: {
        sql: "พื้นฐาน Database, query, แบบฝึก, sandbox",
        sg: "เตรียมสอบ Information Security Management",
        itpass: "พื้นฐาน IT Passport และแบบฝึกข้อสอบ", typing: "ฝึกพิมพ์ภาษาญี่ปุ่น",
        codingTyping: "ฝึกพิมพ์และไวยากรณ์โค้ด", java: "พื้นฐาน Java, ไวยากรณ์, แบบฝึก",
        python: "พื้นฐาน Python, ไวยากรณ์, แบบฝึก"
      },
      codingTyping: {
        title: "ฝึกพิมพ์โค้ด", subtitle: "ฝึกพิมพ์เพื่อฝึกไวยากรณ์โปรแกรมมิ่ง",
        language: "เลือกภาษา", category: "หมวดหมู่", allCategories: "ทั้งหมด",
        difficulty: "ระดับความยาก", code: "โค้ดเป้าหมาย", input: "ป้อน",
        inputPlaceholder: "พิมพ์โค้ดที่นี่...", focus: "จุดฝึก", hint: "คำใบ้",
        speed: "CPM", accuracy: "ความแม่นยำ", progress: "ความคืบหน้า",
        time: "เวลา", errors: "ข้อผิดพลาด", complete: "เสร็จสิ้น!",
        result: "ผลลัพธ์", totalTime: "เวลา", errorChars: "อักขระที่ผิดบ่อย",
        recentHistory: "ประวัติล่าสุด", noHistory: "ยังไม่มีประวัติ",
        next: "ถัดไป (Alt+N)", random: "สุ่ม (Alt+S)", restart: "ใหม่ (Alt+R)",
        continueRandom: "ฝึกสุ่มต่อ", python: "Python", java: "Java", sql: "SQL", symbols: "สัญลักษณ์",
        normalizeFullWidth: "เต็ม→ครึ่ง", lenientIndent: "ย่อหน้ายืดหยุ่น",
        sendToSandbox: "ส่งไป Sandbox", runInSandbox: "รันใน Sandbox",
        sandboxUnavailable: "Sandbox ไม่พร้อม", sentToSandbox: "ส่งไป Sandbox แล้ว",
        openSandbox: "เปิด Sandbox",
        shortcutHelp: "ปุ่มลัด: Alt+N ถัดไป / Alt+R ใหม่ / Alt+S สุ่ม",
        summaryLine: "CPM: {cpm} | แม่นยำ: {acc}% | ผิดพลาด: {err} | เวลา: {time}",
        backToList: "กลับไปรายการฝึก",
        exerciseList: "รายการฝึก"
      },
      subnav: {
        lessons: "แบบเรียนและ Sandbox ฝึก", practicalExam: "จำลองสอบ",
        study: "เรียนแบบเรียน", pastExams: "ข้อสอบเก่า & จำลองสอบ",
        javaIntro: "บทนำ (พื้นฐาน) — บทที่ 1~8", javaPractice: "บทปฏิบัติ (ประยุกต์) — บทที่ 1~11"
      },
      sidebar: { progress: "ความคืบหน้า", unitList: "รายการบท" },
      sandbox: {
        run: "รัน", playground: "แซนด์บ็อกซ์", runSql: "รัน SQL", resetDb: "รีเซ็ต DB",
        hint: "คำใบ้", maximize: "ขยาย", output: "ผลลัพธ์",
        toolbarCopy: "คัดลอก",
        toolbarCopyTitle: "คัดลอกโค้ด",
        toolbarClear: "ล้าง",
        toolbarClearTitle: "ล้างและรีเซ็ตโค้ด",
        sqlPlaceholder: "พิมพ์คำสั่ง SQL...", currentMission: "ภารกิจปัจจุบัน",
        randomPractice: "ฝึกสุ่ม", databaseStructure: "โครงสร้าง Database",
        schoolDb: "DB โรงเรียน", bookstoreDb: "DB ร้านหนังสือ",
        stdinPlaceholder: "ใส่ข้อมูล stdin สำหรับโปรแกรม...",
        runCode: "รัน",
        sqlPlayground: "SQL Playground", readyStatus: "พร้อมแล้ว",
        outputPlaceholder: "(ผลลัพธ์จะแสดงที่นี่)"
      },
      toast: {
        verifySuccess: "ตรวจผ่านแล้ว บันทึกคำตอบเรียบร้อย",
        outputMismatch: "ผลลัพธ์ output ไม่ตรงกับที่คาดหวัง ตรวจไม่ผ่าน",
        runtimeError: "เกิดข้อผิดพลาดระหว่างรัน"
      },
      exam: {
        verify: "ตรวจและบันทึก", submit: "ส่งข้อสอบ", previous: "ข้อก่อนหน้า",
        next: "ข้อถัดไป", exit: "ออกจากการสอบ", launchBtn: "เริ่มสอบ",
        status: "สถานะ: ", notStarted: "ยังไม่ได้ทำ", remainingTime: "เวลาที่เหลือ: "
      },
      glossary: {
        title: "ศัพท์ IT", open: "เปิดศัพท์", close: "ปิด", searchPlaceholder: "ค้นหาศัพท์...",
        allCategories: "ทั้งหมด", category: "หมวดหมู่", noResults: "ไม่พบศัพท์ที่เกี่ยวข้อง",
        currentLanguage: "ภาษาปัจจุบัน"
      },
      quiz: {
        title: "แบบทดสอบความเข้าใจ",
        questionCount: "ข้อ {current} / {total}",
        previous: "ข้อก่อนหน้า",
        next: "ข้อถัดไป",
        submitAnswer: "ส่งคำตอบ",
        empty: "บทนี้ยังไม่มีแบบทดสอบ",
        unavailable: "แบบทดสอบของ {title} ยังไม่ได้แปลสำหรับภาษาที่เลือก",
        thisLesson: "บทนี้"
      },
      auth: { account: "บัญชี", sync: "ซิงค์", loginTitle: "เข้าสู่ระบบ", signedOut: "ออกจากระบบ" },
      dashboard: { japaneseTyping: "พิมพ์ภาษาญี่ปุ่น" },
      tools: {
        quickStart: "เริ่มต้นอย่างรวดเร็ว",
        quickStartTitle: "ขั้นตอนการเรียน",
        quickStartDesc: "เลือกวิชา → เรียนบท → ฝึกทำ → ดูสถิติ",
        tools: "เครื่องมือ",
        openMenu: "เปิดเมนูเครื่องมือ",
        closeMenu: "ปิดเมนูเครื่องมือ",
        learningTools: "เครื่องมือการเรียน",
        glossary: "คำศัพท์",
        glossaryDesc: "ค้นหาคำศัพท์ IT / SQL / การเขียนโปรแกรม",
        settings: "การตั้งค่า",
        settingsDesc: "ปรับการตั้งค่าเครื่องมือการเรียน",
        dangerous: "การดำเนินการที่อันตราย",
        resetProgress: "รีเซ็ตความคืบหน้า",
        resetProgressDesc: "ล้างความคืบหน้าการเรียนปัจจุบัน ต้องยืนยัน",
        confirmResetTitle: "รีเซ็ตความคืบหน้า?",
        confirmResetBody: "การดำเนินการนี้จะล้างความคืบหน้าการเรียนปัจจุบัน ดำเนินการต่อ?",
        cancel: "ยกเลิก",
        confirmReset: "ยืนยันการรีเซ็ต",
        examTools: "เครื่องมือสอบ",
        examHistory: "ประวัติการสอบ",
        examHistoryDesc: "ดูผลการสอบจำลองล่าสุด",
        dashboard: "สถิติการเรียน",
        dashboardDesc: "ดูความคืบหน้าและภาพรวมข้อมูล",
        exportData: "ส่งออกข้อมูลการเรียน",
        exportDataDesc: "ดาวน์โหลดสำรองข้อมูลการเรียนในเครื่อง (JSON)",
        continueLearning: "เรียนต่อ",
        continueLearningDesc: "ดำเนินการต่อจากครั้งที่แล้ว"
      }
    },
    "id-ID": {
      common: {
        save: "Simpan", cancel: "Batal", confirm: "OK", delete: "Hapus",
        reset: "Reset", resetProgress: "Reset progres", loading: "Memuat...",
        themeToggle: "Beralih mode terang/gelap", close: "Tutup", open: "Buka", search: "Cari",
        random: "Acak", restart: "Ulang", copy: "Salin", maximize: "Maksimalkan",
        minimize: "Minimalkan", challenging: "Menantang", lessonIndex: "Pelajaran {id}",
        changeQuestion: "Ganti soal", stop: "Berhenti", subtitle: "Platform pendukung belajar"
      },
      nav: {
        sql: "Belajar SQL", java: "Belajar Java", python: "Belajar Python",
        itpass: "Persiapan IT Passport", sg: "Persiapan SG",
        typing: "Mengetik Jepang", codingTyping: "Mengetik kode", sqlChallenge: "Tantangan acak"
      },
      moduleSwitch: { title: "Pilih bagian belajar", close: "Tutup", triggerLabel: "Buka pemilih bagian" },
      moduleDesc: {
        sql: "Dasar Database, query, latihan, sandbox",
        sg: "Persiapan ujian Information Security Management",
        itpass: "Dasar IT Passport dan latihan soal", typing: "Latihan mengetik bahasa Jepang",
        codingTyping: "Latihan mengetik dan sintaks kode", java: "Dasar Java, sintaks, latihan",
        python: "Dasar Python, sintaks, latihan"
      },
      codingTyping: {
        title: "Latihan mengetik kode", subtitle: "Latih mengetik untuk menguasai sintaks pemrograman",
        language: "Pilih bahasa", category: "Kategori", allCategories: "Semua",
        difficulty: "Tingkat kesulitan", code: "Kode target", input: "Input",
        inputPlaceholder: "Ketik kode di sini...", focus: "Poin latihan", hint: "Petunjuk",
        speed: "CPM", accuracy: "Akurasi", progress: "Progres",
        time: "Waktu", errors: "Kesalahan", complete: "Selesai!",
        result: "Hasil", totalTime: "Waktu", errorChars: "Karakter sering salah",
        recentHistory: "Riwayat terbaru", noHistory: "Belum ada riwayat",
        next: "Berikutnya (Alt+N)", random: "Acak (Alt+S)", restart: "Ulang (Alt+R)",
        continueRandom: "Lanjut latihan acak", python: "Python", java: "Java", sql: "SQL", symbols: "Simbol",
        normalizeFullWidth: "Penuh→Setengah", lenientIndent: "Indent longgar",
        sendToSandbox: "Kirim ke Sandbox", runInSandbox: "Jalankan di Sandbox",
        sandboxUnavailable: "Sandbox tidak tersedia", sentToSandbox: "Terkirim ke Sandbox",
        openSandbox: "Buka Sandbox",
        shortcutHelp: "Pintasan: Alt+N berikutnya / Alt+R ulang / Alt+S acak",
        summaryLine: "CPM: {cpm} | Akurasi: {acc}% | Salah: {err} | Waktu: {time}",
        backToList: "Kembali ke daftar latihan"
      },
      subnav: {
        lessons: "Buku & Sandbox latihan", practicalExam: "Simulasi ujian",
        study: "Belajar buku", pastExams: "Soal lampau & simulasi ujian",
        javaIntro: "Pengantar (dasar) — Bab 1~8", javaPractice: "Praktik (terapan) — Bab 1~11"
      },
      sidebar: { progress: "Progres", unitList: "Daftar bab" },
      sandbox: {
        run: "Jalankan", playground: "Sandbox", runSql: "Jalankan SQL", resetDb: "Reset DB",
        hint: "Petunjuk", maximize: "Maksimalkan", output: "Hasil",
        sqlPlaceholder: "Ketik perintah SQL...", currentMission: "Misi saat ini",
        randomPractice: "Latihan acak", databaseStructure: "Struktur Database",
        schoolDb: "DB sekolah", bookstoreDb: "DB toko buku",
        sqlPlayground: "SQL Playground", readyStatus: "Siap",
        outputPlaceholder: "(Hasil akan tampil di sini)"
      },
      exam: {
        verify: "Periksa & simpan", submit: "Kumpulkan ujian", previous: "Soal sebelumnya",
        next: "Soal berikutnya", exit: "Keluar ujian", launchBtn: "Mulai ujian",
        status: "Status: ", notStarted: "Belum dikerjakan", remainingTime: "Sisa waktu: "
      },
      glossary: {
        title: "Glosarium IT", open: "Buka glosarium", close: "Tutup", searchPlaceholder: "Cari istilah...",
        allCategories: "Semua", category: "Kategori", noResults: "Tidak ada istilah terkait",
        currentLanguage: "Bahasa saat ini"
      },
      quiz: {
        title: "Tes pemahaman", previous: "Sebelumnya", next: "Berikutnya",
        submitAnswer: "Kirim jawaban"
      },
      auth: { account: "Akun", sync: "Sinkron", loginTitle: "Masuk", signedOut: "Keluar" },
      dashboard: { japaneseTyping: "Mengetik Jepang" }
    }
  };

  window.I18nUiDict["default-ja-zh"] = {
    common: {
      save: "保存 / 保存",
      cancel: "取消 / キャンセル",
      confirm: "确认 / 確認",
      delete: "删除 / 削除",
      reset: "重置 / リセット",
      resetProgress: "重置进度 / 進捗をリセット",
      loading: "加载中... / 読み込み中...",
      themeToggle: "切换浅色/深色模式 / ライト/ダークモードを切り替え",
      close: "关闭 / 閉じる",
      open: "打开 / 開く",
      search: "搜索 / 検索",
      random: "随机 / ランダム",
      restart: "重新开始 / 最初から",
      copy: "复制 / コピー",
      maximize: "最大化 / 最大化",
      minimize: "最小化 / 最小化",
      challenging: "挑战中 / 挑戦中",
      lessonIndex: "第{id}节 / 第{id}課",
      changeQuestion: "换一题 / 別の問題",
      stop: "结束 / 終了",
      subtitle: "配套备考平台 / 学習支援プラットフォーム"
    },
    nav: {
      sql: "SQL 学习 / SQL学習",
      java: "Java 学习 / Java学習",
      python: "Python 学习 / Python学習",
      itpass: "IT Passport 备考 / ITパスポート対策",
      sg: "SG 备考 / 情報セキュリティマネジメント対策",
      typing: "日本语打字 / 日本語タイピング",
      codingTyping: "编程打字 / プログラミングタイピング",
      sqlChallenge: "综合随机挑战 / 総合ランダム挑戦"
    },
    moduleSwitch: {
      title: "选择学习版块 / 学習セクションを選択",
      close: "关闭 / 閉じる",
      triggerLabel: "打开版块选择 / セクション選択を開く"
    },
    moduleDesc: {
      sql: "数据库基础、查询、练习与沙盒 / データベース基礎、クエリ、練習、サンドボックス",
      sg: "情報セキュリティマネジメント試験対策",
      itpass: "IT Passport 基础与真题练习 / ITパスポート基礎と過去問演習",
      typing: "日本语输入与打字练习 / 日本語入力とタイピング練習",
      codingTyping: "代码输入与语法练习 / コード入力と構文練習",
      java: "Java 基础、语法与练习 / Java の基礎、構文、練習",
      python: "Python 基础、语法与练习 / Python の基礎、構文、練習"
    },
    codingTyping: {
      title: "编程打字练习 / プログラミングタイピング練習",
      subtitle: "通过打字练习掌握编程语法。 / タイピング練習でプログラミング文法をマスター。",
      language: "选择语言 / 言語選択",
      category: "类别过滤 / カテゴリフィルター",
      allCategories: "全部 / すべて",
      difficulty: "难度 / 難易度",
      next: "下一题 / 次の問題",
      random: "随机一题 / ランダム",
      restart: "重来 / やり直し",
      code: "目标代码 / 目標コード",
      input: "输入区 / 入力エリア",
      inputPlaceholder: "在此输入目标代码... / 目標コードを入力...",
      focus: "练习要点 / 練習ポイント",
      hint: "提示 / ヒント",
      speed: "CPM",
      accuracy: "正确率 / 正確率",
      progress: "进度 / 進捗",
      time: "用时 / 時間",
      errors: "错误 / エラー",
      complete: "完成！ / 完了！",
      result: "练习结果 / 練習結果",
      totalTime: "总用时 / 総時間",
      errorChars: "常错字符 / よく間違える文字",
      recentHistory: "最近记录 / 最近の記録",
      noHistory: "暂无记录 / 記録なし",
      python: "Python",
      java: "Java",
      sql: "SQL",
      symbols: "符号 / 記号",
      normalizeFullWidth: "全角→半角 / 全角→半角",
      lenientIndent: "缩进宽容 / インデント寛容",
      sendToSandbox: "发送到沙盒 / サンドボックスに送信",      runInSandbox: "在沙盒中运行 / サンドボックスで実行",      sandboxUnavailable: "沙盒不可用 / サンドボックス利用不可",      sentToSandbox: "已发送到沙盒 / サンドボックスに送信しました",      openSandbox: "打开沙盒 / サンドボックスを開く",      shortcutHelp: "快捷键: Alt+N 下一题 / Alt+R 重来 / Alt+S 随机 / 快捷键: Alt+N 次の問題 / Alt+R やり直し / Alt+S ランダム",
      examInsight: "考试理解卡片 / 試験理解カード",
      examRelevance: "考试相关性 / 出題頻度",
      examTargets: "考试目标 / 試験対策対象",
      relatedTerms: "相关术语 / 関連用語",
      meaning: "含义 / 意味",
      memoryHook: "记忆法 / 覚え方",
      examPoint: "考点 / 試験ポイント",
      commonMistake: "易错点 / 間違えやすい点",
      high: "高 / 高",
      medium: "中 / 中",
      low: "低 / 低",
      backToList: "返回练习列表 / 練習リストへ戻る",
      exerciseList: "练习列表 / 練習リスト"
    },
    subnav: {
      lessons: "教科书与演练沙盒 / テキスト・演習",
      practicalExam: "实操模拟考试 / 実技模擬試験",
      study: "教科书章节学习 / テキスト学習",
      pastExams: "过去问道场 & 模拟考试 / 過去問道場・模擬試験",
      javaIntro: "入门编 (基础篇) — 第1〜8章 / 入門編 — 第1〜8章",
      javaPractice: "实践编 (进阶篇) — 第1〜11章 / 実践編 — 第1〜11章"
    },
    sidebar: {
      progress: "学习进度 / 進捗状況",
      unitList: "课程目录 / 単元リスト"
    },
    sandbox: {
      run: "执行 / 実行",
      playground: "沙盒 / サンドボックス",
      runSql: "执行 SQL / SQLを実行",
      resetDb: "重置 DB / DBリセット",
      hint: "提示 / ヒント",
      maximize: "最大化 / 最大化",
      output: "执行结果 / 実行結果",
      sqlPlaceholder: "请在此处输入 SQL 语句... / ここにSQLを入力してください...",
      currentMission: "当前任务 / ミッション",
      randomPractice: "随机指令练习 / ランダム指令練習",
      databaseStructure: "数据库表结构参考 / データベース構造",
      schoolDb: "学校DB / 学校DB",
      bookstoreDb: "书店DB / 书店DB",
      javaSandboxTitle: "Java 编程沙盒 / Javaプログラミング沙盒",
      pythonSandboxTitle: "Python 编程沙盒 / Pythonプログラミング沙盒",
      pythonSandboxDesc: "Python 3.12 · 10s · 离线沙盒 / オフラインサンドボックス",
      sqlPlayground: "SQL 演练沙盒 / SQL プレイグラウンド",
      chooseLessonLoadCode: "— 选择课程后载入示例代码 — / — レッスンを選択してコードをロード —",
      toolbarTemplate: "模板 / テンプレ",
      toolbarTemplateTitle: "加载当前课程模板 / テンプレートを読み込む",
      toolbarCopy: "复制 / コピー",
      toolbarCopyTitle: "复制代码 / コードをコピー",
      toolbarClear: "重置 / リセット",
      toolbarClearTitle: "清空并重置 / コードをクリア",
      javaEditorPlaceholder: "Java 代码を这里に入力してください... / Javaコードをここに入力してください...",
      pythonEditorPlaceholder: "Python 代码を这里に入力してください... / Pythonコードをここに入力してください...",
      stdinHeader: "标准输入 / 標準入力 (stdin)",
      stdinPlaceholder: "程序接收的输入内容写在这里... / 標準入力の値をここに入力...",
      runCode: "运行 / 実行",
      readyStatus: "已就绪 / 準備完了 / Ready",
      outputPlaceholder: "（点击运行按钮，结果将显示在这里） / （実行ボタンを押すと、ここに結果が表示されます）"
    },
    exam: {
      verify: "验证并保存答案 / 判定して保存",
      submit: "提交试卷并交卷 / 試験終了",
      previous: "上一题 / 前へ",
      next: "下一题 / 次へ",
      exit: "中断退出 / 試験離脱",
      ruleTitle: "过去问道场 — CBT 模拟考试系统 / 過去問道場 — CBT模擬試験",
      ruleDesc: "基于近年 IT 考试真题，模拟真实计算机机考 (CBT) 环境。 / 過去のIT試験に基づき、CBT環境をシミュレート。",
      ruleDetailTitle: "考试规则 / 試験規則:",
      ruleDetail1: "题目均为实操题，给日文文字任务或预期输出结果。 / 問題はすべて実技問題で、日本語タスクまたは期待出力が与えられます。",
      ruleDetail2: "支持在右侧模拟沙盒中即时编写、运行和调试代码。 / 右側のシミュレータで即座にコードを記述・デバッグできます。",
      ruleDetail3: "完成代码后，点击“验证并保存答案(判定)”。 / コード完成後、「判定して保存」をクリックします。",
      ruleDetail4: "测试成功（Passed ✓）计入得分，可反复修改。 / テスト合格で得点となり、何度でも修正可能です。",
      ruleDetail5: "请在倒计时结束前点击“提交试卷并交卷”。 / 制限時間内に「試験終了」をクリックしてください。",
      selectYear: "选择考试年度/真题: / 試験年度を選択:",
      questionCount: "模拟考试题量: / 模擬試験の問題数:",
      launchBtn: "开始机考挑战 / 試験開始",
      status: "测试状态: / テスト状態: ",
      notStarted: "未作答 (未着手)",
      cbtSimulator: "CBT 模拟系统 / CBT模擬システム",
      remainingTime: "剩余时间: / 残り時間: ",
      exitCbt: "中断退出 / 中断退出",
      submitCbt: "提交试卷 / 試験提出",
      questionNumber: "第 {current} 题 / 問 {current}",
      flagReview: "标记此题复查 / この問題をマーク",
      metaExpand: "题目信息 / 問題情報 <i class=\"fa-solid fa-chevron-down\"></i>",
      metaCollapse: "收起 / 閉じる <i class=\"fa-solid fa-chevron-up\"></i>",
      metaSource: "出典：",
      metaYear: "年度 / 年度",
      metaQNum: "题号 / 問番号",
      metaCategory: "分类 / 分類",
      metaSubcategory: "子分类 / サブカテゴリ",
      metaTopic: "主题 / トピック",
      ruleTitleCoding: "实操编程模拟考试 / 実技プログラミング模擬試験",
      ruleDescCoding: "模拟日本 IT 专门学校的编程考试环境。 / IT専門学校のプログラミング試験環境を再現。",
      launchBtnCoding: "开始实操考试 / 試験開始",
      cbtSimulatorCoding: "实操机考 / 実技模擬試験",
      expectedOutput: "期待される出力結果 (期望执行输出)",
      resultsTitle: "实操考试成绩单 / 評価結果",
      resultsTitleCbt: "模拟考试测评成绩单 / 模擬試験成績書",
      backToHome: "返回主页 / ホームに戻る",
      backToDojo: "返回道场 / 道場に戻る",
      passLabel: "合格判定 / 合否判定",
      passStatus: "合格 / 合格",
      failStatus: "不合格 / 不合格",
      correctCountText: "{count} / {total} 题通过 / {count} / {total} 問合格",
      totalScoreText: "{score} / 1000 点 / {score} / 1000 点",
      correctRate: "得点率 (通过率) / 得点率",
      timeSpent: "考试用时 / 所要時間:",
      detailsTitle: "各题验证测试详情 / 各問題の判定詳細",
      reviewTitle: "真题回顾与错题分析 / 問題の振り返りと誤答分析",
      reviewTip: "点击行可直接展开查看题干详情及中日文解析。 / 行をクリックすると、問題の詳細と中日解説を展開できます。",
      reviewCollapse: "收起解析 / 解説を閉じる",
      reviewExplanation: "解析 / 解説",
      resultCorrect: "正确 / 正解",
      resultIncorrect: "错误 / 不正解",
      tableQNo: "题号 / 問",
      tableTitle: "考核知识点 / タイトル",
      tableField: "领域 / 分野",
      tableDifficulty: "难度 / 難易度",
      tableResult: "验证结果 / 判定結果",
      tableYourAnswer: "您的答案 / 解答",
      tableCorrectAnswer: "正确答案 / 正解",
      tableAction: "操作 / 操作",
      tableActionDetail: "查看 / 詳細",
      personalizeTitle: "机考个性化选项 / 模擬試験の個人化設定:",
      optShuffle: "随机打乱选择肢顺序 / 選択肢の順序をシャッフル",
      optNocalc: "过滤计算题 / 計算問題をフィルター",
      all: "随机混合真题 (推荐) / 過去問ランダムミックス (推奨)",
      r8: "令和8年度 (R8) 公开问题 / 令和8年度 (R8) 公開問題",
      r7: "令和7年度 (R7) 公开问题 / 令和7年度 (R7) 公開問題",
      r6: "令和6年度 (R6) 公开问题 / 令和6年度 (R6) 公開問題",
      r5: "令和5年度 (R5) 公开问题 / 令和5年度 (R5) 公開問題",
      r4: "令和4年度 (R4) 公开问题 / 令和4年度 (R4) 公開問題",
      r3: "令和3年度 (R3) 公开问题 / 令和3年度 (R3) 公開問題",
      cbtQ100: "100 题 (限时120分钟) / 100問 (制限時間120分)",
      cbtQ50: "50 题 (限时60分钟) / 50問 (制限時間60分)",
      cbtQ30: "30 题 (限时36分钟) / 30問 (制限時間36分)",
      cbtQ10: "10 题 (限时12分钟) / 10問 (制限時間12分)",
      q5: "5 题 (快速测试 - 15分钟) / 5問 (クイックテスト - 15分)",
      q10: "10 题 (标准实操 - 30分钟) / 10問 (標準実技 - 30分)",
      q15: "15 题 (全面挑战 - 45分钟) / 15問 (フルチャレンジ - 45分)",
      gridTitle: "答题卡看板 / マークシート",
      unanswered: "未答 / 未答",
      answered: "已答 / 既答",
      flagged: "标记 / マーク",
      totalScoreLabel: "综合得点 (总分) / 総合得点",
      domainStrat: "ストラテジ系 (策略领域) / ストラテジ系",
      domainMan: "マネジメント系 (管理领域) / マネジメント系",
      domainTech: "テクノロジ系 (技术领域) / テクノロジ系",
      domainSubb: "科目B (场景应用) / 科目B"
    },
    toast: {
      alreadyFirst: "已经是第一题了。 / 最初の問題です。",
      alreadyLast: "已经是最后一题了。 / 最後の問題です。",
      saved: "已保存。 / 保存しました。",
      verifySuccess: "判定通过！已保存答案。 / 判定に合格しました。答案を保存しました。",
      dbReset: "模拟数据库已重置回初始数据。 / 模擬データベースを初期状態に戻しました。",
      examStarted: "CBT 模拟考试已生成，考试开始！ / CBT模擬試験を開始しました。",
      lessonQuizPassed: "🏆 恭喜！{title} 课后小测已全部通过！ / 🏆 {title} のクイズにすべて合格！",
      dbSwitched: "🗄️ 已切换数据库《{dbName}》- {tables} / 《{dbName}》- {tables} に切り替えました。",
      sqlHint: "💡 提示 / ヒント：<br><code style=\"font-family:'Fira Code',monospace;color:#a5f3fc;\">{code}</code>",
      noCompletedLessons: "⚠️ 还没有已完成的课节！ / 完了した単元がありません！",
      challengeStarted: "🔥 综合挑战开始！共 {count} 题 / 🔥 総合挑戦開始！計 {count} 問",
      challengeEnded: "✅ 综合挑战结束。 / 総合挑戦が終了しました。",
      challengeQuestionCorrect: "🎉 正确！已通过第 {sourceId} 节的题目。 / 🎉 正解！レッスン {sourceId} クリア。",
      challengeSuccess: "综合挑战达成！ / 総合挑戦達成！",
      examEmpty: "⚠️ 符合条件的真题为空！ / 条件に合う過去問がありません！",
      examTimeout: "⏰ 时间到，自动交卷！ / 制限時間になり自動提出します。",
      examPassed: "🎉 恭喜！成功通过本次模拟测试！ / 模擬テストに合格しました！",
      examFailed: "💪 未通过合格标准，请继续努力！ / 合格基準に達していません。",
      examDbNotFound: "⚠️ 未找到该科目考题数据库！ / 模擬データベースが見つかりません。",
      practicalExamStarted: "📝 实操模拟考试开始！ / 実技模擬試験を開始しました。",
      sqlSyntaxError: "❌ SQL 语法错误！ / SQL構文エラー。",
      sqlOutputMismatch: "❌ 结果不一致，请检查排序或过滤！ / 実行結果が異なります。",
      compileError: "❌ 编译错误，判定失败！ / コンパイルエラー。",
      runtimeError: "❌ 运行错误，判定失败！ / 実行エラー。",
      outputMismatch: "❌ 输出内容不一致！ / 出力結果が異なります。",
      practicalExamSubmitted: "🎉 交卷成功！已生成成绩单。 / 提出成功！成績書を生成しました。"
    },
    dialog: {
      resetDbConfirm: "模拟数据库将被重置回初始状态，继续吗？ / 模擬データベースを初期状態に戻します。続行しますか？",
      exitExamConfirm: "确定要中断退出当前考试吗？进度将不被保存。 / 現在の試験を中断して終了しますか？進捗は保存されません。",
      switchExamConfirm: "当前有正在运行的考试。切换科目会中断当前考试，确定要切换吗？ / 現在実行中の試験があります。科目を切り替えると現在の試験は中断されます。切り替えますか？",
      clearCodeConfirm: "确定要清空代码吗？ / コードをクリアしますか？",
      resetProgressConfirm: "确定要重置所有 {subject} 学习进度吗？ / すべての {subject} の学習進捗をリセットしますか？",
      submitExamConfirm: "确定要交卷吗？ / 試験を終了して提出しますか？",
      submitExamUnansweredConfirm: "还有 {count} 道题未作答，确定现在交卷吗？ / 未解答の問題が {count} 問あります。本当に今提出しますか？",
      submitCodingExamUnansweredConfirm: "还有 {count} 道题未作答或未判定成功，确定现在交卷吗？ / 未解答または判定未合格の問題が {count} 問あります。本当に今提出しますか？"
    },

    tools: {
      quickStart: "快速开始 / クイックスタート",
      quickStartTitle: "学习流程 / 学習の流れ",
      quickStartDesc: "选择科目 → 学习课程 → 做练习 → 查看统计 / 科目を選ぶ → 課程を学ぶ → 練習する → 統計を確認する",
      tools: "工具 / ツール",
      openMenu: "打开工具菜单 / ツールメニューを開く",
      closeMenu: "关闭工具菜单 / ツールメニューを閉じる",
      learningTools: "学习工具 / 学習ツール",
      glossary: "术语表 / 用語集",
      glossaryDesc: "查询 IT / SQL / 编程术语 / IT / SQL / プログラミング用語を検索",
      examTools: "考试工具 / 試験ツール",
      examHistory: "考试历史 / 試験履歴",
      examHistoryDesc: "查看最近的模拟考试记录 / 最近の模擬試験記録を確認",
      settings: "设置 / 設定",
      settingsDesc: "调整学习工具偏好 / 学習ツールの設定を調整",
      dangerous: "危险操作 / 危険な操作",
      resetProgress: "重置进度 / 進捗をリセット",
      resetProgressDesc: "清空当前学习进度，需要确认 / 現在の学習進捗を消去します。確認が必要です",
      confirmResetTitle: "确认重置进度？ / 進捗をリセットしますか？",
      confirmResetBody: "此操作会清空当前学习进度，是否继续？ / 現在の学習進捗が消去されます。続行しますか？",
      cancel: "取消 / キャンセル",
      confirmReset: "确认重置 / リセットする"
    },
    examHistory: {
      title: "最近考试历史 / 最近の試験履歴",
      subtitle: "查看已保存的模拟考试结果 / 保存された模擬試験結果を確認",
      close: "关闭 / 閉じる",
      filterAll: "全部 / すべて",
      empty: "还没有考试历史 / 試験履歴はまだありません",
      emptyHint: "完成一次模拟考试后，这里会显示记录 / 模擬試験を完了すると、ここに記録が表示されます",
      questions: "题量 / 問題数",
      correct: "正确 / 正解",
      accuracy: "正确率 / 正答率",
      score: "分数 / 点数",
      passed: "合格 / 合格",
      failed: "未合格 / 不合格",
      time: "用时 / 所要時間",
      viewDetails: "查看详情 / 詳細を見る",
      hideDetails: "收起详情 / 詳細を閉じる",
      examConfig: "考试配置 / 試験設定",
      fieldScores: "分类子分 / 分野別スコア",
      questionResults: "逐题结果 / 問題別結果",
      yearFilter: "年度筛选 / 年度フィルター",
      submittedAt: "提交时间 / 提出日時",
      yourAnswer: "用户答案 / あなたの答え",
      correctAnswer: "正确答案 / 正解",
      unanswered: "未答 / 未回答",
      questionNotFound: "题目数据未找到 / 問題データが見つかりません"
    },
    wrongBook: {
      title: "错题本 / 間違いノート",
      subtitle: "复习答错的题目 / 間違えた問題を復習",
      emptyTitle: "还没有错题 / 間違えた問題はまだありません",
      emptyDesc: "答错的题目会自动收集到这里 / 間違えた問題は自動的にここへ保存されます",
      all: "全部 / すべて",
      unmastered: "未掌握 / 未習得",
      mastered: "已掌握 / 習得済み",
      markMastered: "标记已掌握 / 習得済みにする",
      unmarkMastered: "取消已掌握 / 未習得に戻す",
      remove: "移除 / 削除",
      wrongCount: "答错次数 / 間違えた回数",
      lastWrongAt: "最近答错 / 最終間違い日時",
      yourAnswer: "我的错误答案 / あなたの誤答",
      correctAnswer: "正确答案 / 正解",
      explanation: "解析 / 解説",
      source: "来源 / 出典",
      confirmRemove: "确定从错题本移除这道题吗？ / この問題を間違いノートから削除しますか？",
      viewDetails: "展开详情 / 詳細を見る",
      hideDetails: "收起详情 / 詳細を閉じる",
      choices: "选项 / 選択肢",
      question: "题目 / 問題",
      unknownAnswer: "未记录 / 記録なし"
    },
    message: {
      inputSqlRequired: "请输入 SQL 语句。 / SQLクエリを入力してください。"
    },
    settings: {
      language: "语言 / 言語"
    },
    lesson: {
      locateBookPage: "定位原书页 / 原書ページへ",
      explanationJa: "解説 (日本語) / 解説 (日本語)",
      explanationZh: "讲解 (中文) / 解説 (中国語)",
      compareMode: "左右对照 (日/中) / 左右対称 (日/中)",
      japaneseOnly: "日本語のみ / 日本語のみ",
      chineseOnly: "中文简体 / 中国語のみ",
      analogy: "身近な例え (形象比喻) / 身近な例え",
      sqlExample: "SQL构文例 (SQL示例) / SQL構文例",
      javaExample: "Java 代码例 (示例代码) / Java コード例",
      pythonExample: "Python 代码例 (示例代码) / Python コード例",
      glossary: "重要用語・公式 (核心术语 & 公式) / 重要用語・公式",
      copyToEditor: "复制到编辑器 / エディタへコピー",
      showPdf: "显示原书 PDF / 原書PDFを表示",
      translationZh: "中文翻译对照 / 中国語対訳",
      checklist: "要点確認 / 要点速记",
      calculator: "計算ツール / 计算工具箱",
      calculatorSelect: "ツールを選択 / 选择工具:"
    },
    flashcard: {
      title: "単語カード / 词汇闪卡 / 用語カード",
      flip: "翻转 / Flip / フリップ",
      clickToFlip: "点击闪卡即可翻转查看中日对照翻译及释义： / カードをクリックして裏表を切り替えます：",
      clickToFlipHint: "点击此处翻转 / クリックして裏を見る",
      localizedDesc: "{title} 的核心术语卡片 / {title} の重要用語カード。"
    },
    glossary: {
      title: "IT术语表 / IT用語集",
      open: "术语表 / 用語集",
      close: "关闭 / 閉じる",
      searchPlaceholder: "搜索术语... / 用語を検索...",
      allCategories: "全部 / すべて",
      filterDatabase: "数据库 / データベース",
      filterSecurity: "安全 / セキュリティ",
      filterNetwork: "网络 / ネットワーク",
      filterProgramming: "编程 / プログラミング",
      filterSystem: "系统 / システム",
      category: "分类 / カテゴリ",
      japanese: "日语 / 日本語",
      english: "英语 / 英語",
      currentLanguage: "当前语言 / 現在の言語",
      aliases: "别名 / 別名",
      relatedTerms: "相关术语 / 関連用語",
      example: "代码示例 / コード例",
      source: "来源 / 出典",
      needsReview: "⚠ 翻译待校对 / 翻訳確認中",
      noResults: "没有找到相关术语 / 該当する用語はありません",
      keepEnglishNote: "保留英文技术术语 / 英語の技術用語を保持",
      resultCount: "共 {count} 个术语 / {count} 件の用語"
    },
    quiz: {
      title: "理解度测试 (章节小测) / 理解度テスト",
      questionCount: "问题 {current} / 共 {total} 题 / 問題 {current} / 全 {total} 問",
      previous: "上一题 / 前へ",
      next: "下一题 / 次へ",
      submitAnswer: "提交回答 / 回答する",
      empty: "本章暂无课后练习小测 / この章にはまだ小テストがありません。",
      unavailable: "{title} 的小测暂未完成当前显示语言本地化 / {title} の小テストはまだこの表示言語に対応していません。",
      thisLesson: "本章 / この章"
    },
    typing: {
      library: "打字文库 / タイピング文庫",
      libraryDesc: "从 N3 到 N1 实战日本語输入，支持生活、职场、长文练习。 / 実用日本語入力練習。生活・職場・長文を自分のペースで。",
      search: "文库搜索 / ライブラリ検索",
      searchPlaceholder: "标题、内容或标签 / タイトル、内容、またはタグ",
      articleList: "文章列表 / 記事一覧",
      randomArticle: "随机一篇 / ランダム",
      restart: "重新开始 / 最初から",
      timeLimit: "限时 / 制限時間",
      imeMode: "IME 实战模式 / IMEモード",
      speed: "速度 / 速度",
      accuracy: "正确率 / 正確率",
      progress: "有效进度 / 有効進捗",
      edits: "修改次数 / 修正回数",
      time: "时间 / 時間",
      recentScores: "最近成绩 / 最近の成績",
      personalReview: "个人复练 / 個人復習",
      reviewDesc: "正确率低于 98% 的文章会自动加入错句复练。 / 正確率98%未満の記事は自動的に復習リストに追加されます。",
      reviewCount: "{count} 篇待复练 / {count} 件の復習待ち",
      emptyReview: "无需要复练的文章 / 復習待ちの記事はありません",
      imePlaceholder: "日语输入法をオンにして、这里から输入してください。 / 日本語IMEをオンにして、ここから入力してください。",
      typingHelp: "系统日语 IME 输入。全角与半角会统一比较。 / システム日本語IME入力。全角/半角は統一して比較されます。",
      resultTitle: "练习完成 / 練習完了",
      continueRandom: "继续随机练习 / ランダム練習を続ける",
      favoriteBtn: "收藏 / お気に入り",
      favoriteTitle: "收藏当前文章 / この記事をお気に入りに追加",
      noLimit: "不限时 / 制限なし",
      t1min: "1分钟 / 1分",
      t3min: "3分钟 / 3分",
      t5min: "5分钟 / 5分",
      t10min: "10分钟 / 10分"
    },
    console: {
      output: "执行结果 / 実行結果",
      rows: "{count}行 / {count}行",
      emptyResult: "查询结果表格将在此处显示。 / 結果テーブルがここに表示されます。",
      emptyResultSub: "(执行 SQL 后显示) / （SQL実行後に結果が表示されます）",
      maximize: "最大化 / 最大化",
      minimize: "最小化 / 最小化"
    },
    auth: {
      account: "账号 / アカウント",
      close: "关闭 / 閉じる",
      currentUser: "当前用户 / 現在のユーザー",
      notLoggedIn: "未登录 / ログインしていません",
      signedIn: "已登录 / ログイン済み",
      signedOut: "未登录 / ログアウト済み",
      localMode: "本地模式 / ローカルモード",
      sync: "同步 / 同期",
      syncNow: "立即同步 / 今すぐ同期",
      syncingNow: "正在同步 / 同期中",
      syncSuccess: "同步成功 / 同期成功",
      syncFailed: "同步失败 / 同期失敗",
      syncFailedDetail: "部分同步失败，请重试 / 一部の同期が失敗しました。再試行してください",
      syncIntro: "多设备同步进度及设置 / 複数デバイスで進捗と設定を同期",
      syncScope: "只同步学习进度和设置 / 学習進捗と設定のみ同期",
      onlySyncTypingBookmarks: "当前仅同步日语打字收藏 / 現在は日本語タイピングのお気に入りのみ同期",
      noAiKeyUpload: "不会上传 AI Key / AI Key はアップロードされません",
      noAiCacheUpload: "不会上传 AI 翻译缓存 / AI翻訳キャッシュはアップロードされません",
      noAutoSync: "当前不会自动同步 / 自動同期は行われません",
      signInFirst: "请先登录 / 先にログインしてください",
      signInSuccess: "登录成功 / ログイン成功",
      signInFailed: "登录失败 / ログイン失敗",
      signOut: "登出 / ログアウト",
      exportSnapshot: "导出本地快照 / ローカルスナップショットをエクスポート",
      continueLocal: "继续本地使用 / ローカル使用を続ける",
      mockSignIn: "模拟登录状态 / ログイン状態を模擬",
      mockSignOut: "退出模拟登录 / 模擬ログインを終了",
      syncInDev: "同步功能开发中 / 同期機能は開発中",
      noUpload: "不会上传数据 / データはアップロードされません",
      deviceId: "设备 ID / デバイス ID",
      pendingSync: "待同步 / 同期待ち",
      lastSync: "上次同步 / 最終同期",
      noUpload: "不会上传数据 / データはアップロードされません",
      deviceAndSyncDetails: "设备与同步详情 / デバイス・同期詳細",
      loginTitle: "登录 / ログイン",
      registerTitle: "注册 / アカウント登録",
      username: "用户名 / ユーザー名",
      usernamePlaceholder: "3-24 位字母、数字、下划线或连字符 / 3～24文字の英数字、_、-",
      displayNameOptional: "昵称（可选） / 表示名（任意）",
      password: "密码 / パスワード",
      confirmPassword: "确认密码 / パスワード確認",
      loginButton: "登录 / ログイン",
      createAccountButton: "创建账号 / アカウント作成",
      noEmailRequired: "无需邮箱 / メール不要",
      noEmailRequiredDesc: "登录只需用户名和密码 / ユーザー名とパスワードだけでログイン",
      usernameRequired: "请输入用户名 / ユーザー名を入力してください",
      usernameTooShort: "用户名至少 3 位 / ユーザー名は3文字以上です",
      usernameTooLong: "用户名最多 24 位 / ユーザー名は24文字以内です",
      usernameInvalid: "用户名格式不正确 / ユーザー名の形式が正しくありません",
      passwordRequired: "请输入密码 / パスワードを入力してください",
      passwordTooShort: "密码至少 6 位 / パスワードは6文字以上で入力してください",
      passwordMismatch: "两次密码不一致 / パスワードが一致しません",
      loginSuccess: "登录成功 / ログイン成功",
      loginFailed: "登录失败 / ログイン失敗",
      registerSuccess: "注册成功 / 登録成功",
      registerFailed: "注册失败 / 登録失敗",
      usernameOrPasswordWrong: "用户名或密码不正确 / ユーザー名またはパスワードが正しくありません",
      usernameTaken: "该用户名已被使用 / このユーザー名は既に使用されています",
      tooManyRequests: "请求过于频繁，请稍后再试 / リクエストが多すぎます。しばらくしてから再試行してください",
      usernameAccount: "用户名账号 / ユーザー名アカウント",
      legacyEmailAccount: "旧邮箱账号 / 旧メールアカウント",
      accountType: "账号类型 / アカウント種別",
      emailConfirmationUsernameWarning: "项目启用了邮箱确认，用户名账号无法接收确认邮件。 / メール確認が有効なため、ユーザー名アカウントは確認メールを受信できません。",
      disabled: "已配置但未启用 / 設定済みだが無効",
      initializationError: "初始化失败 / 初期化失敗",
      unknown: "未知 / 不明",
      sdkMissing: "SDK 未加载 / SDK未ロード",
      supabaseConnected: "Supabase 已连接 / Supabase 接続済み",
      supabaseNotConfigured: "Supabase 未配置 / Supabase未設定",
      conflictsResolved: "已合并远端进度 / リモート進捗をマージ",
      syncSummaryTitle: "本次同步摘要 / 今回の同期サマリー",
      progressPushed: "进度上传 / 進捗アップロード",
      progressPulled: "进度下载 / 進捗ダウンロード",
      quizPushed: "测验结果 / クイズ結果",
      bookmarksPushed: "收藏上传 / お気に入りアップロード",
      bookmarksPulled: "收藏下载 / お気に入りダウンロード",
      bookmarksMerged: "收藏已合并 / お気に入りマージ済み",
      bookmarksDeletedSynced: "收藏删除已同步 / お気に入り削除同期済み",
      bookmarksRestored: "收藏已恢复 / お気に入り復元済み",
      bookmarksConflictsResolved: "收藏冲突已处理 / お気に入り競合解決済み",
      wrongBookPushed: "错题上传 / 間違いノートアップロード",
      wrongBookPulled: "错题下载 / 間違いノートダウンロード",
      wrongBookMerged: "错题已合并 / 間違いノートマージ済み",
      wrongBookSkipped: "错题已跳过 / 間違いノートスキップ済み",
      wrongBookRetrySettingsPushed: "错题重练设置已上传 / 間違いノート再練習設定アップロード",
      wrongBookRetrySettingsPulled: "错题重练设置已下载 / 間違いノート再練習設定ダウンロード",
      wrongBookRetrySettingsMerged: "错题重练设置已合并 / 間違いノート再練習設定マージ済み",
      wrongBookRetrySettingsFailed: "错题重练设置同步失败 / 間違いノート再練習設定同期失敗",
      typingSessionsPushed: "打字练习上传 / タイピング練習アップロード",
      typingSessionsPulled: "打字练习下载 / タイピング練習ダウンロード",
      examSessionsPushed: "考试记录上传 / 試験記録アップロード",
      examSessionsPulled: "考试记录下载 / 試験記録ダウンロード"
    }
  };
  var AUTH_ROUND_17_5 = {
    "zh-CN": {
      magicLinkTitle: "使用 Magic Link 登录", sendMagicLink: "发送登录链接", email: "邮箱",
      password: "密码", passwordTest: "邮箱密码登录（测试功能）", passwordSignIn: "邮箱密码登录（测试）",
      signOut: "登出", currentUser: "当前用户", supabaseConnected: "Supabase 已连接",
      supabaseNotConfigured: "Supabase 未配置", sdkMissing: "SDK 未加载", signInSuccess: "登录成功",
      signInFailed: "登录失败", checkEmail: "请检查邮箱", noLearningSync: "当前不会同步学习数据",
      emailRequired: "请输入邮箱", credentialsRequired: "请输入邮箱和密码"
    },
    "ja-JP": {
      magicLinkTitle: "Magic Link でログイン", sendMagicLink: "ログインリンクを送信", email: "メールアドレス",
      password: "パスワード", passwordTest: "メール・パスワードログイン（テスト）", passwordSignIn: "メールでログイン（テスト）",
      signOut: "ログアウト", currentUser: "現在のユーザー", supabaseConnected: "Supabase 接続済み",
      supabaseNotConfigured: "Supabase 未設定", sdkMissing: "SDK 未読み込み", signInSuccess: "ログイン成功",
      signInFailed: "ログイン失敗", checkEmail: "メールを確認してください", noLearningSync: "学習データは現在同期されません",
      emailRequired: "メールアドレスを入力してください", credentialsRequired: "メールアドレスとパスワードを入力してください"
    },
    "en-US": {
      magicLinkTitle: "Sign in with Magic Link", sendMagicLink: "Send sign-in link", email: "Email",
      password: "Password", passwordTest: "Email/password sign-in (test)", passwordSignIn: "Sign in with password (test)",
      signOut: "Sign out", currentUser: "Current user", supabaseConnected: "Supabase connected",
      supabaseNotConfigured: "Supabase not configured", sdkMissing: "SDK not loaded", signInSuccess: "Sign-in successful",
      signInFailed: "Sign-in failed", checkEmail: "Please check your email", noLearningSync: "Learning data is not currently synced",
      emailRequired: "Enter your email", credentialsRequired: "Enter your email and password"
    },
    "vi-VN": {
      magicLinkTitle: "Đăng nhập bằng Magic Link", sendMagicLink: "Gửi liên kết đăng nhập", email: "Email",
      password: "Mật khẩu", passwordTest: "Đăng nhập email/mật khẩu (thử nghiệm)", passwordSignIn: "Đăng nhập bằng mật khẩu (thử nghiệm)",
      signOut: "Đăng xuất", currentUser: "Người dùng hiện tại", supabaseConnected: "Đã kết nối Supabase",
      supabaseNotConfigured: "Chưa cấu hình Supabase", sdkMissing: "SDK chưa tải", signInSuccess: "Đăng nhập thành công",
      signInFailed: "Đăng nhập thất bại", checkEmail: "Vui lòng kiểm tra email", noLearningSync: "Dữ liệu học tập hiện chưa được đồng bộ",
      emailRequired: "Vui lòng nhập email", credentialsRequired: "Vui lòng nhập email và mật khẩu"
    },
    "fr-FR": {
      magicLinkTitle: "Connexion avec Magic Link", sendMagicLink: "Envoyer le lien de connexion", email: "E-mail",
      password: "Mot de passe", passwordTest: "Connexion e-mail/mot de passe (test)", passwordSignIn: "Se connecter avec le mot de passe (test)",
      signOut: "Se déconnecter", currentUser: "Utilisateur actuel", supabaseConnected: "Supabase connecté",
      supabaseNotConfigured: "Supabase non configuré", sdkMissing: "SDK non chargé", signInSuccess: "Connexion réussie",
      signInFailed: "Échec de la connexion", checkEmail: "Veuillez vérifier votre e-mail", noLearningSync: "Les données d’apprentissage ne sont pas synchronisées actuellement",
      emailRequired: "Saisissez votre e-mail", credentialsRequired: "Saisissez votre e-mail et votre mot de passe"
    },
    "my-MM": {
      magicLinkTitle: "Magic Link ဖြင့် ဝင်မည်", sendMagicLink: "ဝင်ရောက်ရန် လင့်ခ်ပို့မည်", email: "အီးမေးလ်",
      password: "စကားဝှက်", passwordTest: "အီးမေးလ်/စကားဝှက် ဝင်ရောက်မှု (စမ်းသပ်)", passwordSignIn: "စကားဝှက်ဖြင့် ဝင်မည် (စမ်းသပ်)",
      signOut: "ထွက်မည်", currentUser: "လက်ရှိအသုံးပြုသူ", supabaseConnected: "Supabase ချိတ်ဆက်ပြီး",
      supabaseNotConfigured: "Supabase မသတ်မှတ်ရသေး", sdkMissing: "SDK မတင်ရသေး", signInSuccess: "ဝင်ရောက်မှု အောင်မြင်သည်",
      signInFailed: "ဝင်ရောက်မှု မအောင်မြင်ပါ", checkEmail: "အီးမေးလ်ကို စစ်ဆေးပါ", noLearningSync: "သင်ယူမှုဒေတာကို လောလောဆယ် မစင့်ခ်ပါ",
      emailRequired: "အီးမေးလ်ထည့်ပါ", credentialsRequired: "အီးမေးလ်နှင့် စကားဝှက်ထည့်ပါ"
    },
    "ko-KR": {
      magicLinkTitle: "Magic Link로 로그인", sendMagicLink: "로그인 링크 보내기", email: "이메일",
      password: "비밀번호", passwordTest: "이메일/비밀번호 로그인(테스트)", passwordSignIn: "비밀번호로 로그인(테스트)",
      signOut: "로그아웃", currentUser: "현재 사용자", supabaseConnected: "Supabase 연결됨",
      supabaseNotConfigured: "Supabase 미설정", sdkMissing: "SDK 미로드", signInSuccess: "로그인 성공",
      signInFailed: "로그인 실패", checkEmail: "이메일을 확인하세요", noLearningSync: "현재 학습 데이터는 동기화되지 않습니다",
      emailRequired: "이메일을 입력하세요", credentialsRequired: "이메일과 비밀번호를 입력하세요"
    }
  };

  Object.keys(AUTH_ROUND_17_5).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_ROUND_17_5[locale]);
    }
  });

  var SYNC_ROUND_17_6 = {
    "zh-CN": {
      syncNow: "立即同步", syncingNow: "正在同步", syncSuccess: "同步成功", syncFailed: "同步失败",
      signInFirst: "请先登录", supabaseNotConfigured: "Supabase 未配置", lastSync: "上次同步",
      pendingSync: "待同步", syncScope: "只同步学习进度和设置", noAiKeyUpload: "不会上传 AI Key",
      manualSyncOnly: "仅在点击“立即同步”后同步"
    },
    "ja-JP": {
      syncNow: "今すぐ同期", syncingNow: "同期中", syncSuccess: "同期成功", syncFailed: "同期失敗",
      signInFirst: "先にログインしてください", supabaseNotConfigured: "Supabase は未設定です", lastSync: "最終同期",
      pendingSync: "同期待ち", syncScope: "学習進捗と設定のみ同期します", noAiKeyUpload: "AI Key はアップロードしません",
      manualSyncOnly: "「今すぐ同期」を押した場合のみ同期します"
    },
    "en-US": {
      syncNow: "Sync now", syncingNow: "Syncing", syncSuccess: "Sync successful", syncFailed: "Sync failed",
      signInFirst: "Please sign in first", supabaseNotConfigured: "Supabase not configured", lastSync: "Last sync",
      pendingSync: "Pending", syncScope: "Only learning progress and settings are synced", noAiKeyUpload: "AI keys are never uploaded",
      manualSyncOnly: "Sync runs only when you click Sync now"
    },
    "vi-VN": {
      syncNow: "Đồng bộ ngay", syncingNow: "Đang đồng bộ", syncSuccess: "Đồng bộ thành công", syncFailed: "Đồng bộ thất bại",
      signInFirst: "Vui lòng đăng nhập trước", supabaseNotConfigured: "Supabase chưa được cấu hình", lastSync: "Lần đồng bộ cuối",
      pendingSync: "Đang chờ", syncScope: "Chỉ đồng bộ tiến độ học và cài đặt", noAiKeyUpload: "Không tải AI Key lên",
      manualSyncOnly: "Chỉ đồng bộ khi bạn bấm Đồng bộ ngay"
    },
    "fr-FR": {
      syncNow: "Synchroniser", syncingNow: "Synchronisation", syncSuccess: "Synchronisation réussie", syncFailed: "Échec de la synchronisation",
      signInFirst: "Connectez-vous d'abord", supabaseNotConfigured: "Supabase non configuré", lastSync: "Dernière synchronisation",
      pendingSync: "En attente", syncScope: "Seuls la progression et les réglages sont synchronisés", noAiKeyUpload: "Les clés IA ne sont jamais envoyées",
      manualSyncOnly: "La synchronisation démarre uniquement avec le bouton Synchroniser"
    },
    "my-MM": {
      syncNow: "ယခု စင့်ခ်လုပ်ရန်", syncingNow: "စင့်ခ်လုပ်နေသည်", syncSuccess: "စင့်ခ်အောင်မြင်သည်", syncFailed: "စင့်ခ်မအောင်မြင်ပါ",
      signInFirst: "ဦးစွာ အကောင့်ဝင်ပါ", supabaseNotConfigured: "Supabase ကို မသတ်မှတ်ရသေးပါ", lastSync: "နောက်ဆုံး စင့်ခ်",
      pendingSync: "စောင့်ဆိုင်းနေသည်", syncScope: "သင်ယူမှုတိုးတက်မှုနှင့် ဆက်တင်များကိုသာ စင့်ခ်လုပ်သည်", noAiKeyUpload: "AI Key ကို မတင်ပါ",
      manualSyncOnly: "ယခု စင့်ခ်လုပ်ရန်ကို နှိပ်မှသာ စင့်ခ်လုပ်သည်"
    },
    "ko-KR": {
      syncNow: "지금 동기화", syncingNow: "동기화 중", syncSuccess: "동기화 성공", syncFailed: "동기화 실패",
      signInFirst: "먼저 로그인하세요", supabaseNotConfigured: "Supabase가 구성되지 않음", lastSync: "마지막 동기화",
      pendingSync: "대기 중", syncScope: "학습 진도와 설정만 동기화합니다", noAiKeyUpload: "AI Key를 업로드하지 않습니다",
      manualSyncOnly: "지금 동기화를 눌렀을 때만 동기화합니다"
    }
  };

  var SYNC_ROUND_17_9 = {
    "zh-CN": {
      alreadySyncing: "正在同步，请稍候",
      syncFailedDetail: "部分同步失败，请重试",
      progressPushed: "进度上传",
      progressPulled: "进度下载",
      quizPushed: "测验结果",
      conflictsResolved: "已合并远程进度",
      mergedRemote: "已合并其他设备的进度",
      localKept: "保留本地进度",
      noDataToSync: "没有需要同步的数据",
      syncSummaryTitle: "本次同步摘要",
      noAiDataUpload: "不会上传 AI 数据",
    },
    "ja-JP": {
      alreadySyncing: "同期中です。お待ちください",
      syncFailedDetail: "一部の同期に失敗しました。もう一度お試しください",
      progressPushed: "進捗アップロード",
      progressPulled: "進捗ダウンロード",
      quizPushed: "クイズ結果",
      conflictsResolved: "リモートの進捗をマージしました",
      mergedRemote: "他のデバイスの進捗をマージしました",
      localKept: "ローカル進捗を保持",
      noDataToSync: "同期するデータはありません",
      syncSummaryTitle: "今回の同期サマリー",
      noAiDataUpload: "AI データはアップロードしません",
    },
    "en-US": {
      alreadySyncing: "Syncing, please wait",
      syncFailedDetail: "Some sync steps failed, please retry",
      progressPushed: "Progress pushed",
      progressPulled: "Progress pulled",
      quizPushed: "Quiz results",
      conflictsResolved: "Merged remote progress",
      mergedRemote: "Merged progress from other devices",
      localKept: "Local progress preserved",
      noDataToSync: "No data to sync",
      syncSummaryTitle: "Sync summary",
      noAiDataUpload: "AI data is never uploaded",
    },
    "vi-VN": {
      alreadySyncing: "Đang đồng bộ, vui lòng chờ",
      syncFailedDetail: "Một số bước đồng bộ thất bại, vui lòng thử lại",
      progressPushed: "Tiến độ đã tải lên",
      progressPulled: "Tiến độ đã tải xuống",
      quizPushed: "Kết quả quiz",
      conflictsResolved: "Đã hợp nhất tiến độ từ xa",
      mergedRemote: "Đã hợp nhất tiến độ từ thiết bị khác",
      localKept: "Giữ lại tiến độ cục bộ",
      noDataToSync: "Không có dữ liệu để đồng bộ",
      syncSummaryTitle: "Tóm tắt đồng bộ",
      noAiDataUpload: "Không tải dữ liệu AI lên",
    },
    "fr-FR": {
      alreadySyncing: "Synchronisation en cours, veuillez patienter",
      syncFailedDetail: "Certaines étapes ont échoué, veuillez réessayer",
      progressPushed: "Progression envoyée",
      progressPulled: "Progression reçue",
      quizPushed: "Résultats des quiz",
      conflictsResolved: "Progression distante fusionnée",
      mergedRemote: "Progression fusionnée depuis d'autres appareils",
      localKept: "Progression locale conservée",
      noDataToSync: "Aucune donnée à synchroniser",
      syncSummaryTitle: "Résumé de la synchronisation",
      noAiDataUpload: "Les données IA ne sont jamais envoyées",
    },
    "my-MM": {
      alreadySyncing: "စင့်ခ်လုပ်နေသည်၊ ခဏစောင့်ပါ",
      syncFailedDetail: "စင့်ခ်လုပ်ရန် မအောင်မြင်ပါ၊ ထပ်စမ်းပါ",
      progressPushed: "တိုးတက်မှု တင်ပြီး",
      progressPulled: "တိုးတက်မှု ရယူပြီး",
      quizPushed: "မေးခွန်းရလဒ်များ",
      conflictsResolved: "အဝေးမှ တိုးတက်မှုကို ပေါင်းစည်းပြီး",
      mergedRemote: "အခြားစက်မှ တိုးတက်မှုကို ပေါင်းစည်းပြီး",
      localKept: "ဒေသတွင်း တိုးတက်မှုကို ထိန်းသိမ်းထားသည်",
      noDataToSync: "စင့်ခ်လုပ်ရန် ဒေတာမရှိပါ",
      syncSummaryTitle: "စင့်ခ်အကျဉ်းချုပ်",
      noAiDataUpload: "AI ဒေတာကို မတင်ပါ",
    },
    "ko-KR": {
      alreadySyncing: "동기화 중입니다. 잠시 기다려주세요",
      syncFailedDetail: "일부 동기화에 실패했습니다. 다시 시도하세요",
      progressPushed: "진행 상황 업로드",
      progressPulled: "진행 상황 다운로드",
      quizPushed: "퀴즈 결과",
      conflictsResolved: "원격 진행 상황 병합 완료",
      mergedRemote: "다른 기기의 진행 상황 병합 완료",
      localKept: "로컬 진행 상황 유지",
      noDataToSync: "동기화할 데이터가 없습니다",
      syncSummaryTitle: "동기화 요약",
      noAiDataUpload: "AI 데이터를 업로드하지 않습니다",
    },
  };

  Object.keys(SYNC_ROUND_17_6).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, SYNC_ROUND_17_6[locale]);
    }
  });

  // Round 17.9 sync UX i18n additions
  Object.keys(SYNC_ROUND_17_9).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, SYNC_ROUND_17_9[locale]);
    }
  });

  // Round 17.10 auth UI polyglot and layout slimming translations
  var AUTH_SYNC_ROUND_17_10 = {
    "zh-CN": {
      account: "账号",
      localMode: "本地模式",
      signedIn: "已登录",
      notLoggedIn: "未登录",
      currentUser: "当前用户",
      supabaseConnected: "Supabase 已连接",
      supabaseNotConfigured: "Supabase 未配置",
      sdkMissing: "SDK 未加载",
      sync: "同步",
      syncNow: "立即同步",
      syncingNow: "正在同步",
      syncSuccess: "同步成功",
      syncFailed: "同步失败",
      lastSync: "上次同步",
      pendingSync: "待同步",
      syncSummaryTitle: "本次同步摘要",
      conflictsResolved: "已合并远端进度",
      localKept: "保留本地进度",
      syncScope: "只同步学习进度和设置",
      noAiKeyUpload: "不会上传 AI Key",
      magicLinkTitle: "使用 Magic Link 登录",
      sendMagicLink: "发送登录链接",
      email: "邮箱",
      password: "密码",
      passwordTest: "密码登录 (测试)",
      passwordSignIn: "登录",
      signOut: "登出",
      continueLocal: "继续本地使用",
      mockSignIn: "模拟登录状态",
      mockSignOut: "退出模拟登录",
      close: "关闭",
      signInFirst: "请先登录",
      noAutoSync: "当前不会自动同步",
      syncIntro: "多设备同步进度及设置",
      deviceId: "设备 ID"
    },
    "ja-JP": {
      account: "アカウント",
      localMode: "ローカルモード",
      signedIn: "ログイン済み",
      notLoggedIn: "未ログイン",
      currentUser: "現在のユーザー",
      supabaseConnected: "Supabase 接続済み",
      supabaseNotConfigured: "Supabase 未設定",
      sdkMissing: "SDK 未読み込み",
      sync: "同期",
      syncNow: "今すぐ同期",
      syncingNow: "同期中",
      syncSuccess: "同期成功",
      syncFailed: "同期失敗",
      lastSync: "最終同期",
      pendingSync: "同期待ち",
      syncSummaryTitle: "今回の同期サマリー",
      conflictsResolved: "リモートの進捗をマージしました",
      localKept: "ローカル進捗を保持",
      syncScope: "学習進捗と設定のみ同期します",
      noAiKeyUpload: "AI Key はアップロードしません",
      magicLinkTitle: "Magic Link でログイン",
      sendMagicLink: "ログインリンクを送信",
      email: "メール",
      password: "パスワード",
      passwordTest: "パスワードログイン (テスト)",
      passwordSignIn: "ログイン",
      signOut: "ログアウト",
      continueLocal: "ローカル利用を継続",
      mockSignIn: "ログインを模擬",
      mockSignOut: "模擬ログイン終了",
      close: "閉じる",
      signInFirst: "先にログインしてください",
      noAutoSync: "自動同期は行われません",
      syncIntro: "複数端末で進捗と設定を同期",
      deviceId: "デバイス ID"
    },
    "en-US": {
      account: "Account",
      localMode: "Local Mode",
      signedIn: "Signed In",
      notLoggedIn: "Not Logged In",
      currentUser: "Current User",
      supabaseConnected: "Supabase Connected",
      supabaseNotConfigured: "Supabase Not Configured",
      sdkMissing: "SDK Not Loaded",
      sync: "Sync",
      syncNow: "Sync Now",
      syncingNow: "Syncing",
      syncSuccess: "Sync Successful",
      syncFailed: "Sync Failed",
      lastSync: "Last Sync",
      pendingSync: "Pending",
      syncSummaryTitle: "Sync Summary",
      conflictsResolved: "Merged Remote Progress",
      localKept: "Local Progress Preserved",
      syncScope: "Syncs only progress & settings",
      noAiKeyUpload: "AI keys are never uploaded",
      magicLinkTitle: "Sign in with Magic Link",
      sendMagicLink: "Send Sign-in Link",
      email: "Email",
      password: "Password",
      passwordTest: "Password Sign-in (Test)",
      passwordSignIn: "Sign In",
      signOut: "Sign Out",
      continueLocal: "Continue Locally",
      mockSignIn: "Mock Sign In",
      mockSignOut: "Exit Mock",
      close: "Close",
      signInFirst: "Please sign in first",
      noAutoSync: "No automatic sync",
      syncIntro: "Sync progress & settings across devices",
      deviceId: "Device ID"
    },
    "vi-VN": {
      account: "Tài khoản",
      localMode: "Chế độ cục bộ",
      signedIn: "Đã đăng nhập",
      notLoggedIn: "Chưa đăng nhập",
      currentUser: "Người dùng hiện tại",
      supabaseConnected: "Đã kết nối Supabase",
      supabaseNotConfigured: "Chưa cấu hình Supabase",
      sdkMissing: "SDK chưa tải",
      sync: "Đồng bộ",
      syncNow: "Đồng bộ ngay",
      syncingNow: "Đang đồng bộ",
      syncSuccess: "Đồng bộ thành công",
      syncFailed: "Đồng bộ thất bại",
      lastSync: "Lần đồng bộ cuối",
      pendingSync: "Đang chờ",
      syncSummaryTitle: "Tóm tắt đồng bộ",
      conflictsResolved: "Đã hợp nhất tiến độ từ xa",
      localKept: "Giữ lại tiến độ cục bộ",
      syncScope: "Chỉ đồng bộ tiến độ học & cài đặt",
      noAiKeyUpload: "Không tải AI Key lên",
      magicLinkTitle: "Đăng nhập bằng Magic Link",
      sendMagicLink: "Gửi liên kết đăng nhập",
      email: "Email",
      password: "Mật khẩu",
      passwordTest: "Đăng nhập mật khẩu (Thử)",
      passwordSignIn: "Đăng nhập",
      signOut: "Đăng xuất",
      continueLocal: "Tiếp tục dùng cục bộ",
      mockSignIn: "Giả lập đăng nhập",
      mockSignOut: "Thoát giả lập",
      close: "Đóng",
      signInFirst: "Vui lòng đăng nhập trước",
      noAutoSync: "Không tự động đồng bộ",
      syncIntro: "Đồng bộ tiến độ & cài đặt trên nhiều thiết bị",
      deviceId: "Mã thiết bị"
    },
    "fr-FR": {
      account: "Compte",
      localMode: "Mode local",
      signedIn: "Connecté",
      notLoggedIn: "Non connecté",
      currentUser: "Utilisateur actuel",
      supabaseConnected: "Supabase connecté",
      supabaseNotConfigured: "Supabase non configuré",
      sdkMissing: "SDK non chargé",
      sync: "Synchro",
      syncNow: "Synchroniser",
      syncingNow: "Synchronisation",
      syncSuccess: "Synchro réussie",
      syncFailed: "Synchro échouée",
      lastSync: "Dernière synchro",
      pendingSync: "En attente",
      syncSummaryTitle: "Résumé synchro",
      conflictsResolved: "Progression distante fusionnée",
      localKept: "Progression locale conservée",
      syncScope: "Progression et réglages uniquement",
      noAiKeyUpload: "Clés IA jamais envoyées",
      magicLinkTitle: "Connexion via Magic Link",
      sendMagicLink: "Envoyer le lien",
      email: "E-mail",
      password: "Mot de passe",
      passwordTest: "Connexion mot de passe (Test)",
      passwordSignIn: "Connexion",
      signOut: "Déconnexion",
      continueLocal: "Continuer en local",
      mockSignIn: "Simuler connexion",
      mockSignOut: "Quitter simulation",
      close: "Fermer",
      signInFirst: "Connectez-vous d'abord",
      noAutoSync: "Pas de synchro auto",
      syncIntro: "Synchro de la progression multi-appareil",
      deviceId: "ID de l'appareil"
    },
    "my-MM": {
      account: "အကောင့်",
      localMode: "ဒေသတွင်း မုဒ်",
      signedIn: "ဝင်ရောက်ပြီး",
      notLoggedIn: "မဝင်ရသေးပါ",
      currentUser: "လက်ရှိအသုံးပြုသူ",
      supabaseConnected: "Supabase ချိတ်ဆက်ပြီး",
      supabaseNotConfigured: "Supabase မသတ်မှတ်ရသေး",
      sdkMissing: "SDK မတင်ရသေး",
      sync: "စင့်ခ်လုပ်ခြင်း",
      syncNow: "ယခု စင့်ခ်လုပ်ရန်",
      syncingNow: "စင့်ခ်လုပ်နေသည်",
      syncSuccess: "စင့်ခ်အောင်မြင်သည်",
      syncFailed: "စင့်ခ်မအောင်မြင်ပါ",
      lastSync: "နောက်ဆုံး စင့်ခ်",
      pendingSync: "စောင့်ဆိုင်းနေသည်",
      syncScope: "သင်ယူမှုတိုးတက်မှုနှင့် ဆက်တင်များသာ",
      noAiKeyUpload: "AI Key ကို မတင်ပါ",
      magicLinkTitle: "Magic Link ဖြင့် ဝင်မည်",
      sendMagicLink: "ဝင်ရောက်ရန် လင့်ခ်ပို့မည်",
      email: "အီးမေးလ်",
      password: "စကားဝှက်",
      passwordTest: "စကားဝှက်ဖြင့်ဝင်ရန် (စမ်းသပ်)",
      passwordSignIn: "ဝင်ရောက်ရန်",
      signOut: "ထွက်မည်",
      continueLocal: "ဒေသတွင်း ဆက်သုံးမည်",
      mockSignIn: "စမ်းသပ်ဝင်မည်",
      mockSignOut: "စမ်းသပ်ထွက်မည်",
      close: "ပိတ်ရန်",
      signInFirst: "ဦးစွာ ဝင်ရောက်ပါ",
      noAutoSync: "အလိုအလျောက် စင့်ခ်မလုပ်ပါ",
      syncIntro: "စက်ပစ္စည်းအများအပြားတွင် စင့်ခ်လုပ်ရန်",
      deviceId: "စက်ပစ္စည်း ID"
    },
    "ko-KR": {
      account: "계정",
      localMode: "로컬 모드",
      signedIn: "로그인됨",
      notLoggedIn: "로그인하지 않음",
      currentUser: "현재 사용자",
      supabaseConnected: "Supabase 연결됨",
      supabaseNotConfigured: "Supabase 미설정",
      sdkMissing: "SDK 미로드",
      sync: "동기화",
      syncNow: "지금 동기화",
      syncingNow: "동기화 중",
      syncSuccess: "동기화 성공",
      syncFailed: "동기화 실패",
      lastSync: "마지막 동기화",
      pendingSync: "대기 중",
      syncSummaryTitle: "동기화 요약",
      conflictsResolved: "원격 진행 상황 병합 완료",
      localKept: "로컬 진행 상황 유지",
      syncScope: "학습 진도와 설정만 동기화",
      noAiKeyUpload: "AI Key를 업로드하지 않습니다",
      magicLinkTitle: "Magic Link로 로그인",
      sendMagicLink: "로그인 링크 보내기",
      email: "이메일",
      password: "비밀번호",
      passwordTest: "비밀번호 로그인(테스트)",
      passwordSignIn: "로그인",
      signOut: "로그아웃",
      continueLocal: "로컬 사용 계속",
      mockSignIn: "로그인 시뮬레이션",
      mockSignOut: "시뮬레이션 종료",
      close: "닫기",
      signInFirst: "먼저 로그인하세요",
      noAutoSync: "자동 동기화 없음",
      syncIntro: "여러 기기에서 진도 및 설정 동기화",
      deviceId: "기기 ID"
    }
  };

  var AUTH_SYNC_ROUND_19_1 = {
    "zh-CN": {
      bookmarksPushed: "收藏上传",
      bookmarksPulled: "收藏下载",
      bookmarksMerged: "收藏已合并",
      onlySyncTypingBookmarks: "当前仅同步日语打字收藏",
      noAiCacheUpload: "不会上传 AI 翻译缓存"
    },
    "ja-JP": {
      bookmarksPushed: "お気に入りアップロード",
      bookmarksPulled: "お気に入りダウンロード",
      bookmarksMerged: "お気に入りマージ完了",
      onlySyncTypingBookmarks: "現在は日本語タイピングのお気に入りのみ同期します",
      noAiCacheUpload: "AI翻訳キャッシュはアップロードしません"
    },
    "en-US": {
      bookmarksPushed: "Favorites uploaded",
      bookmarksPulled: "Favorites downloaded",
      bookmarksMerged: "Favorites merged",
      onlySyncTypingBookmarks: "Currently only syncing Japanese typing favorites",
      noAiCacheUpload: "AI translation cache is never uploaded"
    },
    "vi-VN": {
      bookmarksPushed: "Đã tải lên yêu thích",
      bookmarksPulled: "Đã tải xuống yêu thích",
      bookmarksMerged: "Đã gộp yêu thích",
      onlySyncTypingBookmarks: "Hiện tại chỉ đồng bộ hóa các yêu thích gõ tiếng Nhật",
      noAiCacheUpload: "Không bao giờ tải lên bộ nhớ đệm dịch AI"
    },
    "fr-FR": {
      bookmarksPushed: "Favoris téléversés",
      bookmarksPulled: "Favoris téléchargés",
      bookmarksMerged: "Favoris fusionnés",
      onlySyncTypingBookmarks: "Actuellement, seule la synchronisation des favoris de dactylographie japonaise est prise en charge",
      noAiCacheUpload: "Le cache de traduction IA n'est jamais téléversé"
    },
    "my-MM": {
      bookmarksPushed: "အကြိုက်ဆုံးများ စင့်ခ်လုပ်ခဲ့သည်",
      bookmarksPulled: "အကြိုက်ဆုံးများ ရယူခဲ့သည်",
      bookmarksMerged: "အကြိုက်ဆုံးများ ပေါင်းစည်းခဲ့သည်",
      onlySyncTypingBookmarks: "လက်ရှိတွင် ဂျပန်စာရိုက်အကြိုက်ဆုံးများကိုသာ စင့်ခ်လုပ်နိုင်ပါသည်",
      noAiCacheUpload: "AI ဘာသာပြန်ကက်ရှ်ကို လုံးဝတင်မည်မဟုတ်ပါ"
    },
    "ko-KR": {
      bookmarksPushed: "즐겨찾기 업로드됨",
      bookmarksPulled: "즐겨찾기 다운로드됨",
      bookmarksMerged: "즐겨찾기 병합됨",
      onlySyncTypingBookmarks: "현재 일본어 타자 즐겨찾기만 동기화됩니다",
      noAiCacheUpload: "AI 번역 캐시는 업로드되지 않습니다"
    }
  };

  Object.keys(AUTH_SYNC_ROUND_17_10).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_17_10[locale]);
    }
  });

  Object.keys(AUTH_SYNC_ROUND_19_1).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_19_1[locale]);
    }
  });

  var AUTH_SYNC_ROUND_19_2 = {
    "zh-CN": {
      deviceAndSyncDetails: "设备与同步详情",
      passwordTestTitle: "使用密码登录 (仅测试/开发)"
    },
    "ja-JP": {
      deviceAndSyncDetails: "デバイス・同期詳細",
      passwordTestTitle: "パスワードでログイン (テスト・開発専用)"
    },
    "en-US": {
      deviceAndSyncDetails: "Device & Sync Details",
      passwordTestTitle: "Sign in with password (test/dev only)"
    },
    "vi-VN": {
      deviceAndSyncDetails: "Chi tiết thiết bị & đồng bộ",
      passwordTestTitle: "Đăng nhập bằng mật khẩu (chỉ dùng thử/phát triển)"
    },
    "fr-FR": {
      deviceAndSyncDetails: "Détails de l'appareil et de la synchronisation",
      passwordTestTitle: "Connexion avec mot de passe (test/dév uniquement)"
    },
    "my-MM": {
      deviceAndSyncDetails: "စက်ပစ္စည်းနှင့် စင့်ခ်လုပ်ခြင်း အသေးစိတ်",
      passwordTestTitle: "စကားဝှက်ဖြင့် ဝင်ရောက်ရန် (စမ်းသပ်/ဖွံ့ဖြိုးမှုသာ)"
    },
    "ko-KR": {
      deviceAndSyncDetails: "기기 및 동기화 상세 정보",
      passwordTestTitle: "비밀번호로 로그인 (테스트/개발용)"
    }
  };

  Object.keys(AUTH_SYNC_ROUND_19_2).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_19_2[locale]);
    }
  });
 
   var AUTH_SYNC_ROUND_19_5 = {
     "zh-CN": {
       bookmarksDeletedSynced: "收藏删除已同步",
       bookmarksRestored: "收藏已恢复",
       bookmarksConflictsResolved: "收藏冲突已处理",
       bookmarksSoftDelete: "删除同步仅为软删除",
       bookmarksNoClear: "不会清空本地收藏"
     },
     "ja-JP": {
       bookmarksDeletedSynced: "お気に入り削除を同期しました",
       bookmarksRestored: "お気に入りを復元しました",
       bookmarksConflictsResolved: "お気に入りの競合を解決しました",
       bookmarksSoftDelete: "削除同期はソフトデリートのみです",
       bookmarksNoClear: "ローカルのお気に入りは空になりません"
     },
     "en-US": {
       bookmarksDeletedSynced: "Bookmark deletions synced",
       bookmarksRestored: "Bookmarks restored",
       bookmarksConflictsResolved: "Bookmark conflicts resolved",
       bookmarksSoftDelete: "Delete sync is soft-delete only",
       bookmarksNoClear: "Local bookmarks will not be cleared"
     },
     "vi-VN": {
       bookmarksDeletedSynced: "Đã đồng bộ xóa yêu thích",
       bookmarksRestored: "Đã khôi phục yêu thích",
       bookmarksConflictsResolved: "Đã giải quyết xung đột yêu thích",
       bookmarksSoftDelete: "Đồng bộ xóa chỉ là xóa mềm",
       bookmarksNoClear: "Sẽ không xóa yêu thích cục bộ"
     },
     "fr-FR": {
       bookmarksDeletedSynced: "Suppressions de favoris synchronisées",
       bookmarksRestored: "Favoris restaurés",
       bookmarksConflictsResolved: "Conflits de favoris résolus",
       bookmarksSoftDelete: "La synchronisation des suppressions est une suppression logicielle uniquement",
       bookmarksNoClear: "Les favoris locaux ne seront pas effacés"
     },
     "my-MM": {
       bookmarksDeletedSynced: "စာညှပ်များ ဖျက်ခြင်းကို ထပ်တူပြုပြီး",
       bookmarksRestored: "စာညှပ်များ ပြန်လည်ရယူပြီး",
       bookmarksConflictsResolved: "စာညှပ် ကွဲလွဲမှုများကို ဖြေရှင်းပြီး",
       bookmarksSoftDelete: "ဖျက်ခြင်းထပ်တူပြုခြင်းသည် ပျော့ဖျက်ခြင်းသာဖြစ်သည်",
       bookmarksNoClear: "ဒေသခံ စာညှပ်များကို ရှင်းလင်းမည်မဟုတ်ပါ"
     },
     "ko-KR": {
       bookmarksDeletedSynced: "즐겨찾기 삭제 동기화 완료",
       bookmarksRestored: "즐겨찾기 복원됨",
       bookmarksConflictsResolved: "즐겨찾기 충돌 해결됨",
       bookmarksSoftDelete: "삭제 동기화는 소프트 삭제만 가능합니다",
       bookmarksNoClear: "로컬 즐겨찾기는 지워지지 않습니다"
     }
   };
 
   Object.keys(AUTH_SYNC_ROUND_19_5).forEach(function (locale) {
     if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
       Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_19_5[locale]);
     }
   });

  var AUTH_SYNC_ROUND_23_8 = {
    "zh-CN": {
      wrongBookPushed: "错题上传",
      wrongBookPulled: "错题下载",
      wrongBookMerged: "错题已合并",
      wrongBookSkipped: "错题已跳过",
      wrongBookFailed: "错题同步失败"
    },
    "ja-JP": {
      wrongBookPushed: "間違いノートアップロード",
      wrongBookPulled: "間違いノートダウンロード",
      wrongBookMerged: "間違いノートをマージしました",
      wrongBookSkipped: "間違いノートをスキップしました",
      wrongBookFailed: "間違いノートの同期に失敗しました"
    },
    "en-US": {
      wrongBookPushed: "Wrong book uploaded",
      wrongBookPulled: "Wrong book downloaded",
      wrongBookMerged: "Wrong book merged",
      wrongBookSkipped: "Wrong book skipped",
      wrongBookFailed: "Wrong book sync failed"
    },
    "vi-VN": {
      wrongBookPushed: "Đã tải lên sổ sai",
      wrongBookPulled: "Đã tải xuống sổ sai",
      wrongBookMerged: "Đã gộp sổ sai",
      wrongBookSkipped: "Đã bỏ qua sổ sai",
      wrongBookFailed: "Đồng bộ sổ sai thất bại"
    },
    "fr-FR": {
      wrongBookPushed: "Cahier d'erreurs téléversé",
      wrongBookPulled: "Cahier d'erreurs téléchargé",
      wrongBookMerged: "Cahier d'erreurs fusionné",
      wrongBookSkipped: "Cahier d'erreurs ignoré",
      wrongBookFailed: "Échec de la synchronisation du cahier d'erreurs"
    },
    "my-MM": {
      wrongBookPushed: "မှားယွင်းမှုစာအုပ် စင့်ခ်လုပ်ခဲ့သည်",
      wrongBookPulled: "မှားယွင်းမှုစာအုပ် ဒေါင်းလုဒ်လုပ်ခဲ့သည်",
      wrongBookMerged: "မှားယွင်းမှုစာအုပ် ပေါင်းစည်းပြီး",
      wrongBookSkipped: "မှားယွင်းမှုစာအုပ် ကျော်လွှားခဲ့သည်",
      wrongBookFailed: "မှားယွင်းမှုစာအုပ် ထပ်တူပြုခြင်း မအောင်မြင်ပါ"
    },
    "ko-KR": {
      wrongBookPushed: "오답노트 업로드됨",
      wrongBookPulled: "오답노트 다운로드됨",
      wrongBookMerged: "오답노트 병합됨",
      wrongBookSkipped: "오답노트 건너뜀",
      wrongBookFailed: "오답노트 동기화 실패"
    }
  };

  Object.keys(AUTH_SYNC_ROUND_23_8).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_23_8[locale]);
    }
  });

  var AUTH_SYNC_ROUND_23_12 = {
    "zh-CN": {
      wrongBookRetrySettingsPushed: "错题重练设置已上传",
      wrongBookRetrySettingsPulled: "错题重练设置已下载",
      wrongBookRetrySettingsMerged: "错题重练设置已合并",
      wrongBookRetrySettingsFailed: "错题重练设置同步失败"
    },
    "ja-JP": {
      wrongBookRetrySettingsPushed: "間違いノート再練習設定をアップロードしました",
      wrongBookRetrySettingsPulled: "間違いノート再練習設定をダウンロードしました",
      wrongBookRetrySettingsMerged: "間違いノート再練習設定をマージしました",
      wrongBookRetrySettingsFailed: "間違いノート再練習設定の同期に失敗しました"
    },
    "en-US": {
      wrongBookRetrySettingsPushed: "Wrong-book retry settings uploaded",
      wrongBookRetrySettingsPulled: "Wrong-book retry settings downloaded",
      wrongBookRetrySettingsMerged: "Wrong-book retry settings merged",
      wrongBookRetrySettingsFailed: "Wrong-book retry settings sync failed"
    },
    "vi-VN": {
      wrongBookRetrySettingsPushed: "Đã tải lên cài đặt luyện lại câu sai",
      wrongBookRetrySettingsPulled: "Đã tải xuống cài đặt luyện lại câu sai",
      wrongBookRetrySettingsMerged: "Đã hợp nhất cài đặt luyện lại câu sai",
      wrongBookRetrySettingsFailed: "Đồng bộ cài đặt luyện lại câu sai thất bại"
    },
    "fr-FR": {
      wrongBookRetrySettingsPushed: "Paramètres de révision des erreurs téléversés",
      wrongBookRetrySettingsPulled: "Paramètres de révision des erreurs téléchargés",
      wrongBookRetrySettingsMerged: "Paramètres de révision des erreurs fusionnés",
      wrongBookRetrySettingsFailed: "Échec de la synchronisation des paramètres de révision"
    },
    "my-MM": {
      wrongBookRetrySettingsPushed: "အမှားစာအုပ် ပြန်လေ့ကျင့်မှု ဆက်တင်များ အပ်လုဒ်ပြီး",
      wrongBookRetrySettingsPulled: "အမှားစာအုပ် ပြန်လေ့ကျင့်မှု ဆက်တင်များ ဒေါင်းလုဒ်ပြီး",
      wrongBookRetrySettingsMerged: "အမှားစာအုပ် ပြန်လေ့ကျင့်မှု ဆက်တင်များ ပေါင်းစည်းပြီး",
      wrongBookRetrySettingsFailed: "အမှားစာအုပ် ပြန်လေ့ကျင့်မှု ဆက်တင်များ စင့်ခ်မအောင်မြင်ပါ"
    },
    "ko-KR": {
      wrongBookRetrySettingsPushed: "오답 재연습 설정 업로드 완료",
      wrongBookRetrySettingsPulled: "오답 재연습 설정 다운로드 완료",
      wrongBookRetrySettingsMerged: "오답 재연습 설정 병합 완료",
      wrongBookRetrySettingsFailed: "오답 재연습 설정 동기화 실패"
    }
  };

  Object.keys(AUTH_SYNC_ROUND_23_12).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_23_12[locale]);
    }
  });

  // Round 25.0 typing & exam sync labels (zh-CN, ja-JP, en-US, ko-KR)
  var AUTH_SYNC_ROUND_25_0 = {
    "zh-CN": {
      typingSessionsPushed: "打字练习上传",
      typingSessionsPulled: "打字练习下载",
      examSessionsPushed: "考试记录上传",
      examSessionsPulled: "考试记录下载"
    },
    "ja-JP": {
      typingSessionsPushed: "タイピング練習アップロード",
      typingSessionsPulled: "タイピング練習ダウンロード",
      examSessionsPushed: "試験記録アップロード",
      examSessionsPulled: "試験記録ダウンロード"
    },
    "en-US": {
      typingSessionsPushed: "Typing sessions uploaded",
      typingSessionsPulled: "Typing sessions downloaded",
      examSessionsPushed: "Exam records uploaded",
      examSessionsPulled: "Exam records downloaded"
    },
    "ko-KR": {
      typingSessionsPushed: "타이핑 연습 업로드",
      typingSessionsPulled: "타이핑 연습 다운로드",
      examSessionsPushed: "시험 기록 업로드",
      examSessionsPulled: "시험 기록 다운로드"
    }
  };

  Object.keys(AUTH_SYNC_ROUND_25_0).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_ROUND_25_0[locale]);
    }
  });

  var USER_TRANSLATIONS_ROUND_20_2 = {
    "zh-CN": {
      userTranslationSave: "保存",
      userTranslationCancel: "取消",
      userTranslationReset: "重置",
      userTranslationEdit: "编辑翻译",
      userTranslationCustomLabel: "自定义",
      userTranslationTooLong: "翻译内容过长",
      userTranslationInvalid: "翻译不能为空",
      userTranslationTextareaPlaceholder: "请输入翻译..."
    },
    "ja-JP": {
      userTranslationSave: "保存",
      userTranslationCancel: "キャンセル",
      userTranslationReset: "リセット",
      userTranslationEdit: "翻訳を編集",
      userTranslationCustomLabel: "カスタム",
      userTranslationTooLong: "翻訳が長すぎます",
      userTranslationInvalid: "翻訳を入力してください",
      userTranslationTextareaPlaceholder: "翻訳を入力..."
    },
    "en-US": {
      userTranslationSave: "Save",
      userTranslationCancel: "Cancel",
      userTranslationReset: "Reset",
      userTranslationEdit: "Edit translation",
      userTranslationCustomLabel: "Custom",
      userTranslationTooLong: "Translation too long",
      userTranslationInvalid: "Translation cannot be empty",
      userTranslationTextareaPlaceholder: "Enter your translation..."
    },
    "vi-VN": {
      userTranslationSave: "Lưu",
      userTranslationCancel: "Hủy",
      userTranslationReset: "Đặt lại",
      userTranslationEdit: "Sửa bản dịch",
      userTranslationCustomLabel: "Tùy chỉnh",
      userTranslationTooLong: "Bản dịch quá dài",
      userTranslationInvalid: "Bản dịch không được để trống",
      userTranslationTextareaPlaceholder: "Nhập bản dịch..."
    },
    "fr-FR": {
      userTranslationSave: "Enregistrer",
      userTranslationCancel: "Annuler",
      userTranslationReset: "Réinitialiser",
      userTranslationEdit: "Modifier la traduction",
      userTranslationCustomLabel: "Personnalisé",
      userTranslationTooLong: "Traduction trop longue",
      userTranslationInvalid: "La traduction ne peut pas être vide",
      userTranslationTextareaPlaceholder: "Entrez votre traduction..."
    },
    "my-MM": {
      userTranslationSave: "သိမ်းမည်",
      userTranslationCancel: "ပယ်ဖျက်မည်",
      userTranslationReset: "မူလအတိုင်းပြန်ထားမည်",
      userTranslationEdit: "ဘာသာပြန်ချက်ကို ပြင်ဆင်မည်",
      userTranslationCustomLabel: "စိတ်ကြိုက်",
      userTranslationTooLong: "ဘာသာပြန်ချက် ရှည်လွန်းသည်",
      userTranslationInvalid: "ဘာသာပြန်ချက်ကို ထည့်သွင်းပါ",
      userTranslationTextareaPlaceholder: "ဘာသာပြန်ချက်ကို ထည့်ပါ..."
    },
    "ko-KR": {
      userTranslationSave: "저장",
      userTranslationCancel: "취소",
      userTranslationReset: "초기화",
      userTranslationEdit: "번역 편집",
      userTranslationCustomLabel: "사용자 지정",
      userTranslationTooLong: "번역이 너무 깁니다",
      userTranslationInvalid: "번역을 입력하세요",
      userTranslationTextareaPlaceholder: "번역을 입력하세요..."
    }
  };

  Object.keys(USER_TRANSLATIONS_ROUND_20_2).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].auth) {
      Object.assign(window.I18nUiDict[locale].auth, USER_TRANSLATIONS_ROUND_20_2[locale]);
    }
  });


  var CODING_TYPING_ROUND_21_4 = {
    "zh-CN": {
      level: "难度",
      difficultyFilter: "难度过滤",
      allDifficulties: "全部难度",
      historyFilter: "历史筛选",
      allLanguages: "全部语言",
      records: "条",
      totalCompleted: "总完成次数",
      averageCpm: "平均CPM",
      bestCpm: "最高CPM",
      averageAccuracy: "平均正确率",
      totalPracticeTime: "总练习时间",
      favoriteLanguage: "最常练语言",
      commonMistakes: "常错字符Top 10",
      clearHistory: "清空历史",
      clearHistoryConfirm: "确认清空编程打字历史记录？此操作不可恢复。",
      historyCleared: "历史记录已清空",
      noMistakes: "暂无错误记录"
    },
    "ja-JP": {
      level: "難易度",
      difficultyFilter: "難易度フィルター",
      allDifficulties: "すべての難易度",
      historyFilter: "履歴フィルター",
      allLanguages: "すべての言語",
      records: "件",
      totalCompleted: "総完了回数",
      averageCpm: "平均CPM",
      bestCpm: "最高CPM",
      averageAccuracy: "平均正確率",
      totalPracticeTime: "総練習時間",
      favoriteLanguage: "最も練習した言語",
      commonMistakes: "よく間違える文字Top 10",
      clearHistory: "履歴を消去",
      clearHistoryConfirm: "プログラミングタイピングの履歴を消去しますか？この操作は元に戻せません。",
      historyCleared: "履歴を消去しました",
      noMistakes: "エラー記録なし"
    },
    "en-US": {
      level: "Level",
      difficultyFilter: "Difficulty",
      allDifficulties: "All Levels",
      historyFilter: "History Filter",
      allLanguages: "All Languages",
      records: "records",
      totalCompleted: "Total Completed",
      averageCpm: "Average CPM",
      bestCpm: "Best CPM",
      averageAccuracy: "Avg Accuracy",
      totalPracticeTime: "Total Practice Time",
      favoriteLanguage: "Most Practiced",
      commonMistakes: "Common Mistakes Top 10",
      clearHistory: "Clear History",
      clearHistoryConfirm: "Clear coding typing history? This cannot be undone.",
      historyCleared: "History cleared",
      noMistakes: "No mistakes recorded"
    },
    "ko-KR": {
      level: "난이도",
      difficultyFilter: "난이도 필터",
      allDifficulties: "모든 난이도",
      historyFilter: "기록 필터",
      allLanguages: "모든 언어",
      records: "건",
      totalCompleted: "총 완료 횟수",
      averageCpm: "평균 CPM",
      bestCpm: "최고 CPM",
      averageAccuracy: "평균 정확도",
      totalPracticeTime: "총 연습 시간",
      favoriteLanguage: "가장 많이 연습한 언어",
      commonMistakes: "자주 틀리는 문자 Top 10",
      clearHistory: "기록 삭제",
      clearHistoryConfirm: "코딩 타이핑 기록을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.",
      historyCleared: "기록이 삭제되었습니다",
      noMistakes: "오류 기록 없음"
    },
    "vi-VN": {
      level: "Độ khó",
      difficultyFilter: "Lọc độ khó",
      allDifficulties: "Tất cả cấp độ",
      historyFilter: "Lọc lịch sử",
      allLanguages: "Tất cả ngôn ngữ",
      records: "bản ghi",
      totalCompleted: "Tổng số hoàn thành",
      averageCpm: "CPM trung bình",
      bestCpm: "CPM cao nhất",
      averageAccuracy: "Độ chính xác TB",
      totalPracticeTime: "Tổng thời gian luyện",
      favoriteLanguage: "Ngôn ngữ yêu thích",
      commonMistakes: "Lỗi thường gặp Top 10",
      clearHistory: "Xóa lịch sử",
      clearHistoryConfirm: "Xóa lịch sử gõ code? Không thể hoàn tác.",
      historyCleared: "Đã xóa lịch sử",
      noMistakes: "Không có lỗi"
    },
    "my-MM": {
      level: "အခက်အခဲ",
      difficultyFilter: "အခက်အခဲစစ်ထုတ်",
      allDifficulties: "အားလုံးအဆင့်",
      historyFilter: "မှတ်တမ်းစစ်ထုတ်",
      allLanguages: "ဘာသာစကားအားလုံး",
      records: "ခု",
      totalCompleted: "စုစုပေါင်းပြီးဆုံး",
      averageCpm: "ပျမ်းမျှ CPM",
      bestCpm: "အကောင်းဆုံး CPM",
      averageAccuracy: "ပျမ်းမျှတိကျမှု",
      totalPracticeTime: "စုစုပေါင်းလေ့ကျင့်ချိန်",
      favoriteLanguage: "အများဆုံးလေ့ကျင့်",
      commonMistakes: "အမှားများသောစာလုံး Top 10",
      clearHistory: "မှတ်တမ်းဖျက်မည်",
      clearHistoryConfirm: "ကုဒ်ရိုက်ခြင်းမှတ်တမ်းကိုဖျက်မလား? ပြန်ပြင်မရပါ။",
      historyCleared: "မှတ်တမ်းဖျက်ပြီး",
      noMistakes: "အမှားမရှိပါ"
    },
    "fr-FR": {
      level: "Difficulté",
      difficultyFilter: "Filtre de difficulté",
      allDifficulties: "Tous les niveaux",
      historyFilter: "Filtre historique",
      allLanguages: "Toutes les langues",
      records: "enreg.",
      totalCompleted: "Total terminé",
      averageCpm: "CPM moyen",
      bestCpm: "Meilleur CPM",
      averageAccuracy: "Précision moyenne",
      totalPracticeTime: "Temps total",
      favoriteLanguage: "Langue favorite",
      commonMistakes: "Erreurs fréquentes Top 10",
      clearHistory: "Effacer historique",
      clearHistoryConfirm: "Effacer l'historique de frappe? Irréversible.",
      historyCleared: "Historique effacé",
      noMistakes: "Aucune erreur"
    }
  };

  Object.keys(CODING_TYPING_ROUND_21_4).forEach(function (locale) {
    if (window.I18nUiDict[locale] && window.I18nUiDict[locale].codingTyping) {
      Object.assign(window.I18nUiDict[locale].codingTyping, CODING_TYPING_ROUND_21_4[locale]);
    }
  });

  var CODING_TYPING_ROUND_21_8 = {
    "zh-CN": { switchedToSandbox: "已切换到沙盒", codeSentAndOpened: "代码已发送并打开沙盒", sqlSeedIncluded: "已附带建表与测试数据并打开 SQL 沙盒" },
    "ja-JP": { switchedToSandbox: "サンドボックスに切り替えました", codeSentAndOpened: "コードを送信し、サンドボックスを開きました", sqlSeedIncluded: "テーブル定義とテストデータを添えて SQL サンドボックスを開きました" },
    "en-US": { switchedToSandbox: "Switched to the sandbox", codeSentAndOpened: "Code sent and sandbox opened", sqlSeedIncluded: "SQL sandbox opened with table definitions and test data" },
    "ko-KR": { switchedToSandbox: "샌드박스로 전환했습니다", codeSentAndOpened: "코드를 보내고 샌드박스를 열었습니다", sqlSeedIncluded: "테이블 정의와 테스트 데이터를 포함해 SQL 샌드박스를 열었습니다" },
    "vi-VN": { switchedToSandbox: "Đã chuyển sang sandbox", codeSentAndOpened: "Đã gửi mã và mở sandbox", sqlSeedIncluded: "Đã mở sandbox SQL kèm định nghĩa bảng và dữ liệu kiểm thử" },
    "my-MM": { switchedToSandbox: "Sandbox သို့ ပြောင်းပြီးပါပြီ", codeSentAndOpened: "ကုဒ်ပို့ပြီး Sandbox ကို ဖွင့်ထားပါသည်", sqlSeedIncluded: "ဇယားသတ်မှတ်ချက်နှင့် စမ်းသပ်ဒေတာပါသော SQL Sandbox ကို ဖွင့်ထားပါသည်" },
    "fr-FR": { switchedToSandbox: "Passage au bac à sable effectué", codeSentAndOpened: "Code envoyé et bac à sable ouvert", sqlSeedIncluded: "Bac à sable SQL ouvert avec les tables et les données de test" },
    "th-TH": { switchedToSandbox: "สลับไปยังแซนด์บ็อกซ์แล้ว", codeSentAndOpened: "ส่งโค้ดและเปิดแซนด์บ็อกซ์แล้ว", sqlSeedIncluded: "เปิดแซนด์บ็อกซ์ SQL พร้อมโครงสร้างตารางและข้อมูลทดสอบแล้ว" },
    "id-ID": { switchedToSandbox: "Beralih ke sandbox", codeSentAndOpened: "Kode dikirim dan sandbox dibuka", sqlSeedIncluded: "Sandbox SQL dibuka dengan definisi tabel dan data uji" },
    "default-ja-zh": { switchedToSandbox: "已切换到沙盒 / サンドボックスに切り替えました", codeSentAndOpened: "代码已发送并打开沙盒 / コードを送信し、サンドボックスを開きました", sqlSeedIncluded: "已附带建表与测试数据 / テーブル定義とテストデータを添えました" }
  };

  Object.keys(CODING_TYPING_ROUND_21_8).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].codingTyping) window.I18nUiDict[locale].codingTyping = {};
    Object.assign(window.I18nUiDict[locale].codingTyping, CODING_TYPING_ROUND_21_8[locale]);
  });

  var CODING_TYPING_ROUND_22_1 = {
    "zh-CN": { practiceTitle: "编程打字练习", howToPractice: "怎么练", practiceDescription: "看目标代码，在输入区完整打出来。", stepChoose: "选择语言和类别", stepType: "输入目标代码", stepSandbox: "发送到沙盒或运行检查", sendOnlyHelp: "只发送到沙盒，不执行", runHelp: "发送到沙盒并立即执行", practicePoints: "练习要点", shortcuts: "快捷键", selectPractice: "请选择一个练习", enterCodeFirst: "请先输入代码" },
    "ja-JP": { practiceTitle: "プログラミングタイピング練習", howToPractice: "使い方", practiceDescription: "目標コードを見ながら、入力欄にそのまま入力します。", stepChoose: "言語とカテゴリを選ぶ", stepType: "目標コードを入力する", stepSandbox: "サンドボックスに送信、または実行して確認する", sendOnlyHelp: "サンドボックスへ送るだけで、実行はしません", runHelp: "サンドボックスへ送ってすぐ実行します", practicePoints: "練習ポイント", shortcuts: "ショートカット", selectPractice: "練習を選択してください", enterCodeFirst: "先にコードを入力してください" },
    "en-US": { practiceTitle: "Programming typing practice", howToPractice: "How to practice", practiceDescription: "Type the target code exactly into the input area.", stepChoose: "Choose a language and category", stepType: "Type the target code", stepSandbox: "Send to sandbox or run it", sendOnlyHelp: "Send only, do not run", runHelp: "Send to sandbox and run immediately", practicePoints: "Practice points", shortcuts: "Shortcuts", selectPractice: "Select a practice item", enterCodeFirst: "Enter code first" },
    "ko-KR": { practiceTitle: "프로그래밍 타이핑 연습", howToPractice: "연습 방법", practiceDescription: "목표 코드를 보고 입력 영역에 그대로 입력하세요.", stepChoose: "언어와 카테고리 선택", stepType: "목표 코드 입력", stepSandbox: "샌드박스로 보내거나 실행해 확인", sendOnlyHelp: "샌드박스로 보내기만 하고 실행하지 않음", runHelp: "샌드박스로 보내고 즉시 실행", practicePoints: "연습 포인트", shortcuts: "단축키", selectPractice: "연습 항목을 선택하세요", enterCodeFirst: "먼저 코드를 입력하세요" },
    "vi-VN": { practiceTitle: "Luyện gõ mã lập trình", howToPractice: "Cách luyện tập", practiceDescription: "Nhập chính xác mã mẫu vào vùng nhập liệu.", stepChoose: "Chọn ngôn ngữ và danh mục", stepType: "Nhập mã mẫu", stepSandbox: "Gửi đến sandbox hoặc chạy thử", sendOnlyHelp: "Chỉ gửi đến sandbox, không chạy", runHelp: "Gửi đến sandbox và chạy ngay", practicePoints: "Điểm luyện tập", shortcuts: "Phím tắt", selectPractice: "Hãy chọn một bài luyện", enterCodeFirst: "Hãy nhập mã trước" },
    "my-MM": { practiceTitle: "Programming စာရိုက်လေ့ကျင့်ခန်း", howToPractice: "လေ့ကျင့်နည်း", practiceDescription: "ပစ်မှတ်ကုဒ်ကို ကြည့်ပြီး input area တွင် အတိအကျ ရိုက်ပါ။", stepChoose: "ဘာသာစကားနှင့် အမျိုးအစားရွေးပါ", stepType: "ပစ်မှတ်ကုဒ်ကို ရိုက်ပါ", stepSandbox: "Sandbox သို့ပို့ပါ သို့မဟုတ် run လုပ်ပါ", sendOnlyHelp: "Sandbox သို့သာပို့ပြီး မ run ပါ", runHelp: "Sandbox သို့ပို့ပြီး ချက်ချင်း run ပါ", practicePoints: "လေ့ကျင့်ရန်အချက်များ", shortcuts: "Shortcut များ", selectPractice: "လေ့ကျင့်ခန်းတစ်ခုရွေးပါ", enterCodeFirst: "ကုဒ်ကို အရင်ရိုက်ပါ" },
    "fr-FR": { practiceTitle: "Exercice de frappe de code", howToPractice: "Comment pratiquer", practiceDescription: "Saisissez exactement le code cible dans la zone de saisie.", stepChoose: "Choisir un langage et une catégorie", stepType: "Saisir le code cible", stepSandbox: "Envoyer au bac à sable ou exécuter", sendOnlyHelp: "Envoyer seulement, sans exécuter", runHelp: "Envoyer puis exécuter immédiatement", practicePoints: "Points à travailler", shortcuts: "Raccourcis", selectPractice: "Sélectionnez un exercice", enterCodeFirst: "Saisissez d'abord du code" },
    "th-TH": { practiceTitle: "ฝึกพิมพ์โค้ดโปรแกรม", howToPractice: "วิธีฝึก", practiceDescription: "พิมพ์โค้ดเป้าหมายลงในช่องป้อนข้อมูลให้ตรงกัน", stepChoose: "เลือกภาษาและหมวดหมู่", stepType: "พิมพ์โค้ดเป้าหมาย", stepSandbox: "ส่งไปแซนด์บ็อกซ์หรือรัน", sendOnlyHelp: "ส่งอย่างเดียว ไม่รัน", runHelp: "ส่งแล้วรันทันที", practicePoints: "จุดฝึก", shortcuts: "ปุ่มลัด", selectPractice: "เลือกแบบฝึกหัด", enterCodeFirst: "กรุณาพิมพ์โค้ดก่อน" },
    "id-ID": { practiceTitle: "Latihan mengetik kode", howToPractice: "Cara berlatih", practiceDescription: "Ketik kode target dengan tepat di area input.", stepChoose: "Pilih bahasa dan kategori", stepType: "Ketik kode target", stepSandbox: "Kirim ke sandbox atau jalankan", sendOnlyHelp: "Hanya kirim, jangan jalankan", runHelp: "Kirim lalu langsung jalankan", practicePoints: "Poin latihan", shortcuts: "Pintasan", selectPractice: "Pilih item latihan", enterCodeFirst: "Masukkan kode terlebih dahulu" },
    "default-ja-zh": { practiceTitle: "编程打字练习 / プログラミングタイピング練習", howToPractice: "怎么练 / 使い方", practiceDescription: "看目标代码，在输入区完整打出来。 / 目標コードを見ながら入力します。", stepChoose: "选择语言和类别 / 言語とカテゴリを選ぶ", stepType: "输入目标代码 / 目標コードを入力する", stepSandbox: "发送到沙盒或运行检查 / 送信または実行して確認", sendOnlyHelp: "只发送，不执行 / 送信のみで実行しません", runHelp: "发送并立即执行 / 送信してすぐ実行", practicePoints: "练习要点 / 練習ポイント", shortcuts: "快捷键 / ショートカット", selectPractice: "请选择一个练习 / 練習を選択してください", enterCodeFirst: "请先输入代码 / 先にコードを入力してください" }
  };

  Object.keys(CODING_TYPING_ROUND_22_1).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].codingTyping) window.I18nUiDict[locale].codingTyping = {};
    Object.assign(window.I18nUiDict[locale].codingTyping, CODING_TYPING_ROUND_22_1[locale]);
  });

  var CODING_TYPING_ROUND_22_1_1 = {
    "zh-CN": { sentToSandbox: "已发送到沙盒", sendingAndRunning: "正在发送并运行", ranInSandbox: "已在沙盒中运行", runFailed: "运行失败", sendFailed: "发送到沙盒失败", directRunUnsupported: "当前练习不支持直接运行" },
    "ja-JP": { sentToSandbox: "サンドボックスに送信しました", sendingAndRunning: "送信して実行しています", ranInSandbox: "サンドボックスで実行しました", runFailed: "実行に失敗しました", sendFailed: "サンドボックスへの送信に失敗しました", directRunUnsupported: "この練習は直接実行に対応していません" },
    "en-US": { sentToSandbox: "Sent to sandbox", sendingAndRunning: "Sending and running", ranInSandbox: "Ran in sandbox", runFailed: "Run failed", sendFailed: "Failed to send to sandbox", directRunUnsupported: "This practice item does not support direct run" },
    "ko-KR": { sentToSandbox: "샌드박스로 전송했습니다", sendingAndRunning: "전송 후 실행 중입니다", ranInSandbox: "샌드박스에서 실행했습니다", runFailed: "실행에 실패했습니다", sendFailed: "샌드박스 전송에 실패했습니다", directRunUnsupported: "이 연습은 직접 실행을 지원하지 않습니다" },
    "vi-VN": { sentToSandbox: "Đã gửi đến sandbox", sendingAndRunning: "Đang gửi và chạy", ranInSandbox: "Đã chạy trong sandbox", runFailed: "Chạy thất bại", sendFailed: "Không thể gửi đến sandbox", directRunUnsupported: "Bài luyện này không hỗ trợ chạy trực tiếp" },
    "my-MM": { sentToSandbox: "Sandbox သို့ ပို့ပြီးပါပြီ", sendingAndRunning: "ပို့ပြီး လုပ်ဆောင်နေသည်", ranInSandbox: "Sandbox တွင် လုပ်ဆောင်ပြီးပါပြီ", runFailed: "လုပ်ဆောင်မှု မအောင်မြင်ပါ", sendFailed: "Sandbox သို့ မပို့နိုင်ပါ", directRunUnsupported: "ဤလေ့ကျင့်ခန်းကို တိုက်ရိုက်လုပ်ဆောင်၍ မရပါ" },
    "fr-FR": { sentToSandbox: "Envoyé au bac à sable", sendingAndRunning: "Envoi et exécution en cours", ranInSandbox: "Exécuté dans le bac à sable", runFailed: "Échec de l’exécution", sendFailed: "Échec de l’envoi au bac à sable", directRunUnsupported: "Cet exercice ne prend pas en charge l’exécution directe" },
    "th-TH": { sentToSandbox: "ส่งไปยังแซนด์บ็อกซ์แล้ว", sendingAndRunning: "กำลังส่งและรัน", ranInSandbox: "รันในแซนด์บ็อกซ์แล้ว", runFailed: "รันไม่สำเร็จ", sendFailed: "ส่งไปยังแซนด์บ็อกซ์ไม่สำเร็จ", directRunUnsupported: "แบบฝึกหัดนี้ไม่รองรับการรันโดยตรง" },
    "id-ID": { sentToSandbox: "Dikirim ke sandbox", sendingAndRunning: "Mengirim dan menjalankan", ranInSandbox: "Dijalankan di sandbox", runFailed: "Gagal menjalankan", sendFailed: "Gagal mengirim ke sandbox", directRunUnsupported: "Latihan ini tidak mendukung eksekusi langsung" },
    "default-ja-zh": { sentToSandbox: "已发送到沙盒 / サンドボックスに送信しました", sendingAndRunning: "正在发送并运行 / 送信して実行しています", ranInSandbox: "已在沙盒中运行 / サンドボックスで実行しました", runFailed: "运行失败 / 実行に失敗しました", sendFailed: "发送失败 / 送信に失敗しました", directRunUnsupported: "当前练习不支持直接运行 / この練習は直接実行に対応していません" }
  };

  Object.keys(CODING_TYPING_ROUND_22_1_1).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].codingTyping) window.I18nUiDict[locale].codingTyping = {};
    Object.assign(window.I18nUiDict[locale].codingTyping, CODING_TYPING_ROUND_22_1_1[locale]);
    if (!window.I18nUiDict[locale].wrongBook && window.I18nUiDict["en-US"].wrongBook) {
      window.I18nUiDict[locale].wrongBook = Object.assign({}, window.I18nUiDict["en-US"].wrongBook);
    }
  });

  var WRONG_BOOK_ROUND_23_4_EN = {
    moduleFilter: "Module",
    allModules: "All modules",
    uncategorized: "Uncategorized",
    startRetry: "Start retry",
    retryDisabled: "No retryable questions in the current filter",
    retryTitle: "Wrong answer retry",
    backToWrongBook: "Back to wrong book",
    submitAnswer: "Submit answer",
    nextQuestion: "Next question",
    finishRetry: "Finish retry",
    retryResult: "Retry result",
    retryTotal: "Total questions",
    retryCorrect: "Correct",
    retryWrong: "Wrong",
    retryAccuracy: "Accuracy",
    newMastered: "Newly mastered",
    retryAgain: "Retry again",
    practiceResultCorrect: "Correct",
    practiceResultWrong: "Incorrect"
  };
  var WRONG_BOOK_ROUND_23_4 = {
    "zh-CN": {
      moduleFilter: "模块", allModules: "全部模块", uncategorized: "未分类", startRetry: "开始重练",
      retryDisabled: "当前筛选无可重练错题", retryTitle: "错题重练", backToWrongBook: "返回错题本",
      submitAnswer: "提交答案", nextQuestion: "下一题", finishRetry: "结束重练", retryResult: "重练结果",
      retryTotal: "本次重练题数", retryCorrect: "答对数", retryWrong: "答错数", retryAccuracy: "正确率",
      newMastered: "本次新掌握数量", retryAgain: "再次重练", practiceResultCorrect: "回答正确",
      practiceResultWrong: "回答错误"
    },
    "ja-JP": {
      moduleFilter: "モジュール", allModules: "すべてのモジュール", uncategorized: "未分類", startRetry: "再練習を開始",
      retryDisabled: "現在の絞り込みに再練習できる問題はありません", retryTitle: "間違い再練習",
      backToWrongBook: "間違いノートへ戻る", submitAnswer: "回答を送信", nextQuestion: "次の問題",
      finishRetry: "再練習を終了", retryResult: "再練習結果", retryTotal: "問題数", retryCorrect: "正解数",
      retryWrong: "不正解数", retryAccuracy: "正答率", newMastered: "今回習得した問題",
      retryAgain: "もう一度練習", practiceResultCorrect: "正解です", practiceResultWrong: "不正解です"
    },
    "en-US": WRONG_BOOK_ROUND_23_4_EN,
    "ko-KR": {
      moduleFilter: "모듈", allModules: "모든 모듈", uncategorized: "미분류", startRetry: "재연습 시작",
      retryDisabled: "현재 필터에 재연습할 문제가 없습니다", retryTitle: "오답 재연습",
      backToWrongBook: "오답 노트로 돌아가기", submitAnswer: "답안 제출", nextQuestion: "다음 문제",
      finishRetry: "재연습 종료", retryResult: "재연습 결과", retryTotal: "문제 수", retryCorrect: "정답 수",
      retryWrong: "오답 수", retryAccuracy: "정답률", newMastered: "새로 숙달", retryAgain: "다시 연습",
      practiceResultCorrect: "정답입니다", practiceResultWrong: "오답입니다"
    },
    "vi-VN": WRONG_BOOK_ROUND_23_4_EN,
    "my-MM": WRONG_BOOK_ROUND_23_4_EN,
    "fr-FR": WRONG_BOOK_ROUND_23_4_EN,
    "th-TH": WRONG_BOOK_ROUND_23_4_EN,
    "id-ID": WRONG_BOOK_ROUND_23_4_EN,
    "default-ja-zh": Object.assign({}, WRONG_BOOK_ROUND_23_4_EN, {
      moduleFilter: "模块 / モジュール", allModules: "全部模块 / すべてのモジュール",
      uncategorized: "未分类 / 未分類", startRetry: "开始重练 / 再練習を開始",
      retryDisabled: "当前筛选无可重练错题 / 現在の絞り込みに再練習できる問題はありません",
      retryTitle: "错题重练 / 間違い再練習", backToWrongBook: "返回错题本 / 間違いノートへ戻る",
      submitAnswer: "提交答案 / 回答を送信", nextQuestion: "下一题 / 次の問題",
      finishRetry: "结束重练 / 再練習を終了", retryResult: "重练结果 / 再練習結果",
      retryTotal: "本次重练题数 / 問題数", retryCorrect: "答对数 / 正解数",
      retryWrong: "答错数 / 不正解数", retryAccuracy: "正确率 / 正答率",
      newMastered: "本次新掌握数量 / 今回習得した問題", retryAgain: "再次重练 / もう一度練習",
      practiceResultCorrect: "回答正确 / 正解です", practiceResultWrong: "回答错误 / 不正解です"
    })
  };
  var TOOLBAR_ROUND_23_4 = {
    "zh-CN": { language: "语言", localMode: "本地模式", settings: "设置", randomChallenge: "综合随机挑战", aiLearning: "AI 学习" },
    "ja-JP": { language: "言語", localMode: "ローカルモード", settings: "設定", randomChallenge: "総合ランダムチャレンジ", aiLearning: "AIアシスタント" },
    "en-US": { language: "Language", localMode: "Local mode", settings: "Settings", randomChallenge: "Random challenge", aiLearning: "AI Learning" },
    "ko-KR": { language: "언어", localMode: "로컬 모드", settings: "설정", randomChallenge: "종합 랜덤 도전", aiLearning: "AI 학습" },
    "my-MM": { language: "ဘာသာစကား", localMode: "ဒေသတွင်းမုဒ်", settings: "ဆက်တင်များ", randomChallenge: "ကျပန်းစိန်ခေါ်မှု", aiLearning: "AI သင်ယူမှု" },
    "vi-VN": { language: "Ngôn ngữ", localMode: "Chế độ cục bộ", settings: "Cài đặt", randomChallenge: "Thử thách ngẫu nhiên", aiLearning: "AI Học tập" },
    "th-TH": { language: "ภาษา", localMode: "โหมดภายใน", settings: "ตั้งค่า", randomChallenge: "สุ่มท้าทาย", aiLearning: "AI เรียนรู้" },
    "id-ID": { language: "Bahasa", localMode: "Mode lokal", settings: "Pengaturan", randomChallenge: "Tantangan acak", aiLearning: "AI Belajar" },
    "fr-FR": { language: "Langue", localMode: "Mode local", settings: "Paramètres", randomChallenge: "Défi aléatoire", aiLearning: "IA Apprentissage" },
    "default-ja-zh": { language: "语言 / 言語", localMode: "本地模式 / ローカルモード", settings: "设置 / 設定", randomChallenge: "综合随机挑战 / 総合ランダムチャレンジ", aiLearning: "AI 学习 / AIアシスタント" }
  };
  Object.keys(WRONG_BOOK_ROUND_23_4).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].wrongBook) window.I18nUiDict[locale].wrongBook = {};
    Object.assign(window.I18nUiDict[locale].wrongBook, WRONG_BOOK_ROUND_23_4[locale]);
    if (!window.I18nUiDict[locale].toolbar) window.I18nUiDict[locale].toolbar = {};
    Object.assign(window.I18nUiDict[locale].toolbar, TOOLBAR_ROUND_23_4[locale] || TOOLBAR_ROUND_23_4["en-US"]);
  });

  /* ── Round 23.5: retry settings, history, trends ─────────────── */
  var WRONG_BOOK_ROUND_23_5_EN = {
    retryLimit: "Questions", retryLimit10: "10", retryLimit20: "20", retryLimit30: "30",
    retryLimitAll: "All", shuffle: "Shuffle", shuffleOn: "On", shuffleOff: "Off",
    retryHistory: "Retry history", retryHistoryEmpty: "No retry records yet",
    lastRetry: "Last retry", recentRetries: "Recent 7 retries summary",
    averageAccuracy: "Avg accuracy", totalPracticed: "Total practiced", totalNewMastered: "Newly mastered",
    duration: "Duration", lastPracticedAt: "Last practiced", correctRetryCount: "Retry correct"
  };
  var WRONG_BOOK_ROUND_23_5 = {
    "zh-CN": {
      retryLimit: "题量", retryLimit10: "10 题", retryLimit20: "20 题", retryLimit30: "30 题",
      retryLimitAll: "全部", shuffle: "随机顺序", shuffleOn: "开", shuffleOff: "关",
      retryHistory: "重练历史", retryHistoryEmpty: "暂无重练记录",
      lastRetry: "最近一次重练", recentRetries: "最近 7 次重练摘要",
      averageAccuracy: "平均正确率", totalPracticed: "总练习题数", totalNewMastered: "新掌握总数",
      duration: "用时", lastPracticedAt: "最近练习", correctRetryCount: "重练答对"
    },
    "ja-JP": {
      retryLimit: "問題数", retryLimit10: "10問", retryLimit20: "20問", retryLimit30: "30問",
      retryLimitAll: "すべて", shuffle: "ランダム順序", shuffleOn: "入", shuffleOff: "切",
      retryHistory: "再練習履歴", retryHistoryEmpty: "再練習記録はありません",
      lastRetry: "最近の再練習", recentRetries: "直近7回の再練習サマリー",
      averageAccuracy: "平均正答率", totalPracticed: "総練習問題数", totalNewMastered: "新規習得数",
      duration: "所要時間", lastPracticedAt: "最近の練習", correctRetryCount: "再練習正解"
    },
    "ko-KR": {
      retryLimit: "문제 수", retryLimit10: "10문제", retryLimit20: "20문제", retryLimit30: "30문제",
      retryLimitAll: "전체", shuffle: "랜덤 순서", shuffleOn: "켜기", shuffleOff: "끄기",
      retryHistory: "재연습 기록", retryHistoryEmpty: "재연습 기록이 없습니다",
      lastRetry: "최근 재연습", recentRetries: "최근 7회 재연습 요약",
      averageAccuracy: "평균 정답률", totalPracticed: "총 연습 문제 수", totalNewMastered: "새로 숙달한 수",
      duration: "소요 시간", lastPracticedAt: "최근 연습", correctRetryCount: "재연습 정답"
    },
    "en-US": WRONG_BOOK_ROUND_23_5_EN,
    "vi-VN": WRONG_BOOK_ROUND_23_5_EN,
    "my-MM": WRONG_BOOK_ROUND_23_5_EN,
    "fr-FR": WRONG_BOOK_ROUND_23_5_EN,
    "th-TH": WRONG_BOOK_ROUND_23_5_EN,
    "id-ID": WRONG_BOOK_ROUND_23_5_EN,
    "default-ja-zh": Object.assign({}, WRONG_BOOK_ROUND_23_5_EN, {
      retryLimit: "题量 / 問題数", retryLimit10: "10 题 / 10問", retryLimit20: "20 题 / 20問",
      retryLimit30: "30 题 / 30問", retryLimitAll: "全部 / すべて",
      shuffle: "随机顺序 / ランダム順序", shuffleOn: "开 / 入", shuffleOff: "关 / 切",
      retryHistory: "重练历史 / 再練習履歴", retryHistoryEmpty: "暂无重练记录 / 再練習記録はありません",
      lastRetry: "最近一次重练 / 最近の再練習", recentRetries: "最近 7 次重练摘要 / 直近7回の再練習サマリー",
      averageAccuracy: "平均正确率 / 平均正答率", totalPracticed: "总练习题数 / 総練習問題数",
      totalNewMastered: "新掌握总数 / 新規習得数", duration: "用时 / 所要時間",
      lastPracticedAt: "最近练习 / 最近の練習", correctRetryCount: "重练答对 / 再練習正解"
    })
  };
  Object.keys(WRONG_BOOK_ROUND_23_5).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].wrongBook) window.I18nUiDict[locale].wrongBook = {};
    Object.assign(window.I18nUiDict[locale].wrongBook, WRONG_BOOK_ROUND_23_5[locale]);
  });

  /* ── Round 23.13: username-only authentication UX ──────────── */
  var AUTH_ROUND_23_13_EN = {
    registerTitle: "Create Account",
    username: "Username",
    usernamePlaceholder: "3-24 letters, numbers, underscores, or hyphens",
    displayNameOptional: "Display name (optional)",
    createAccountButton: "Create Account",
    usernameRequired: "Please enter a username",
    usernameTooShort: "Username must be at least 3 characters",
    usernameTooLong: "Username must be no more than 24 characters",
    usernameInvalid: "Use only letters, numbers, underscores, and hyphens",
    usernameTaken: "This username is already in use",
    usernameRegistrationUnavailable: "Username account creation is unavailable. Check the Supabase sign-up and email confirmation settings.",
    usernameOrPasswordWrong: "Username or password is incorrect",
    usernameAccount: "Username account",
    legacyEmailAccount: "Legacy email account",
    accountType: "Account type",
    emailConfirmationUsernameWarning: "The account was created, but email confirmation is enabled. Username accounts cannot receive confirmation mail. Disable email confirmation for this project or use an existing test account.",
    noEmailRequired: "No email required",
    noEmailRequiredDesc: "Sign in with only your username and password."
  };
  var AUTH_ROUND_23_13 = {
    "zh-CN": {
      registerTitle: "创建账号",
      username: "用户名",
      usernamePlaceholder: "3-24 位字母、数字、下划线或连字符",
      displayNameOptional: "昵称（可选）",
      createAccountButton: "创建账号",
      usernameRequired: "请输入用户名",
      usernameTooShort: "用户名至少 3 位",
      usernameTooLong: "用户名最多 24 位",
      usernameInvalid: "用户名只能包含字母、数字、下划线和连字符",
      usernameTaken: "该用户名已被使用",
      usernameRegistrationUnavailable: "当前无法创建用户名账号，请检查 Supabase 的注册与邮箱确认设置。",
      usernameOrPasswordWrong: "用户名或密码不正确",
      usernameAccount: "用户名账号",
      legacyEmailAccount: "旧邮箱账号",
      accountType: "账号类型",
      emailConfirmationUsernameWarning: "账号已创建，但项目启用了邮箱确认。用户名账号无法接收确认邮件，请管理员关闭邮箱确认，或使用现有测试账号。",
      noEmailRequired: "无需邮箱",
      noEmailRequiredDesc: "登录只需用户名和密码。"
    },
    "ja-JP": {
      registerTitle: "アカウント作成",
      username: "ユーザー名",
      usernamePlaceholder: "3～24文字の英数字、_、-",
      displayNameOptional: "表示名（任意）",
      createAccountButton: "アカウント作成",
      usernameRequired: "ユーザー名を入力してください",
      usernameTooShort: "ユーザー名は3文字以上です",
      usernameTooLong: "ユーザー名は24文字以内です",
      usernameInvalid: "英数字、アンダースコア、ハイフンのみ使用できます",
      usernameTaken: "このユーザー名は既に使用されています",
      usernameRegistrationUnavailable: "ユーザー名アカウントを作成できません。Supabase の登録とメール確認設定を確認してください。",
      usernameOrPasswordWrong: "ユーザー名またはパスワードが正しくありません",
      usernameAccount: "ユーザー名アカウント",
      legacyEmailAccount: "旧メールアカウント",
      accountType: "アカウント種別",
      emailConfirmationUsernameWarning: "アカウントは作成されましたが、メール確認が有効です。ユーザー名アカウントは確認メールを受信できません。プロジェクトのメール確認を無効にするか、既存のテストアカウントを使用してください。",
      noEmailRequired: "メール不要",
      noEmailRequiredDesc: "ユーザー名とパスワードだけでログインできます。"
    },
    "en-US": AUTH_ROUND_23_13_EN,
    "ko-KR": {
      registerTitle: "계정 만들기",
      username: "사용자 이름",
      usernamePlaceholder: "3-24자의 영문, 숫자, 밑줄 또는 하이픈",
      displayNameOptional: "표시 이름(선택)",
      createAccountButton: "계정 만들기",
      usernameRequired: "사용자 이름을 입력해 주세요",
      usernameTooShort: "사용자 이름은 3자 이상이어야 합니다",
      usernameTooLong: "사용자 이름은 24자 이하여야 합니다",
      usernameInvalid: "영문, 숫자, 밑줄, 하이픈만 사용할 수 있습니다",
      usernameTaken: "이미 사용 중인 사용자 이름입니다",
      usernameRegistrationUnavailable: "사용자 이름 계정을 만들 수 없습니다. Supabase 가입 및 이메일 확인 설정을 확인하세요.",
      usernameOrPasswordWrong: "사용자 이름 또는 비밀번호가 올바르지 않습니다",
      usernameAccount: "사용자 이름 계정",
      legacyEmailAccount: "기존 이메일 계정",
      accountType: "계정 유형",
      emailConfirmationUsernameWarning: "계정은 생성되었지만 이메일 확인이 활성화되어 있습니다. 사용자 이름 계정은 확인 메일을 받을 수 없습니다. 프로젝트의 이메일 확인을 끄거나 기존 테스트 계정을 사용하세요.",
      noEmailRequired: "이메일 불필요",
      noEmailRequiredDesc: "사용자 이름과 비밀번호만으로 로그인합니다."
    },
    "vi-VN": {
      registerTitle: "Tạo tài khoản",
      username: "Tên người dùng",
      usernamePlaceholder: "3-24 chữ cái, số, dấu gạch dưới hoặc gạch nối",
      displayNameOptional: "Tên hiển thị (không bắt buộc)",
      createAccountButton: "Tạo tài khoản",
      usernameRequired: "Vui lòng nhập tên người dùng",
      usernameTooShort: "Tên người dùng phải có ít nhất 3 ký tự",
      usernameTooLong: "Tên người dùng không được quá 24 ký tự",
      usernameInvalid: "Chỉ dùng chữ cái, số, dấu gạch dưới và gạch nối",
      usernameTaken: "Tên người dùng này đã được sử dụng",
      usernameRegistrationUnavailable: "Không thể tạo tài khoản tên người dùng. Hãy kiểm tra cài đặt đăng ký và xác nhận email của Supabase.",
      usernameOrPasswordWrong: "Tên người dùng hoặc mật khẩu không đúng",
      usernameAccount: "Tài khoản tên người dùng",
      legacyEmailAccount: "Tài khoản email cũ",
      accountType: "Loại tài khoản",
      emailConfirmationUsernameWarning: AUTH_ROUND_23_13_EN.emailConfirmationUsernameWarning,
      noEmailRequired: "Không cần email",
      noEmailRequiredDesc: "Chỉ cần tên người dùng và mật khẩu để đăng nhập."
    },
    "fr-FR": {
      registerTitle: "Créer un compte",
      username: "Nom d'utilisateur",
      usernamePlaceholder: "3 à 24 lettres, chiffres, tirets bas ou traits d'union",
      displayNameOptional: "Nom affiché (facultatif)",
      createAccountButton: "Créer un compte",
      usernameRequired: "Saisissez un nom d'utilisateur",
      usernameTooShort: "Le nom d'utilisateur doit contenir au moins 3 caractères",
      usernameTooLong: "Le nom d'utilisateur ne doit pas dépasser 24 caractères",
      usernameInvalid: "Utilisez uniquement lettres, chiffres, tirets bas et traits d'union",
      usernameTaken: "Ce nom d'utilisateur est déjà utilisé",
      usernameRegistrationUnavailable: "Impossible de créer le compte. Vérifiez les paramètres d'inscription et de confirmation d'e-mail Supabase.",
      usernameOrPasswordWrong: "Nom d'utilisateur ou mot de passe incorrect",
      usernameAccount: "Compte avec nom d'utilisateur",
      legacyEmailAccount: "Ancien compte e-mail",
      accountType: "Type de compte",
      emailConfirmationUsernameWarning: AUTH_ROUND_23_13_EN.emailConfirmationUsernameWarning,
      noEmailRequired: "Aucun e-mail requis",
      noEmailRequiredDesc: "Connectez-vous avec votre nom d'utilisateur et votre mot de passe."
    },
    "my-MM": Object.assign({}, AUTH_ROUND_23_13_EN),
    "default-ja-zh": {
      registerTitle: "创建账号 / アカウント作成",
      username: "用户名 / ユーザー名",
      usernamePlaceholder: "3-24 位字母、数字、下划线或连字符 / 3～24文字の英数字、_、-",
      displayNameOptional: "昵称（可选） / 表示名（任意）",
      createAccountButton: "创建账号 / アカウント作成",
      usernameRequired: "请输入用户名 / ユーザー名を入力してください",
      usernameTooShort: "用户名至少 3 位 / ユーザー名は3文字以上です",
      usernameTooLong: "用户名最多 24 位 / ユーザー名は24文字以内です",
      usernameInvalid: "用户名格式不正确 / ユーザー名の形式が正しくありません",
      usernameTaken: "该用户名已被使用 / このユーザー名は既に使用されています",
      usernameRegistrationUnavailable: "请检查 Supabase 注册设置 / Supabase の登録設定を確認してください",
      usernameOrPasswordWrong: "用户名或密码不正确 / ユーザー名またはパスワードが正しくありません",
      usernameAccount: "用户名账号 / ユーザー名アカウント",
      legacyEmailAccount: "旧邮箱账号 / 旧メールアカウント",
      accountType: "账号类型 / アカウント種別",
      emailConfirmationUsernameWarning: "项目启用了邮箱确认，用户名账号无法接收确认邮件。 / メール確認が有効なため、ユーザー名アカウントは確認メールを受信できません。",
      noEmailRequired: "无需邮箱 / メール不要",
      noEmailRequiredDesc: "只需用户名和密码 / ユーザー名とパスワードだけでログイン"
    }
  };
  Object.keys(AUTH_ROUND_23_13).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].auth) window.I18nUiDict[locale].auth = {};
    Object.assign(window.I18nUiDict[locale].auth, AUTH_ROUND_23_13[locale]);
  });

  /* Round 23.23: account management MVP */
  var AUTH_ROUND_23_23_EN = {
    accountManagement: "Account management",
    accountManagementHint: "Update your display name or password.",
    usernameReadonly: "Username cannot be changed yet.",
    usernameChangeComingSoon: "Username changes will be available in a future version.",
    displayName: "Display name",
    newDisplayName: "New display name",
    saveDisplayName: "Save display name",
    displayNameRequired: "Please enter a display name",
    displayNameTooLong: "Display name must be no more than 32 characters",
    displayNameUpdated: "Display name updated",
    displayNameUpdateFailed: "Display name update failed",
    changePassword: "Change password",
    newPassword: "New password",
    confirmNewPassword: "Confirm new password",
    updatePassword: "Update password",
    passwordUpdated: "Password updated. Keep it safe.",
    passwordUpdatedPleaseSignIn: "Password updated. Please sign in again.",
    passwordUpdateFailed: "Password update failed",
    passwordKeepSafe: "Use a unique password and keep it safe.",
    accountSecurity: "Account security",
    currentUsername: "Current username",
    currentDisplayName: "Current display name",
    saving: "Saving"
  };
  var AUTH_ROUND_23_23 = {
    "zh-CN": {
      accountManagement: "账号管理",
      accountManagementHint: "修改昵称或密码。",
      usernameReadonly: "用户名暂不支持直接修改。",
      usernameChangeComingSoon: "用户名修改将在后续版本开放。",
      displayName: "昵称 / 显示名",
      newDisplayName: "新昵称",
      saveDisplayName: "保存昵称",
      displayNameRequired: "请输入昵称",
      displayNameTooLong: "昵称最多 32 个字符",
      displayNameUpdated: "昵称已更新",
      displayNameUpdateFailed: "昵称更新失败",
      changePassword: "修改密码",
      newPassword: "新密码",
      confirmNewPassword: "确认新密码",
      updatePassword: "更新密码",
      passwordUpdated: "密码已更新，请妥善保存",
      passwordUpdatedPleaseSignIn: "密码已更新，请重新登录",
      passwordUpdateFailed: "密码更新失败",
      passwordKeepSafe: "请使用独立密码并妥善保管。",
      accountSecurity: "账号安全",
      currentUsername: "当前用户名",
      currentDisplayName: "当前昵称",
      saving: "保存中"
    },
    "ja-JP": {
      accountManagement: "アカウント管理",
      accountManagementHint: "表示名またはパスワードを変更します。",
      usernameReadonly: "ユーザー名の直接変更は現在未対応です。",
      usernameChangeComingSoon: "ユーザー名変更は今後のバージョンで対応予定です。",
      displayName: "ニックネーム / 表示名",
      newDisplayName: "新しい表示名",
      saveDisplayName: "ニックネームを保存",
      displayNameRequired: "表示名を入力してください",
      displayNameTooLong: "表示名は32文字以内で入力してください",
      displayNameUpdated: "表示名を更新しました",
      displayNameUpdateFailed: "表示名の更新に失敗しました",
      changePassword: "パスワード変更",
      newPassword: "新しいパスワード",
      confirmNewPassword: "新しいパスワードを確認",
      updatePassword: "パスワードを更新",
      passwordUpdated: "パスワードを更新しました。安全に保管してください。",
      passwordUpdatedPleaseSignIn: "パスワードを更新しました。再度ログインしてください。",
      passwordUpdateFailed: "パスワードの更新に失敗しました",
      passwordKeepSafe: "他と異なるパスワードを使用し、安全に保管してください。",
      accountSecurity: "アカウントセキュリティ",
      currentUsername: "現在のユーザー名",
      currentDisplayName: "現在の表示名",
      saving: "保存中"
    },
    "en-US": AUTH_ROUND_23_23_EN,
    "ko-KR": {
      accountManagement: "계정 관리",
      accountManagementHint: "표시 이름 또는 비밀번호를 변경합니다.",
      usernameReadonly: "사용자 이름은 아직 직접 변경할 수 없습니다.",
      usernameChangeComingSoon: "사용자 이름 변경은 향후 버전에서 제공됩니다.",
      displayName: "닉네임 / 표시 이름",
      newDisplayName: "새 표시 이름",
      saveDisplayName: "닉네임 저장",
      displayNameRequired: "표시 이름을 입력해 주세요",
      displayNameTooLong: "표시 이름은 32자 이하여야 합니다",
      displayNameUpdated: "표시 이름이 업데이트되었습니다",
      displayNameUpdateFailed: "표시 이름 업데이트에 실패했습니다",
      changePassword: "비밀번호 변경",
      newPassword: "새 비밀번호",
      confirmNewPassword: "새 비밀번호 확인",
      updatePassword: "비밀번호 업데이트",
      passwordUpdated: "비밀번호가 업데이트되었습니다. 안전하게 보관해 주세요.",
      passwordUpdatedPleaseSignIn: "비밀번호가 업데이트되었습니다. 다시 로그인해 주세요.",
      passwordUpdateFailed: "비밀번호 업데이트에 실패했습니다",
      passwordKeepSafe: "고유한 비밀번호를 사용하고 안전하게 보관해 주세요.",
      accountSecurity: "계정 보안",
      currentUsername: "현재 사용자 이름",
      currentDisplayName: "현재 표시 이름",
      saving: "저장 중"
    },
    "my-MM": Object.assign({}, AUTH_ROUND_23_23_EN),
    "vi-VN": Object.assign({}, AUTH_ROUND_23_23_EN),
    "fr-FR": Object.assign({}, AUTH_ROUND_23_23_EN),
    "default-ja-zh": {
      accountManagement: "账号管理 / アカウント管理",
      accountManagementHint: "修改昵称或密码 / 表示名またはパスワードを変更",
      usernameReadonly: "用户名暂不支持直接修改 / ユーザー名の直接変更は現在未対応です",
      usernameChangeComingSoon: "用户名修改将在后续版本开放 / 今後のバージョンで対応予定です",
      displayName: "昵称 / 表示名",
      newDisplayName: "新昵称 / 新しい表示名",
      saveDisplayName: "保存昵称 / ニックネームを保存",
      displayNameRequired: "请输入昵称 / 表示名を入力してください",
      displayNameTooLong: "昵称最多 32 个字符 / 表示名は32文字以内です",
      displayNameUpdated: "昵称已更新 / 表示名を更新しました",
      displayNameUpdateFailed: "昵称更新失败 / 表示名の更新に失敗しました",
      changePassword: "修改密码 / パスワード変更",
      newPassword: "新密码 / 新しいパスワード",
      confirmNewPassword: "确认新密码 / 新しいパスワードを確認",
      updatePassword: "更新密码 / パスワードを更新",
      passwordUpdated: "密码已更新，请妥善保存 / パスワードを更新しました",
      passwordUpdateFailed: "密码更新失败 / パスワードの更新に失敗しました",
      passwordKeepSafe: "请妥善保存密码 / パスワードを安全に保管してください",
      accountSecurity: "账号安全 / アカウントセキュリティ",
      currentUsername: "当前用户名 / 現在のユーザー名",
      currentDisplayName: "当前昵称 / 現在の表示名",
      saving: "保存中 / 保存中"
    }
  };
  Object.keys(AUTH_ROUND_23_23).forEach(function (locale) {
    if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
    if (!window.I18nUiDict[locale].auth) window.I18nUiDict[locale].auth = {};
    Object.assign(window.I18nUiDict[locale].auth, AUTH_ROUND_23_23[locale]);
  });

  var CODING_TYPING_KO_UI_SUBROUND_5 = {
    examInsight: "시험 관점",
    examRelevance: "시험 관련성",
    examTargets: "시험 목표",
    relatedTerms: "관련 용어",
    meaning: "의미",
    memoryHook: "기억 포인트",
    examPoint: "시험 포인트",
    commonMistake: "자주 하는 실수",
    high: "높음",
    medium: "중간",
    low: "낮음",
    sendToSandbox: "샌드박스로 보내기",
    runInSandbox: "샌드박스에서 실행"
  };
  if (!window.I18nUiDict["ko-KR"]) window.I18nUiDict["ko-KR"] = {};
  if (!window.I18nUiDict["ko-KR"].codingTyping) window.I18nUiDict["ko-KR"].codingTyping = {};
  Object.assign(window.I18nUiDict["ko-KR"].codingTyping, CODING_TYPING_KO_UI_SUBROUND_5);

  (function () {
    var HOTFIX2_DICT_PATCH = {
      "ko-KR": {
        common: { confirm: "확인", cancel: "취소", warning: "경고", success: "성공", error: "오류", back: "돌아가기" },
        tools: {
          confirmResetTitle: "학습 진행 상황을 초기화하시겠습니까?",
          confirmResetBody: "이 작업은 취소할 수 없으며 모든 학습 및 시험 기록이 삭제됩니다. 초기화하시겠습니까?",
          confirmReset: "초기화 진행",
          cancel: "취소",
          importPreview: "백업 가져오기 미리보기",
          importPreviewDesc: "읽기 전용 미리보기로, 현재 데이터는 변경되지 않습니다."
        },
        exam: {
          allExamTitle: "종합 랜덤 모의고사",
          yearExamTitle: "{year} 공개 기출문제 모의고사",
          cbtSimulator: "CBT 시뮬레이터",
          remainingTime: "남은 시간: ",
          exitCbt: "중단 및 퇴실",
          submitCbt: "답안 제출",
          exit: "중단 및 퇴실",
          submit: "답안 제출"
        },
        mockExam: {
          navStatus: "제 {current} 번, 총 {total} 문제 (답변 완료 {answered} 문제)",
          questionNumber: "제 {num} 번 (문제 {number})",
          flagged: "검토 표시됨",
          flag: "이 문제 검토 표시",
          scoreFormat: "{score} / 1000 점",
          scorePass: "합격 (>=300)",
          scoreFail: "기준점 미달 (<300)",
          unanswered: "미답"
        },
        examHistory: { passed: "합격", failed: "불합격" },
        aiProvider: {
          title: "AI Provider 설정",
          description: "API 키는 현재 브라우저 세션에만 임시 저장되며 로컬 데이터베이스에는 기록되지 않습니다.",
          close: "닫기",
          provider: "제공자 (Provider)",
          model: "모델",
          modelHint: "기본값을 사용하려면 비워 두십시오",
          apiKey: "API Key",
          apiKeySessionOnly: "sessionStorage에만 저장됨",
          apiKeyPlaceholder: "환경 변수가 구성된 경우 비워 둘 수 있습니다",
          ollamaEndpoint: "Ollama 주소",
          ollamaEndpointHint: "Ollama 사용 시만 적용",
          ollamaPlaceholder: "http://127.0.0.1:11434",
          cancel: "취소",
          save: "설정 저장",
          statusReady: "준비 완료",
          statusNeedConfig: "설정 필요",
          statusOllamaConfigured: "{count}개의 로컬 모델",
          statusOllamaNotRunning: "로컬 서비스가 실행되지 않음",
          statusEnvDetected: "환경 변수 감지됨",
          statusSessionKeyAllowed: "세션 키 사용 가능",
          errorApiKeyMissing: "현재 Provider에 API 키가 없습니다. AI 설정을 열어 구성해 주세요.",
          errorAuthFailed: "API 키가 유효하지 않거나 모델 권한이 없습니다. 확인 후 다시 시도해 주세요.",
          errorServiceUnavailable: "AI 서비스에 일시적으로 연결할 수 없습니다. Ollama를 사용하는 경우 로컬 서비스가 시작되었는지 확인하세요."
        },
        sandbox: {
          clearCodeConfirm: "코드를 클리어하시겠습니까? / Clear code?",
          copied: "복사 완료!",
          error: "오류",
          pleaseEnterCode: "코드를 입력해 주세요.",
          compiling: "컴파일 중...",
          running: "실행 중...",
          warning: "경고",
          serviceUnavailable: "온라인 실행 서비스는 현재 사용할 수 없습니다. 잠시 후 다시 시도하거나 Windows 데스크톱 버전을 사용하세요.",
          timeout: "시간 초과",
          timeoutError: "시간 초과 오류: 코드 실행 시간이 25초를 초과했습니다. 무한 루프가 있는지 확인하세요.",
          serverOff: "로컬 서버 실행 안 됨",
          pythonNotFound: "Python을 찾을 수 없음",
          run: "실행",
          compileError: "컴파일 오류",
          runtimeError: "런타임 오류",
          success: "성공",
          done: "완료",
          noOutput: "프로그램이 정상적으로 종료되었습니다 (출력 없음)",
          executionTime: "실행 시간",
          aiHelp: "AI 분석"
        },
        japaneseTyping: {
          title: "일본어 타이핑",
          categoryAll: "전체",
          categoryDaily: "일상생활",
          categorySocial: "사교",
          categoryWorkplace: "직장",
          categoryBusinessMail: "비즈니스 메일",
          categoryIt: "IT 개발",
          categoryPublic: "공공 절차",
          categoryNews: "뉴스",
          categoryLiterature: "문학 수필",
          categoryCulture: "문화 취미",
          categorySpecial: "스페셜",
          categoryFavorites: "즐겨찾기",
          categoryReview: "틀린 문장 연습",
          emptyFiltered: "조건에 맞는 기사가 없습니다.",
          articleCount: "{count}개",
          finishCompleted: "연습 완료",
          finishTimeout: "시간 종료",
          finishSummary: "{kpm} KPM · 정확도 {accuracy}% · {time} · 수정 {corrections}회",
          emptyHistory: "연습을 완료하면 여기에 최근 기록이 표시됩니다.",
          reviewCount: "{count}개 재연습 대기"
        },
        codingTyping: {
          title: "프로그래밍 타이핑 연습",
          subtitle: "타이핑 연습을 통해 프로그래밍 구문을 마스터하세요. 언어와 카테고리를 선택해 연습을 시작합니다.",
          language: "언어 선택",
          category: "카테고리 필터",
          difficultyFilter: "난이도 필터",
          allDifficulties: "전체",
          exerciseList: "연습 목록",
          practiceTitle: "프로그래밍 타이핑 연습",
          practiceDescription: "대상 코드를 보고 입력 영역에 정확하게 타이핑하세요.",
          stepChoose: "1. 언어 및 카테고리 선택",
          stepType: "2. 대상 코드 입력",
          stepSandbox: "3. 샌드박스로 전송 또는 실행 확인",
          sendToSandbox: "샌드박스로 보내기",
          sendOnlyHelp: "샌드박스로 보내기만 하고 실행은 하지 않음",
          runInSandbox: "샌드박스에서 실행",
          runHelp: "샌드박스로 보내고 즉시 실행",
          backToList: "연습 목록으로 돌아가기",
          random: "랜덤 연습",
          next: "다음 문제",
          restart: "다시 하기",
          normalizeFullWidth: "전각→반각",
          lenientIndent: "들여쓰기 허용",
          code: "대상 코드",
          examInsight: "시험 이해",
          meaning: "의미",
          memoryHook: "암기법",
          examPoint: "시험 포인트",
          commonMistake: "자주 하는 실수",
          input: "입력 영역",
          inputPlaceholder: "여기에 대상 코드를 입력하세요...",
          speed: "CPM",
          accuracy: "정확도",
          progress: "진행 상황",
          time: "시간",
          errors: "오류",
          historyFilter: "기록 필터",
          allLanguages: "모든 언어",
          allCategories: "모든 카테고리",
          clearHistory: "기록 삭제"
        }
      },
      "my-MM": {
        common: { confirm: "Confirm", cancel: "Cancel", warning: "Warning", success: "Success", error: "Error", back: "Back" },
        tools: {
          confirmResetTitle: "Confirm Reset Progress?",
          confirmResetBody: "This will delete all progress and cannot be undone.",
          confirmReset: "Reset Progress",
          cancel: "Cancel",
          importPreview: "Preview Import",
          importPreviewDesc: "Read-only preview, will not change current data"
        },
        exam: {
          allExamTitle: "Comprehensive Random Mock Exam",
          yearExamTitle: "{year} Published Questions Mock Exam",
          cbtSimulator: "CBT Simulator",
          remainingTime: "Remaining Time: ",
          exitCbt: "Exit",
          submitCbt: "Submit",
          exit: "Exit",
          submit: "Submit"
        },
        mockExam: {
          navStatus: "Q {current} of {total} ({answered} answered)",
          questionNumber: "Q {num} (Q{number})",
          flagged: "Flagged",
          flag: "Flag for Review",
          scoreFormat: "{score} / 1000 pts",
          scorePass: "Pass (>=300)",
          scoreFail: "Fail (<300)",
          unanswered: "Unanswered"
        },
        examHistory: { passed: "Pass", failed: "Fail" },
        aiProvider: {
          title: "AI Provider Settings",
          description: "API keys are stored in sessionStorage only and not in database.",
          close: "Close",
          provider: "Provider",
          model: "Model",
          modelHint: "Leave empty for default",
          apiKey: "API Key",
          apiKeySessionOnly: "sessionStorage only",
          apiKeyPlaceholder: "Optional if env variable is set",
          ollamaEndpoint: "Ollama Endpoint",
          ollamaEndpointHint: "Ollama only",
          ollamaPlaceholder: "http://127.0.0.1:11434",
          cancel: "Cancel",
          save: "Save Settings",
          statusReady: "Ready",
          statusNeedConfig: "Need Config",
          statusOllamaConfigured: "{count} local models",
          statusOllamaNotRunning: "Local service not running",
          statusEnvDetected: "Env variable detected",
          statusSessionKeyAllowed: "Session key allowed",
          errorApiKeyMissing: "API key is missing for the current provider.",
          errorAuthFailed: "Invalid API key or model permissions.",
          errorServiceUnavailable: "AI service unavailable. Make sure local service is running if using Ollama."
        },
        sandbox: {
          clearCodeConfirm: "Clear code?",
          copied: "Copied!",
          error: "Error",
          pleaseEnterCode: "Please enter code first.",
          compiling: "Compiling...",
          running: "Running...",
          warning: "Warning",
          serviceUnavailable: "Online service unavailable.",
          timeout: "Timeout",
          timeoutError: "Execution timed out.",
          serverOff: "Server not running",
          pythonNotFound: "Python not found",
          run: "Run",
          compileError: "Compile Error",
          runtimeError: "Runtime Error",
          success: "Success",
          done: "Done",
          noOutput: "Finished successfully with no output",
          executionTime: "Execution Time",
          aiHelp: "AI Help"
        },
        japaneseTyping: {
          title: "Japanese Typing",
          categoryAll: "All",
          categoryDaily: "Daily",
          categorySocial: "Social",
          categoryWorkplace: "Workplace",
          categoryBusinessMail: "Business Mail",
          categoryIt: "IT Dev",
          categoryPublic: "Public Procedures",
          categoryNews: "News",
          categoryLiterature: "Literature",
          categoryCulture: "Culture",
          categorySpecial: "Special",
          categoryFavorites: "Favorites",
          categoryReview: "Review Mistakes",
          emptyFiltered: "No articles found.",
          articleCount: "{count} articles",
          finishCompleted: "Finished",
          finishTimeout: "Time Out",
          finishSummary: "{kpm} KPM · Accuracy {accuracy}% · {time} · {corrections} corrections",
          emptyHistory: "Recent stats will show here after practice.",
          reviewCount: "{count} to review"
        },
        codingTyping: {
          title: "Programming Typing Practice",
          subtitle: "Master programming syntax. Select language and category to begin.",
          language: "Language",
          category: "Category",
          difficultyFilter: "Difficulty",
          allDifficulties: "All",
          exerciseList: "Exercise List",
          practiceTitle: "Programming Typing",
          practiceDescription: "Type the target code exactly.",
          stepChoose: "1. Choose Language & Category",
          stepType: "2. Input target code",
          stepSandbox: "3. Send to Sandbox & Run",
          sendToSandbox: "Send to Sandbox",
          sendOnlyHelp: "Send code only, do not run",
          runInSandbox: "Run in Sandbox",
          runHelp: "Send code and run immediately",
          backToList: "Back to List",
          random: "Random Exercise",
          next: "Next Exercise",
          restart: "Restart",
          normalizeFullWidth: "Full-width -> Half-width",
          lenientIndent: "Lenient Indent",
          code: "Target Code",
          examInsight: "Exam Insight",
          meaning: "Meaning",
          memoryHook: "Memory Hook",
          examPoint: "Exam Point",
          commonMistake: "Common Mistake",
          input: "Input Area",
          inputPlaceholder: "Type code here...",
          speed: "CPM",
          accuracy: "Accuracy",
          progress: "Progress",
          time: "Time",
          errors: "Errors",
          historyFilter: "Filter History",
          allLanguages: "All Languages",
          allCategories: "All Categories",
          clearHistory: "Clear History"
        }
      }
    };

    // Provide English fallback values for locales that lack this patch's modules.
    // The merge below only fills MISSING keys, so existing native translations
    // are never overwritten (the old deep-copy+overwrite destroyed ~159 native strings).
    ["vi-VN", "th-TH", "fr-FR", "id-ID", "en-US"].forEach(function (loc) {
      if (!HOTFIX2_DICT_PATCH[loc]) {
        HOTFIX2_DICT_PATCH[loc] = JSON.parse(JSON.stringify(HOTFIX2_DICT_PATCH["my-MM"]));
      }
    });

    Object.assign(HOTFIX2_DICT_PATCH["fr-FR"].common, {
      confirm: "Confirmer",
      cancel: "Annuler",
      warning: "Avertissement",
      success: "Succès",
      error: "Erreur",
      back: "Retour"
    });
    Object.assign(HOTFIX2_DICT_PATCH["fr-FR"].codingTyping, {
      title: "Exercice de frappe de code",
      subtitle: "Maîtrisez la syntaxe en tapant du code. Choisissez une langue et une catégorie.",
      language: "Langue",
      category: "Catégorie",
      difficultyFilter: "Difficulté",
      allDifficulties: "Tous",
      exerciseList: "Liste des exercices",
      inputPlaceholder: "Tapez le code ici..."
    });

    Object.keys(HOTFIX2_DICT_PATCH).forEach(function (locale) {
      if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
      Object.keys(HOTFIX2_DICT_PATCH[locale]).forEach(function (module) {
        if (!window.I18nUiDict[locale][module]) window.I18nUiDict[locale][module] = {};
        var target = window.I18nUiDict[locale][module];
        var patch = HOTFIX2_DICT_PATCH[locale][module];
        Object.keys(patch).forEach(function (key) {
          if (!(key in target)) target[key] = patch[key];
        });
      });
    });
  })();

  /* ── P15.2：账号与同步（用户名认证 + 真实同步状态模型）── */
  (function () {
    var AUTH_SYNC_P15_2 = {
      "zh-CN": { auth: {
        panelTitle: "账号与同步",
        panelSubtitle: "登录后可在多设备同步学习进度与设置",
        optional: "可选",
        showPassword: "显示密码",
        hidePassword: "隐藏密码",
        loggingIn: "正在登录",
        registering: "正在创建",
        username: "用户名",
        usernamePlaceholder: "3-24 位字母、数字、下划线或连字符",
        usernameRequired: "请输入用户名",
        usernameTooShort: "用户名至少 3 位",
        usernameTooLong: "用户名最多 24 位",
        usernameInvalid: "用户名只能包含字母、数字、下划线和连字符",
        usernameOrPasswordWrong: "用户名或密码不正确",
        usernameTaken: "该用户名已被使用",
        displayNameOptional: "昵称（可选）",
        displayNameDefaultHint: "留空时使用用户名作为昵称",
        displayNameTooLong: "昵称最多 32 个字符",
        passwordRuleHint: "至少 6 位；建议使用独立密码并妥善保管",
        createAccountButton: "创建账号",
        noEmailRequired: "无需邮箱",
        noEmailRequiredDesc: "只需用户名和密码即可登录",
        syncNow: "立即同步",
        syncingNow: "正在同步",
        syncSuccess: "同步成功",
        syncFailed: "同步失败",
        syncScope: "只同步学习进度与设置",
        noAiKeyUpload: "AI 密钥不会上传",
        deviceAndSyncDetails: "设备与同步详情",
        usernameAccount: "用户名账号",
        legacyEmailAccount: "旧邮箱账号",
        syncStatus: { local: "本地", synced: "已同步", syncing: "同步中", offline: "离线", error: "同步异常" },
        syncState: {
          signedOutLocalTitle: "本地模式",
          signedOutLocalDesc: "当前学习记录保存在这台设备上，学习功能完全可用。登录后可开启多设备同步。",
          authenticatingTitle: "正在验证身份",
          authenticatingDesc: "正在与同步服务通信，请稍候。",
          pendingTitle: "已登录，尚未同步",
          pendingDesc: "不会自动同步。点击「立即同步」上传并获取学习进度。",
          syncingTitle: "正在同步",
          syncingDesc: "正在上传或获取学习进度，请不要关闭当前窗口。",
          syncedTitle: "已同步",
          syncedDesc: "你的学习进度与设置已同步到账户。",
          mergedTitle: "已同步（含合并）",
          mergedDesc: "已按「只增不删」策略合并远端与本地进度，本地记录不会被覆盖删除。",
          cloudUnavailableTitle: "云同步不可用",
          cloudUnavailableDesc: "当前无法连接同步服务。学习记录仍安全保存在本机，学习不受影响。",
          failedTitle: "同步失败",
          failedDesc: "本地记录没有丢失。你可以重试同步，或继续本地学习。",
          retrySync: "重试同步",
          retryConnect: "重试连接"
        }
      } },
      "ja-JP": { auth: {
        panelTitle: "アカウントと同期",
        panelSubtitle: "ログインすると学習進捗と設定を複数端末で同期できます",
        optional: "任意",
        showPassword: "パスワードを表示",
        hidePassword: "パスワードを隠す",
        loggingIn: "ログイン中",
        registering: "作成中",
        username: "ユーザー名",
        usernamePlaceholder: "3〜24 文字の英数字・アンダースコア・ハイフン",
        usernameRequired: "ユーザー名を入力してください",
        usernameTooShort: "ユーザー名は 3 文字以上です",
        usernameTooLong: "ユーザー名は 24 文字以内です",
        usernameInvalid: "英数字・アンダースコア・ハイフンのみ使用できます",
        usernameOrPasswordWrong: "ユーザー名またはパスワードが正しくありません",
        usernameTaken: "このユーザー名は既に使用されています",
        displayNameOptional: "表示名（任意）",
        displayNameDefaultHint: "空欄の場合はユーザー名を表示名として使います",
        displayNameTooLong: "表示名は 32 文字以内です",
        passwordRuleHint: "6 文字以上。使い回しのない安全なパスワードを推奨します",
        createAccountButton: "アカウント作成",
        noEmailRequired: "メールアドレス不要",
        noEmailRequiredDesc: "ユーザー名とパスワードだけでログインできます",
        syncNow: "今すぐ同期",
        syncingNow: "同期中",
        syncSuccess: "同期に成功しました",
        syncFailed: "同期に失敗しました",
        syncScope: "同期対象は学習進捗と設定のみです",
        noAiKeyUpload: "AI キーはアップロードされません",
        deviceAndSyncDetails: "端末と同期の詳細",
        usernameAccount: "ユーザー名アカウント",
        legacyEmailAccount: "旧メールアカウント",
        syncStatus: { local: "ローカル", synced: "同期済み", syncing: "同期中", offline: "オフライン", error: "同期エラー" },
        syncState: {
          signedOutLocalTitle: "ローカルモード",
          signedOutLocalDesc: "学習記録はこの端末に保存されており、学習機能はすべて利用できます。ログインすると複数端末の同期が使えます。",
          authenticatingTitle: "本人確認中",
          authenticatingDesc: "同期サービスと通信しています。しばらくお待ちください。",
          pendingTitle: "ログイン済み・未同期",
          pendingDesc: "自動では同期されません。「今すぐ同期」で進捗をアップロード・取得します。",
          syncingTitle: "同期中",
          syncingDesc: "学習進捗を送受信しています。このウィンドウを閉じないでください。",
          syncedTitle: "同期済み",
          syncedDesc: "学習進捗と設定はアカウントに同期されています。",
          mergedTitle: "同期済み（マージあり）",
          mergedDesc: "「追加のみ・削除なし」の方針でリモートとローカルの進捗をマージしました。ローカルの記録が消されることはありません。",
          cloudUnavailableTitle: "クラウド同期を利用できません",
          cloudUnavailableDesc: "同期サービスに接続できません。学習記録はこの端末に安全に保存されており、学習に影響はありません。",
          failedTitle: "同期に失敗しました",
          failedDesc: "ローカルの記録は失われていません。再試行するか、そのままローカルで学習を続けられます。",
          retrySync: "同期を再試行",
          retryConnect: "接続を再試行"
        }
      } },
      "en-US": { auth: {
        panelTitle: "Account & Sync",
        panelSubtitle: "Sign in to sync learning progress and settings across devices",
        optional: "optional",
        showPassword: "Show password",
        hidePassword: "Hide password",
        loggingIn: "Signing in",
        registering: "Creating account",
        username: "Username",
        usernamePlaceholder: "3-24 letters, numbers, underscores, or hyphens",
        usernameRequired: "Please enter a username",
        usernameTooShort: "Username must be at least 3 characters",
        usernameTooLong: "Username must be no more than 24 characters",
        usernameInvalid: "Use only letters, numbers, underscores, and hyphens",
        usernameOrPasswordWrong: "Username or password is incorrect",
        usernameTaken: "This username is already in use",
        displayNameOptional: "Display name (optional)",
        displayNameDefaultHint: "Leave empty to use your username",
        displayNameTooLong: "Display name must be no more than 32 characters",
        passwordRuleHint: "At least 6 characters; use a unique password",
        createAccountButton: "Create Account",
        noEmailRequired: "No email required",
        noEmailRequiredDesc: "Sign in with only your username and password",
        syncNow: "Sync now",
        syncingNow: "Syncing",
        syncSuccess: "Sync successful",
        syncFailed: "Sync failed",
        syncScope: "Only learning progress and settings are synced",
        noAiKeyUpload: "AI keys are never uploaded",
        deviceAndSyncDetails: "Device and sync details",
        usernameAccount: "Username account",
        legacyEmailAccount: "Legacy email account",
        syncStatus: { local: "Local", synced: "Synced", syncing: "Syncing", offline: "Offline", error: "Sync error" },
        syncState: {
          signedOutLocalTitle: "Local mode",
          signedOutLocalDesc: "Your learning data is stored on this device and everything keeps working. Sign in to enable multi-device sync.",
          authenticatingTitle: "Verifying identity",
          authenticatingDesc: "Contacting the sync service. Please wait.",
          pendingTitle: "Signed in, not synced yet",
          pendingDesc: "Nothing syncs automatically. Click “Sync now” to upload and fetch progress.",
          syncingTitle: "Syncing",
          syncingDesc: "Uploading and fetching learning progress. Please keep this window open.",
          syncedTitle: "Synced",
          syncedDesc: "Your learning progress and settings are synced to your account.",
          mergedTitle: "Synced (merged)",
          mergedDesc: "Remote and local progress merged additively — local records are never deleted.",
          cloudUnavailableTitle: "Cloud sync unavailable",
          cloudUnavailableDesc: "Cannot reach the sync service. Your data is safe on this device and learning is unaffected.",
          failedTitle: "Sync failed",
          failedDesc: "No local data was lost. You can retry the sync or keep learning locally.",
          retrySync: "Retry sync",
          retryConnect: "Retry connection"
        }
      } },
      "default-ja-zh": { auth: {
        panelTitle: "账号与同步 / アカウントと同期",
        panelSubtitle: "登录后可在多设备同步学习进度与设置 / ログインすると学習進捗と設定を複数端末で同期できます",
        optional: "可选 / 任意",
        showPassword: "显示密码 / パスワードを表示",
        hidePassword: "隐藏密码 / パスワードを隠す",
        loggingIn: "正在登录 / ログイン中",
        registering: "正在创建 / 作成中",
        username: "用户名 / ユーザー名",
        usernamePlaceholder: "3-24 位字母、数字、下划线或连字符 / 3〜24 文字の英数字・記号",
        usernameRequired: "请输入用户名 / ユーザー名を入力してください",
        usernameTooShort: "用户名至少 3 位 / 3 文字以上",
        usernameTooLong: "用户名最多 24 位 / 24 文字以内",
        usernameInvalid: "仅限字母、数字、下划线和连字符 / 英数字・_・- のみ",
        usernameOrPasswordWrong: "用户名或密码不正确 / ユーザー名またはパスワードが違います",
        usernameTaken: "该用户名已被使用 / このユーザー名は使用中です",
        displayNameOptional: "昵称（可选）/ 表示名（任意）",
        displayNameDefaultHint: "留空时使用用户名 / 空欄ならユーザー名を使用",
        displayNameTooLong: "昵称最多 32 字 / 表示名は 32 文字以内",
        passwordRuleHint: "至少 6 位，建议独立密码 / 6 文字以上・使い回しなし推奨",
        createAccountButton: "创建账号 / アカウント作成",
        noEmailRequired: "无需邮箱 / メールアドレス不要",
        noEmailRequiredDesc: "只需用户名和密码 / ユーザー名とパスワードだけ",
        syncNow: "立即同步 / 今すぐ同期",
        syncingNow: "正在同步 / 同期中",
        syncSuccess: "同步成功 / 同期成功",
        syncFailed: "同步失败 / 同期失敗",
        syncScope: "只同步学习进度与设置 / 同期対象は学習進捗と設定のみ",
        noAiKeyUpload: "AI 密钥不会上传 / AI キーはアップロードされません",
        deviceAndSyncDetails: "设备与同步详情 / 端末と同期の詳細",
        usernameAccount: "用户名账号 / ユーザー名アカウント",
        legacyEmailAccount: "旧邮箱账号 / 旧メールアカウント",
        syncStatus: { local: "本地 / ローカル", synced: "已同步 / 同期済み", syncing: "同步中 / 同期中", offline: "离线 / オフライン", error: "同步异常 / 同期エラー" },
        syncState: {
          signedOutLocalTitle: "本地模式 / ローカルモード",
          signedOutLocalDesc: "学习记录保存在这台设备上，学习功能完全可用。登录后可开启多设备同步。/ 学習記録はこの端末に保存され、機能はすべて利用できます。",
          authenticatingTitle: "正在验证身份 / 本人確認中",
          authenticatingDesc: "正在与同步服务通信，请稍候。/ 同期サービスと通信しています。",
          pendingTitle: "已登录，尚未同步 / ログイン済み・未同期",
          pendingDesc: "不会自动同步。点击「立即同步」。/ 自動同期はありません。「今すぐ同期」を押してください。",
          syncingTitle: "正在同步 / 同期中",
          syncingDesc: "正在传输学习进度，请不要关闭窗口。/ 進捗を送受信中です。ウィンドウを閉じないでください。",
          syncedTitle: "已同步 / 同期済み",
          syncedDesc: "学习进度与设置已同步到账户。/ 学習進捗と設定は同期されています。",
          mergedTitle: "已同步（含合并）/ 同期済み（マージあり）",
          mergedDesc: "按「只增不删」策略合并，本地记录不会被删除。/ 追加のみの方針でマージし、ローカル記録は消えません。",
          cloudUnavailableTitle: "云同步不可用 / クラウド同期を利用できません",
          cloudUnavailableDesc: "无法连接同步服务，学习记录安全保存在本机。/ 接続できませんが、記録はこの端末に安全に保存されています。",
          failedTitle: "同步失败 / 同期失敗",
          failedDesc: "本地记录没有丢失，可重试或继续本地学习。/ ローカル記録は失われていません。再試行できます。",
          retrySync: "重试同步 / 同期を再試行",
          retryConnect: "重试连接 / 接続を再試行"
        }
      } }
    };
    Object.keys(AUTH_SYNC_P15_2).forEach(function (locale) {
      if (!window.I18nUiDict[locale]) window.I18nUiDict[locale] = {};
      if (!window.I18nUiDict[locale].auth) window.I18nUiDict[locale].auth = {};
      Object.assign(window.I18nUiDict[locale].auth, AUTH_SYNC_P15_2[locale].auth);
    });
  })();
})();
