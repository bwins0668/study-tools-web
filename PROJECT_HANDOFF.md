...
* **最新线上 Commit**：`3b47b5c`
* **当前正在进行/已完成本地验证的阶段**：**阶段 9F (SG 题库 JSON 按需加载本地开发及 E2E 验证已全部通过，等待 commit 确认)**
...
[已完成] 阶段9F：SG 历年真题库 JSON 动态加载重构 (等待 Commit)
    ↓
[待启动] 阶段9G：IT Passport 历年真题库 JSON 动态加载重构
    ↓
[待启动] 阶段9H：Python 课程/Lessons JSON 动态加载重构 (体积约 1.2MB)
```
