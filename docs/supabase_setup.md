...
完成 Auth 配置、执行最新版 `tools/init_supabase.sql` 并确认 RLS 后，已登录用户可以在账号面板点击“立即同步”。

本轮只访问：

- `devices`：登记当前设备和最后同步时间。
- `user_settings`：同步语言与主题。
- `learning_progress`：先拉取并与本地完成课程、测验索引做 union merge，再推送合并结果。
- `quiz_results`：仅 push 已完成的测验索引。

请在 Supabase Dashboard 的 Table Editor 中确认这四张表存在，并在 Authentication 登录测试用户后检查 RLS policy 仍限定为：
...
如果项目曾执行旧版 SQL，请再次审查最新版草案。`quiz_results` 需要
`(user_id, subject, lesson_id, quiz_index, device_id)` 唯一索引，供手动同步幂等 upsert 使用。

同步不会自动运行，也不会删除本地进度。任何表缺失、RLS 拒绝或网络错误都会显示为友好失败；修正数据库后再手动点击即可。

绝不会上传 AI API Key、provider/model、Ollama URL、AI 翻译缓存、聊天消息或用户翻译。
