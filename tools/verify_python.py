...
def verify_python():
    print("=== Testing python_lessons.js ===")
    with open(os.path.join(os.path.dirname(__file__), "..", "data", "python_lessons.js"), encoding="utf-8") as f:
        content = f.read()

    idx = content.find("const PYTHON_LESSONS =")
    if idx == -1:
        print("ERROR: Could not find 'const PYTHON_LESSONS ='")
        return False
    
    json_str = content[idx + len("const PYTHON_LESSONS ="):].strip()
    if json_str.endswith(";"):
        json_str = json_str[:-1].strip()

    try:
        lessons = json.loads(json_str)
        print(f"SUCCESS: Loaded {len(lessons)} Python lessons successfully via json.loads!")
    except Exception as e:
        print(f"ERROR: JSON parsing failed: {e}")
        return False

    if len(lessons) != 255:
        print(f"ERROR: Expected 240 lessons, found {len(lessons)}")
        return False
    ...
    for i, lesson in enumerate(lessons):
        ...
        # Verify vocab and quiz are present
    ...
    if success:
        print("SUCCESS: All 255 Python lessons verified successfully!")
    return success

if __name__ == "__main__":
    if verify_python():
        print("Python database verification PASSED.")
    else:
        print("Python database verification FAILED.")
